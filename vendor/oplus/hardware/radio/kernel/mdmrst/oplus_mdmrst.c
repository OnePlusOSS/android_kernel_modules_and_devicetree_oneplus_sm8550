...
/*****yixue.ge ******/
#define MDMREASON_BUF_LEN 256
static wait_queue_head_t mdmreason_wq;
static int g_mdmreason_flag = 0;
...
{
        /*just for test*/
        if (count > MDMREASON_BUF_LEN - 1) {
                count = MDMREASON_BUF_LEN - 1;
        }
        if (count > *ppos) {
                count -= *ppos;
        }
...
unsigned int g_mdmrest_count = 0;
                ...
                size_t count, loff_t *ppos)
{
        ...
        if (copy_to_user(buf, &g_mdmrest_count, 1)) {
                return -EFAULT;
        }
        /*mdmrest_flg = 0;*/
        return strlen(tmp);
