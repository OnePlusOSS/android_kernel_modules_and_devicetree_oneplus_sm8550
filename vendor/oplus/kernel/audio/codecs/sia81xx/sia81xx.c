...
#define SIA81XX_CMD_SET_POWER_SCENE		(7)
...
static sia81xx_dev_t *g_default_sia_dev[4] = {NULL};
...
void distinguish_chip_type(sia81xx_dev_t *sia81xx)
{
#ifdef DISTINGUISH_CHIP_TYPE
	...
	const char *buf)
{
	...
	for (i = 0; i < len; i++) {
		...
		while (retry < MAX_OWI_RETRY_TIMES) {
			...
			ret = sia81xx_owi_write_one_byte(sia81xx, buf[i]);
			if (0 == ret)
				break;
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if (CHIP_TYPE_SIA8101 == sia81xx->chip_type
		...
		sia81xx_regmap_defaults(g_default_sia_dev[0]->regmap,
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if (is_chip_type_supported(sia81xx->chip_type) &&
		...
		if (0 == sia81xx->disable_pin) {
			if (CHIP_TYPE_SIA8101 == sia81xx->chip_type
				&& 0 != sia81xx->channel_num) {
				if (likely(NULL != g_default_sia_dev[0])) {
					default_sia_is_open = gpio_get_value(g_default_sia_dev[0]->rst_pin);
					if (1 == default_sia_is_open)
						sia81xx_suspend(g_default_sia_dev[0]);
				} else {
					pr_err("[  err][%s] %s: g_default_sia_dev[0] == NULL !!! \r\n",
						LOG_FLAG, __func__);
		...
		if (0 == sia81xx->disable_pin) {
			if (CHIP_TYPE_SIA8101 == sia81xx->chip_type
				&& 0 != sia81xx->channel_num) {
				if (1 == default_sia_is_open)
					sia81xx_resume(g_default_sia_dev[0]);
			}
		}
	}
...
int sia81xx_audio_power_scene_set(int8_t channle, int8_t scene_mode)
{
    sia81xx_dev_t *sia81xx;
	int ret = 0;

    pr_debug("[debug][%s] %s: channle = %d, scene_mode = %d \r\n",
		LOG_FLAG, __func__, channle, scene_mode);

    if (channle >= 0 && channle <= 3) {
		if (likely(NULL != g_default_sia_dev[channle])) {
			sia81xx = g_default_sia_dev[channle];
		} else {
			pr_err("[  err][%s] %s: g_default_sia_dev[%d] == NULL !!! \r\n",
				LOG_FLAG, __func__, channle);
			return EINVAL;
		}
	} else {
		pr_err("[  err][%s] %s: set channle val = %ld !!! \r\n",
			LOG_FLAG, __func__, channle);
		return EINVAL;
	}
	...
	if (0 == scene_mode) {
		...
		if (0 > scene_mode || AUDIO_SCENE_NUM < scene_mode) {
			sia81xx->scene = AUDIO_SCENE_PLAYBACK;
			pr_err("[  err][%s] %s: set scene_mode val = %ld !!! \r\n",
			LOG_FLAG, __func__, scene_mode);
		} else {
			sia81xx->scene = scene_mode - 1;
		}
	...
	char *buf)
{
	...
	sia81xx = (sia81xx_dev_t *)dev_get_drvdata(cd);
	if (sia81xx == NULL) {
		return 0;
	}
	...
	size_t len)
{
	...
	case SIA81XX_CMD_SET_POWER_SCENE:
	{
		int8_t channle;
		int8_t scene_mode;
		...
		scene_mode = (int8_t)simple_strtoul(
			strsep(&cur, split_symb), &after, 10);

		if (0 != sia81xx_audio_power_scene_set(channle, scene_mode)) {
			pr_err("[  err][%s] %s: power_scene_set \r\n",
				LOG_FLAG, __func__);
		}
		break;
	}
...
sia81xx_dev_t *find_sia81xx_dev(
	const char *name,
	...
	struct device_node	*sia81xx_of_node)
{
	...
	if (0 != ret) {
		...
		if (NULL == sia81xx) {
			...
			/* default name */
			snprintf(sia81xx->name, strlen("sia81xx-dummy.%u") + 1,
...
static const uint32_t sia81x9_list[] = {
	CHIP_TYPE_SIA8159,	// first chip reg range should cover all other chips
	CHIP_TYPE_SIA8159A,
	CHIP_TYPE_SIA8109
};
...
static int check_sia81xx_status(sia81xx_dev_t *sia81xx)
{
	...
	if (1 == sia81xx->en_dyn_id)
		ret = gpio_get_value(sia81xx->id_pin);
	else
	...
	const struct i2c_device_id *id)
{
	...
	ret = of_property_read_s32(client->dev.of_node, "si,sia81xx-addr_offset", &addr_offset);
	if (0 != ret) {
	...
	// for sia8101 stereo
	g_default_sia_dev[sia81xx->channel_num] = sia81xx;
...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	int disable_pin = 0, owi_pin = 0, rst_pin = 0, id_pin = 0, en_dyn_id = 0;
	...
	if (0 == disable_pin) {
		...
		ret = pinctrl_select_state(sia81xx_pinctrl, pinctrl_state);
		if (0 != ret) {
		...
		sia81xx->en_dyn_id = (unsigned int)en_dyn_id;


		if (1 == sia81xx->en_dyn_id) {
			pinctrl_state = pinctrl_lookup_state(sia81xx_pinctrl, "sia81xx_id");
			if (NULL == pinctrl_state) {
				pr_err("[  err][%s] %s: NULL == pinctrl_state !!! \r\n",
					LOG_FLAG, __func__);
				ret = -ENODEV;
			...
			/* get id pin's sn */
			id_pin = of_get_named_gpio(
				pdev->dev.of_node, "si,sia81xx_id", 0);
			if (id_pin < 0) {
			...
			sia81xx->id_pin = id_pin;
			gpio_direction_input(sia81xx->id_pin);
		}
	...
	soc_component_dev_sia81xx.name = sia81xx->name;
#endif
