...
static const struct reg_map_info reg_map_info_table[] = {
	...
	[CHIP_TYPE_SIA8159A] = {
		.config = &sia8159a_regmap_config,
		.reg_default = &sia8159a_reg_default_val,
		.opt = &sia8159a_opt_if
	}
};
	...
	char *buf)
{
	...
	if (1 == reg_num) {
		...
		if (0 != ret) {
			pr_err("[  err][%s] %s: regmap read err. ret = %d \r\n",
	...
	if (0 != ret) {
		pr_err("[  err][%s] %s: bulk read err. ret = %d \r\n",
			LOG_FLAG, __func__, ret);
	}
	return ret;
}
	...
	const char *buf)
{
	...
	if (1 == reg_num) {
		...
		if (0 != ret) {
			pr_err("[  err][%s] %s: regmap write err. ret = %d \r\n",
				LOG_FLAG, __func__, ret);
		}
		return ret;
	}
	...
	if (0 != ret) {
		pr_err("[  err][%s] %s: bulk write err. ret = %d \r\n",
			LOG_FLAG, __func__, ret);
	}
	return ret;
}
