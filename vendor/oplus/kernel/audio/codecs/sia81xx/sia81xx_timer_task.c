	...
	void *data)
{
	...
	pr_debug("[debug][%s] %s: cur_run_cnt %u\r\n",
					LOG_FLAG, __func__, timer->cur_run_cnt);
	...
	uint32_t hdl)
{
	...
	ret = wake_up_process(timer->task);
	if (ret < 0) {
	...
	uint32_t hdl)
{
	...
	i = 0;
	while (timer->cur_run_cnt != 0) {
		i++;
		msleep(1);

		if (i > 6*TIMER_TASK_CYCLE_TIME_MS)
			break;
	}
	timer->task = NULL;
