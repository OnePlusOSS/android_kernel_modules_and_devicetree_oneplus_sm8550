				...
				unsigned char data[])
{
	...
	if ((tfa->is_probus_device) && (tfa->channel_num == 1) &&
		(param_id == SB_PARAM_GET_RE25C ||
		 param_id == SB_PARAM_GET_LSMODEL ||
		 param_id == SB_PARAM_GET_ALGO_PARAMS ||
		 param_id == SB_PARAM_GET_EXCURSION_FILTERS ||
		 param_id == SB_PARAM_GET_ZFILTER ||
		 param_id == SB_PARAM_GET_XFILTER)) {
...
enum Tfa98xx_Error tfa98xxSetFresFrequency(struct tfa_device *tfa, uint16_t freq)
{
	...
	mtp_freq = (freq - 300) / 2;
	pr_err("\n tfa mtp_freq to be written:%d! \n", mtp_freq);
	return tfa98xx_set_mtp(tfa,mtp_freq<<7,0xFF80); /* val mask */
}
...
enum Tfa98xx_Error tfa_dsp_get_resonance_frequency(struct tfa_device *tfa, uint16_t *fres)
{
	...
	error = tfa_dsp_cmd_id_write_read(tfa, MODULE_SPEAKERBOOST, SB_PARAM_GET_FRES, 6, bytes);
	if (error == Tfa98xx_Error_Ok) {
		tfa98xx_convert_bytes2data(6, bytes, data);
	}
	...
	if (tfa_dev_mtp_get(tfa, TFA_MTP_OTC)) {
		...
		if (error == Tfa98xx_Error_Ok) {
			...
			for (i = 0; i < spkr_count; i++) {
                ...
                if (data[i] < 0) {
                    /* Negative impedance value from Firmware indicates an open-circuited speaker */
                    data[i] = 0;
                }
				scaled_data = data[i];
...
int tfa_dev_mtp_get(struct tfa_device *tfa, enum tfa_mtp item)
{
	...
	case TFA_MTP_RE25_SEC:
		...
		break;
	case TFA_MTP_F0:
		value = tfa_get_bf(tfa, TFA9873_BF_CUSTINFO);//all max 2 devices have same CUSTINFO @
		break;
	case TFA_MTP_LOCK:
		break;
	}
