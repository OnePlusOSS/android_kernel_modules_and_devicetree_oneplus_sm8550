...
#include "../sched_assist/sa_jankinfo.h"
...
#define LATENCY_TRACK_NUM					(64)
#define THROTTLE_TRACK_NUM                                      (64)
struct sched_record_struct {
	pid_t pid;
	u64   delta;
	struct timespec64 time_stamp;
};

static struct sched_record_struct lantency_struct[LATENCY_TRACK_NUM];
static struct sched_record_struct iowait_struct[LATENCY_TRACK_NUM];

struct ux_throttle_struct {
	pid_t pid;
...
static struct ux_throttle_struct ux_throttle_struct[THROTTLE_TRACK_NUM];

static void record_target_latency(struct task_struct *p, unsigned long type, u64 delta)
{
	struct sched_record_struct *cur;
	static int latency_counter;
	struct timespec64 ts;

	ktime_get_real_ts64(&ts);
	cur = &lantency_struct[(latency_counter++)%LATENCY_TRACK_NUM];
	cur->pid = p->pid;
	cur->delta = delta;
	cur->time_stamp.tv_sec = ts.tv_sec;
	cur->time_stamp.tv_nsec = ts.tv_nsec;
}

static void record_target_iowait(struct task_struct *p, unsigned long type, u64 delta)
{
	struct sched_record_struct *cur;
	static int iowait_counter;
	struct timespec64 ts;

	ktime_get_real_ts64(&ts);
	cur = &iowait_struct[(iowait_counter++)%LATENCY_TRACK_NUM];
	cur->pid = p->pid;
	cur->delta = delta;
	cur->time_stamp.tv_sec = ts.tv_sec;
	cur->time_stamp.tv_nsec = ts.tv_nsec;
			...
			unsigned long type, u64 time_in_ns)
{
	...
	if (type == TRACE_RUNNABLE && time_in_ns >= LATENCY_THRESHOLD)
		record_target_latency(task, type, time_in_ns);
...
{
	u32 idx, cpu;
	static int ux_throttle_counter;
	struct cpufreq_policy *policy;
	struct ux_throttle_struct *cur;
	struct timespec64 ts;

	if (is_task_traced(tsk, &idx) && tsk->on_cpu) {
		cur = &ux_throttle_struct[(ux_throttle_counter++)%THROTTLE_TRACK_NUM];
		ktime_get_real_ts64(&ts);
		cur->time_stamp.tv_sec = ts.tv_sec;
		cur->time_stamp.tv_nsec = ts.tv_nsec;
		cpu = task_cpu(tsk);
		...
		cur->pid = tsk->pid;
		memcpy(cur->comm, tsk->comm, TASK_COMM_LEN);
		cpufreq_cpu_put(policy);
	}
}

void osi_scheduler_tick_handler(void *unused, struct rq *rq)
{
...
int tasktrack_register_hook(void)
{
	...
	REGISTER_TRACE_RVH(android_vh_scheduler_tick, osi_scheduler_tick_handler);
	return ret;
}
...
int tasktrack_unregister_hook(void)
{
	...
	UNREGISTER_TRACE_VH(android_vh_binder_wait_for_work,
			android_vh_binder_wait_for_work_hanlder);
	UNREGISTER_TRACE_RVH(android_vh_scheduler_tick, osi_scheduler_tick_handler);
	return ret;
}
...
static int dump_callstack(struct seq_file *m, void *v)
{
	...
	if (!tasktrack_enable) {
		seq_puts(m, "Please enable task_track_enable!!!\n");
		return 0;
	}
	...
	for (i = 0; i < TASK_TRACK_NUM; i++) {
		...
		for (j = 0; j < JANK_WIN_CNT; j++) {
			...
			seq_printf(m, "%llu.%lu,%d;", (u64)cs->ts.tv_sec, cs->ts.tv_nsec, pid);
...
static int proc_sched_latency_show(struct seq_file *m, void *v)
{
	struct sched_record_struct *cur;
	int i;
...
static int proc_sched_iowait_show(struct seq_file *m, void *v)
{
	struct sched_record_struct *cur;
	int i;
...
static int proc_ux_throttle_show(struct seq_file *m, void *v)
{
	struct ux_throttle_struct *cur;
	int i;
...
static int proc_sched_latency_open(struct inode *inode,
	struct file *file)
{
	return single_open(file, proc_sched_latency_show, inode);
}

static int proc_sched_iowait_open(struct inode *inode,
		struct file *file)
{
	return single_open(file, proc_sched_iowait_show, inode);
}

static int proc_ux_throttle_open(struct inode *inode,
		struct file *file)
{
	return single_open(file, proc_ux_throttle_show, inode);
}

const struct proc_ops proc_task_track_operations = {
...
const struct proc_ops proc_task_track_enable_operations = {
...
static const struct proc_ops proc_sched_latency_operations = {
	.proc_open      =       proc_sched_latency_open,
	.proc_read      =       seq_read,
	.proc_lseek     =       seq_lseek,
	.proc_release   =       single_release,
};

static const struct proc_ops proc_sched_iowait_operations = {
	.proc_open      =       proc_sched_iowait_open,
	.proc_read      =       seq_read,
	.proc_lseek     =       seq_lseek,
	.proc_release   =       single_release,
};
...
static const struct proc_ops proc_ux_throttle_operations = {
	.proc_open      =       proc_ux_throttle_open,
	.proc_read      =       seq_read,
	.proc_lseek     =       seq_lseek,
	.proc_release   =       single_release,
};
			...
			struct proc_dir_entry *pde)
{
	...
	entry = proc_create("sched_latency", S_IRUGO,
		pde, &proc_sched_latency_operations);
	if (!entry) {
		pr_err("create sched_latency fail\n");
		goto err_sched_latency;
	}

	entry = proc_create("sched_iowait", S_IRUGO,
		pde, &proc_sched_iowait_operations);
	if (!entry) {
		pr_err("create sched_iowait fail\n");
		goto err_sched_iowait;
	}

	entry = proc_create("ux_throttle", S_IRUGO,
				pde, &proc_ux_throttle_operations);
	if (!entry) {
		pr_err("create ux_throttle fail\n");
...
err_sched_latency:
	remove_proc_entry("sched_latency", pde);

err_task_track_enable:
	remove_proc_entry("task_track", pde);

err_sched_iowait:
	remove_proc_entry("sched_iowait", pde);

err_ux_throttle:
	remove_proc_entry("ux_throttle", pde);
