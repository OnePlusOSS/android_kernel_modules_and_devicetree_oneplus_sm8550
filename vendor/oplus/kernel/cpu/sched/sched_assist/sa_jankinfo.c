			...
			struct task_struct *prev, struct task_struct *next, struct rq *rq)
{
	...
	if (prev) {
		if (prev->__state == TASK_RUNNING || prev->__state == TASK_WAKING) {
			nowtype = TRACE_RUNNABLE;
		} else if (prev->__state & TASK_UNINTERRUPTIBLE) {
			nowtype = TRACE_DISKSLEEP;
		} else if (prev->__state & TASK_INTERRUPTIBLE) {
			nowtype = TRACE_SLEEPING;
		} else if (prev->__state & TASK_DEAD) {
			/* TODO */
			nowtype = TRACE_OTHER;
		} else {
			/* FIXME */
			nowtype = TRACE_OTHER;
#ifdef JANKINFO_DEBUG
			jank_systrace_print(prev->pid, "dbg999_state", prev->__state);
