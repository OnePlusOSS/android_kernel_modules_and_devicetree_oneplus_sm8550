...
static void android_vh_pcpu_rwsem_wait_handler(void *unused, struct percpu_rw_semaphore *sem)
{
	update_locking_time(jiffies, false);
}
#endif
...
/*
	...
	ret = register_trace_android_vh_pcpu_rwsem_wait(
					android_vh_pcpu_rwsem_wait_handler, NULL);
	if (ret != 0) {
		pr_err("android_vh_pcpu_rwsem_wait failed! ret=%d\n", ret);
		goto out3;
	}
#endif
*/
