...
#include "../../../drivers/misc/mediatek/sched/mtk_energy_model/v2/energy_model.h"
#else
...
#if IS_ENABLED(CONFIG_MTK_GEARLESS_SUPPORT)
static ssize_t show_static_leakage(struct oplus_cpufreq_health *och, char *buf)
{
	unsigned long flags;
	int count = 0;
	...
	spin_lock_irqsave(&och->lock, flags);
	count += snprintf(buf + count, PAGE_SIZE - count, "%u, %u, %u, %u",
		och->leakage_info.a, och->leakage_info.b, och->leakage_info.c, och->leakage_info.power);
	spin_unlock_irqrestore(&och->lock, flags);
	return count;
}
#endif
...
#endif
#if IS_ENABLED(CONFIG_MTK_GEARLESS_SUPPORT)
oplus_cpufreq_health_attr_ro(static_leakage);
#endif
...
#if IS_ENABLED(CONFIG_MTK_GEARLESS_SUPPORT)
static int get_static_leakage(int cluster_id)
{
        ...
        och = &oplus_cpufreq_health[cluster_id];
        pd = &mtk_em_pd_ptr_public[cluster_id];
        och->leakage_info.a = pd->table[input_opp].leakage_para.a_b_para.a;
        och->leakage_info.b = pd->table[input_opp].leakage_para.a_b_para.b;
        och->leakage_info.c = pd->table[input_opp].leakage_para.c;
        och->leakage_info.power = input_temp * (input_temp * och->leakage_info.a - och->leakage_info.b) + och->leakage_info.c;
        och->leakage_info.power /= (cluster_id > 0) ? 10 : 100;
...
module_init(oplus_cpufreq_health_init);
