...
static int set_ua_status(void __user *uarg)
{
	...
	pr_info("[ua_ioctl] set_ua_status: status=%d\n", info.status);
...
static const struct proc_ops cpu_ioctrl_fops = {
	.proc_ioctl	= cpu_ioctl,
	.proc_open	= cpu_ctrl_open,
	.proc_release	= cpu_ctrl_release,
#ifdef CONFIG_COMPAT
	.proc_compat_ioctl	= cpu_compat_ioctl,
#endif
