...
static unsigned long uag_gov_get_util(struct uag_gov_cpu *sg_cpu)
{
	...
	sg_cpu->max = max;
	sg_cpu->reasons = 0;
	util = cpu_util_freq_walt(sg_cpu->cpu, &sg_cpu->walt_load, &sg_cpu->reasons);
	sg_cpu->bw_dl = cpu_bw_dl(rq);
...
void set_soft_limit_freq(struct cpufreq_policy *policy, unsigned int soft_freq)
{
	...
	uag_systrace_c(soft_freq, first_cpu, "frameboost_limit");
				...
				unsigned long *util, unsigned long *max)
{
	...
	trace_hispeed_freq(sg_policy->hispeed_util, sg_cpu->prev_frame_loading, *util);
				...
				unsigned long nl, unsigned long *util, unsigned long *max)
{
	...
	if (is_hiload && !is_migration)
		max_and_reason(util, sg_policy->hispeed_util, sg_cpu, CPUFREQ_REASON_HISPEED);

	if (is_hiload && nl >= mult_frac(cpu_util, NL_RATIO, 100))
		max_and_reason(util, *max, sg_cpu, CPUFREQ_REASON_NWD);
...
#else
static void uag_gov_update_single(struct waltgov_callback *cb, u64 time,
				unsigned int flags)
{
	struct uag_gov_cpu *sg_cpu = container_of(cb, struct uag_gov_cpu, cb);
	unsigned long nl;
#endif
	struct uag_gov_policy *sg_policy = sg_cpu->sg_policy;
	...
	if (eas_opt_enable && (util_thresh_percent[cluster_id] != 100)) {
		...
		if (unlikely(eas_opt_debug_enable))
			trace_printk("[eas_opt]: cluster_id: %d, capacity: %d, util_thresh: %d, util_orig: %d, "
					"util: %d, avg_nr_running: %d, fake_cap_multiple: %d,nr_fake_cap_multiple: %d, util_thresh: %d\n",
...
static unsigned int uag_gov_next_freq_shared(struct uag_gov_cpu *sg_cpu, u64 time)
{
	...
	if (eas_opt_enable && (util_thresh_percent[cluster_id] != 100) && count_cpu) {
		...
		util = (util_thresh < util) ?
			(util_thresh + ((avg_nr_running * (util-util_thresh) * nr_fake_cap_multiple[cluster_id]) >> SCHED_CAPACITY_SHIFT)) : util;
		if (unlikely(eas_opt_debug_enable))
			trace_printk("[eas_opt]: cluster_id: %d, util_orig: %d, util: %d, avg_nr_running: %d, fake_cap_multiple: %d,nr_fake_cap_multiple: %d, util_thresh: %d\n",
					cluster_id, util_orig, util, avg_nr_running, fake_cap_multiple[cluster_id], nr_fake_cap_multiple[cluster_id], util_thresh_percent[cluster_id]);
	}
#endif
...
uag_gov_update_shared(struct update_util_data *hook, u64 time, unsigned int flags)
{
	struct uag_gov_cpu *sg_cpu = container_of(hook, struct uag_gov_cpu, update_util);
#else
uag_gov_update_shared(struct waltgov_callback *cb, u64 time, unsigned int flags)
{
	struct uag_gov_cpu *sg_cpu = container_of(cb, struct uag_gov_cpu, cb);
#endif
	struct uag_gov_policy *sg_policy = sg_cpu->sg_policy;
...
EXPORT_SYMBOL(check_freq_update_for_time_uag);
#endif /* CONFIG_ARCH_MEDIATEK */
...
static int uag_gov_init(struct cpufreq_policy *policy)
{
	...
	tunables->multi_tl[UA_UTIL_SYS].ntarget_loads = ARRAY_SIZE(default_target_loads_sys);
...
static void unregister_uag_hooks(void)
{
#ifdef CONFIG_ARCH_MEDIATEK
	/* unregister vendor hook in kernel/drivers/cpuidle/cpuidle.c */
	unregister_trace_android_vh_cpu_idle_enter(android_vh_cpu_idle_enter_handler, NULL);
	unregister_trace_android_vh_cpu_idle_exit(android_vh_cpu_idle_exit_handler, NULL);
#endif /* CONFIG_ARCH_MEDIATEK */
...
static const struct proc_ops multi_tl_stat_ops = {
	.proc_open	= multi_tl_stat_open_proc,
	.proc_read	= seq_read,
