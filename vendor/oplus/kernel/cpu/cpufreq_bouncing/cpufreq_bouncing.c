...
static int cb_pol_idx, cb_switch;
...
void cb_update(struct cpufreq_policy *pol, u64 time)
{
	...
	if (!enable || cb_switch != NR_CLUS_MAX)
		return;
...
static int cb_parse_cpufreq(struct cpufreq_policy *pol)
{
	...
	cb_idx = topology_physical_package_id(cpu_dev->id);
	...
	pr_info("cb_parse_cpufreq first_cpu %d, cb_idx %d cb_switch %d cb_pol_idx %d freq_levels %d max_level %d idx %d \n",
			first_cpu, cb_idx, cb_switch, cb_pol_idx, cb->freq_levels, cb->max_level, idx);
...
void cb_stuff_init(struct cpufreq_policy *policy)
{
	...
	if ((cb_idx == NR_CLUS_MAX-1) && (cb_switch == NR_CLUS_MAX)) {
		cb_task = kthread_run(cb_main, NULL, "cb_task");
		pr_err("cb stuff init sucessful\n");
	}
}
EXPORT_SYMBOL(cb_stuff_init);
