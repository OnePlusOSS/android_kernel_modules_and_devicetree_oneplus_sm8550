...
static int explorer_sdio_suspend(struct device *pdev)
{
	int err;
	...
	if (err) {
		pr_err("%s: error while trying to keep power\n", __func__);
		return err;
	}
	smp_mb();
...
static int explorer_sdio_resume(struct device *pdev)
{
	struct sdio_func *func = dev_to_sdio_func(pdev);
...
static const struct dev_pm_ops explorer_sdio_pm_ops = {
	.suspend    = explorer_sdio_suspend,
	.resume    = explorer_sdio_resume,
};
