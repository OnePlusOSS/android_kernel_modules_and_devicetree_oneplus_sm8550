 ...
 *
 * Helper to forward the custom command to the device
 *
 * @param
 *    [ in] tcm_dev:        the device handle
...
/**
 * syna_tcm_get_extra_rc_byte()
 *
 * this function is used to return the extra rc byte retrieved previously
 *
 * @param
 *    [ in] tcm_dev: the device handle
 *
...
unsigned char syna_tcm_get_extra_rc_byte(struct tcm_dev *tcm_dev)
{
	...
	return tcm_dev->msg_data.rc_byte;
}
