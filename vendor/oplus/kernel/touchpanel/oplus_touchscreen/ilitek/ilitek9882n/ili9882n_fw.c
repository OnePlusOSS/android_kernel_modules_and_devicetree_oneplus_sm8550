...
static int calc_hw_dma_crc(u32 start_addr, u32 block_size)
{
    ...
    if (ilits9882n->chip->dma_reset) {
        ILI_DBG("operate dma reset in reg after tp reset\n");

        if (ili9882n_ice_mode_write(0x40040, 0x00800000, 4) < 0) {
            ILI_ERR("Failed to open DMA reset\n");
        }

        if (ili9882n_ice_mode_write(0x40040, 0x00000000, 4) < 0) {
            ILI_ERR("Failed to close DMA reset\n");
        }
    }

    /* dma1 src1 address */
    if (ili9882n_ice_mode_write(0x072104, start_addr, 4) < 0) {
        ILI_ERR("Write dma1 src1 address failed\n");
    }

    /* dma1 src1 format */
    if (ili9882n_ice_mode_write(0x072108, 0x80000001, 4) < 0) {
        ILI_ERR("Write dma1 src1 format failed\n");
    }

    /* dma1 dest address */
    if (ili9882n_ice_mode_write(0x072114, 0x0002725C, 4) < 0) {
        ILI_ERR("Write dma1 src1 format failed\n");
    }

    /* dma1 dest format */
    if (ili9882n_ice_mode_write(0x072118, 0x80000000, 4) < 0) {
        ILI_ERR("Write dma1 dest format failed\n");
    }

    /* Block size*/
    if (ili9882n_ice_mode_write(0x07211C, block_size, 4) < 0) {
        ILI_ERR("Write block size (%d) failed\n",
                block_size);
    }

    /* crc off */
    if (ili9882n_ice_mode_write(0x041016, 0x00, 1) < 0) {
        ILI_INFO("Write crc of failed\n");
    }

    /* dma crc */
    if (ili9882n_ice_mode_write(0x041017, 0x03, 1) < 0) {
        ILI_ERR("Write dma 1 crc failed\n");
    }

    /* Dma1 stop */
    if (ili9882n_ice_mode_write(0x072100, 0x02000000,
                           4) < 0) {
        ILI_ERR("Write dma1 stop failed\n");
    }

    /* crc on */
    if (ili9882n_ice_mode_write(0x041016, 0x01, 1) < 0) {
        ILI_ERR("Write crc on failed\n");
    }

    /* clr int */
    if (ili9882n_ice_mode_write(0x048006, 0x2, 1) < 0) {
        ILI_ERR("Write clr int failed\n");
    }

    /* Dma1 start */
    if (ili9882n_ice_mode_write(0x072100, 0x01000000,
                           4) < 0) {
    ...
    while (count > 0) {
        ...
        if (ili9882n_ice_mode_read(0x048006, &busy,
                              sizeof(u8)) < 0) {
    ...
    if (ili9882n_ice_mode_read(0x04101C, &busy,
                          sizeof(u32)) < 0) {
                                    ...
                                    u32 start, int len)
{
    ...
    for (cnt = 0, addr = start; cnt < loop;
        ...
        if (ilits9882n->wrapper(cmd, 4, NULL, 0, OFF,
                           OFF) < 0) {
            ILI_ERR("Failed to write iram data\n");
            return -ENODEV;
        }

        if (ilits9882n->wrapper(NULL, 0, buf + cnt * limit,
                           tmp_len, OFF, OFF) < 0) {
...
int ili9882n_fw_dump_iram_data(u32 start, u32 end,
                          bool save, bool mcu)
{
    struct file *f = NULL;
    mm_segment_t old_fs;
    loff_t pos = 0;
    int i, ret = 0;
    int len, tmp = ili9882n_debug_en;
    bool ice = atomic_read(&ilits9882n->ice_stat);

    if (!ice) {
        ret = ili9882n_ice_mode_ctrl(ENABLE, mcu);
    ...
    for (i = 0; i < MAX_HEX_FILE_SIZE; i++) {
        ilits9882n->update_buf[i] = 0xFF;
    }

    ret = ilitek_tddi_fw_iram_read(ilits9882n->update_buf,
                                   start, len);
    ...
    if (save) {
        ...
        set_fs(KERNEL_DS);
        pos = 0;
        vfs_write(f, ilits9882n->update_buf, len, &pos);
        set_fs(old_fs);
        filp_close(f, NULL);
        ...
        ili9882n_dump_data(ilits9882n->update_buf, 8, len, 0,
                      "IRAM");
                                       ...
                                       u8 *w_buf, u32 w_len, u32 split_len)
{
    ...
    if (split_len != 0) {
        for (addr = start, i = 0; addr < end;
            ...
            ilits9882n->update_buf[0] = SPI_WRITE;
            ilits9882n->update_buf[1] = 0x25;
            ilits9882n->update_buf[2] = (char)((addr &
                                           0x000000FF));
            ilits9882n->update_buf[3] = (char)((addr & 0x0000FF00)
                                          >> 8);
            ilits9882n->update_buf[4] = (char)((addr & 0x00FF0000)
                                          >> 16);

            for (j = 0; j < split_len; j++) {
                ilits9882n->update_buf[5 + j] = w_buf[i + j];
            }

            if (fix_4_alignment) {
                ILI_INFO("org split_len = 0x%X\n", split_len);
                ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                         split_len - 4, ilits9882n->update_buf[5 + split_len -
                                                            4]);
                ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                         split_len - 3, ilits9882n->update_buf[5 + split_len -
                                                            3]);
                ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                         split_len - 2, ilits9882n->update_buf[5 + split_len -
                                                            2]);
                ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                         split_len - 1, ilits9882n->update_buf[5 + split_len -
                                                            1]);

                for (j = 0; j < (4 - (split_len % 4)); j++) {
                    ilits9882n->update_buf[5 + j + split_len] = 0xFF;
                    ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                             j + split_len, ilits9882n->update_buf[5 + j +
                                                                split_len]);
                }
            ...
            if (ilits9882n->spi_write_then_read(ilits9882n->spi,
                                           ilits9882n->update_buf, split_len + 5, NULL, 0)) {
        ...
        ilits9882n->update_buf[0] = SPI_WRITE;
        ilits9882n->update_buf[1] = 0x25;
        ilits9882n->update_buf[2] = (char)((start &
                                       0x000000FF));
        ilits9882n->update_buf[3] = (char)((start & 0x0000FF00)
                                      >> 8);
        ilits9882n->update_buf[4] = (char)((start & 0x00FF0000)
                                      >> 16);
        memcpy(&ilits9882n->update_buf[5], w_buf, w_len);
        ...
        if (ilits9882n->spi_write_then_read(ilits9882n->spi,
                                       ilits9882n->update_buf, w_len + 5, NULL, 0) < 0) {
            ILI_ERR("Failed to write data via SPI in host download (%x)\n",
                    w_len + 5);
            return -EIO;
        }
                                       ...
                                       bool mcu)
{
    ...
    if (!ilits9882n->ddi_rest_done) {
        if (ilits9882n->actual_tp_mode !=
        ...
        ilits9882n->ddi_rest_done = false;
    }
    ...
    if (ilits9882n->actual_tp_mode == P5_X_FW_TEST_MODE) {
        mode = MP;
    } else if (ilits9882n->actual_tp_mode ==
               P5_X_FW_GESTURE_MODE) {
        mode = GESTURE;
        crc_len = 0;
    } else {
    ...
    for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
        if (fbi[i].mode == mode && fbi[i].len != 0) {
            ...
            if ((crc != dma) || iram_crc_err) {
                ...
                ili9882n_fw_dump_iram_data(0x0, 0xF, false, OFF);
                return -EFW_CRC;
            }
        }
    }
    ...
    if (ilits9882n->actual_tp_mode !=
        P5_X_FW_GESTURE_MODE) {
        if (ili9982n_reset_ctrl(TP_IC_CODE_RST) < 0) {
    ...
    if (!ilits9882n->info_from_hex
        || (ilits9882n->chip->core_ver < CORE_VER_1410)) {
        mdelay(100);
    }
    ...
    u8 *pfw)
{
    ...
    ILI_INFO("Parsing hex info start addr = 0x%x\n",
             fw_info_addr);
    ipio_memcpy(ilits9882n->fw_info, (pfw + fw_info_addr),
                sizeof(ilits9882n->fw_info), sizeof(ilits9882n->fw_info));
    /* copy fw mp ver */
    fw_mp_ver_addr = fbi[MP].end - INFO_MP_HEX_ADDR;
    ILI_INFO("Parsing hex mp ver addr = 0x%x\n",
             fw_mp_ver_addr);
    ipio_memcpy(ilits9882n->fw_mp_ver,
                pfw + fw_mp_ver_addr, sizeof(ilits9882n->fw_mp_ver),
                sizeof(ilits9882n->fw_mp_ver));
    /* copy fw core ver */
    ilits9882n->chip->core_ver = (ilits9882n->fw_info[68] << 24)
                            | (ilits9882n->fw_info[69] << 16) |
                            (ilits9882n->fw_info[70] << 8) | ilits9882n->fw_info[71];
    ILI_INFO("New FW Core version = %x\n",
             ilits9882n->chip->core_ver);
    /* Get hex fw vers */
    tfd.new_fw_cb = (ilits9882n->fw_info[48] << 24) |
                    (ilits9882n->fw_info[49] << 16) |
                    (ilits9882n->fw_info[50] << 8) | ilits9882n->fw_info[51];
    /* Get hex report info block*/
    ipio_memcpy(&ilits9882n->rib, ilits9882n->fw_info,
                sizeof(ilits9882n->rib), sizeof(ilits9882n->rib));
    ILI_INFO("report_info_block : nReportByPixel = %d, nIsHostDownload = %d, nIsSPIICE = %d, nIsSPISLAVE = %d\n",
             ilits9882n->rib.nReportByPixel,
             ilits9882n->rib.nIsHostDownload, ilits9882n->rib.nIsSPIICE,
             ilits9882n->rib.nIsSPISLAVE);
    ILI_INFO("report_info_block : nIsI2C = %d, nReserved00 = %d, nReserved01 = %x, nReserved02 = %x,  nReserved03 = %x\n",
             ilits9882n->rib.nIsI2C, ilits9882n->rib.nReserved00,
             ilits9882n->rib.nReserved01, ilits9882n->rib.nReserved02,
             ilits9882n->rib.nReserved03);
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
    ...
    if (!ilits9882n->p_firmware_headfile) {
        ILI_ERR("get idev->p_firmware_headfile is NULL\n");
        return -1;
    }

    if (!ilits9882n->p_firmware_headfile->firmware_data
    ...
    CTPM_FW = ilits9882n->p_firmware_headfile->firmware_data;
    size = ilits9882n->p_firmware_headfile->firmware_size;
    ...
    for (i = 0; i < FW_BLOCK_INFO_NUM - 1; i++) {
        ...
        if (((blk_map >> i) & 0x01) == 0x01) {
            ...
            if (num == GESTURE) {
                ilits9882n->gesture_load_code = true;
            }
        }
    }
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
    ...
    if (!ilits9882n->oplus_fw_update) {
        ...
        ilits9882n->tp_fw.size = 0;
        ilits9882n->tp_fw.data = vmalloc(fsize);

        if (ERR_ALLOC_MEM(ilits9882n->tp_fw.data)) {
        ...
        set_fs(KERNEL_DS);
        pos = 0;
        vfs_read(f, (u8 *)ilits9882n->tp_fw.data, fsize, &pos);
        set_fs(old_fs);
        filp_close(f, NULL);
        ilits9882n->tp_fw.size = fsize;
    } else {
    ...
    if (ilitek_tddi_fw_hex_convert((u8 *)
                                   ilits9882n->tp_fw.data, ilits9882n->tp_fw.size, pfw) < 0) {
        ILI_ERR("Convert hex file failed\n");
        ret = -1;
    }

out:
    ili_vfree((void **) & (ilits9882n->tp_fw.data));
    return ret;
}

int ili9882n_fw_upgrade(int op)
{
    int i, ret = 0, retry = 3;
    static bool set_esd_check = true;

    if (ilits9882n->oplus_fw_update
        || ilits9882n->force_fw_update || ERR_ALLOC_MEM(pfw)) {
        ilits9882n->gesture_load_code = false;
        ...
        if (ilits9882n->chip->core_ver >= CORE_VER_1470
            && ilits9882n->rib.nIsHostDownload == 0) {
    ...
    if (ret != UPDATE_PASS) {
        ...
        ilits9882n->xch_num = 0;
        ilits9882n->ych_num = 0;
        return ret;
    }
...
out:
    ilits9882n->sleep_type = NOT_SLEEP_MODE;
    ...
    if (ilits9882n->debug_mode_sta
        && ilits9882n->actual_tp_mode !=
        P5_X_FW_GESTURE_MODE) {
        if (ilits9882n->ts->apk_op
            && ilits9882n->ts->apk_op->apk_debug_set) {
            ilits9882n->ts->apk_op->apk_debug_set((void *)ilits9882n,
                                             true);
        }
    }
