...
{
    ili7807s_dump_data(max_ptr, 32, core_mp.frame_len, core_mp.xch_len, "Dump Benchmark Max : ");
    ili7807s_dump_data(min_ptr, 32, core_mp.frame_len, core_mp.xch_len, "Dump Benchmark Min : ");
}

static void dump_node_type_buffer(s32 *node_ptr, u8 *name)
{
    ili7807s_dump_data(node_ptr, 32, core_mp.frame_len, 0, "Dump NodeType : ");
}
...
static int ilitek_tddi_mp_ini_parser(const char *path)
{
    ...
    if (request_firmware(&ini, path, ilits7807s->dev) < 0) {
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line, int file_size)
{
    ...
    tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
                        "This is %s module\n",
                        ilits7807s->ts->panel_data.manufacture_info.manufacture);
    tmp_line++;
...
static s32 open_c_formula(int inCap1DAC, int inCap1Raw, int accuracy)
{
    ...
    int inVadc_range = 36;
    int inVbk = 39;
    int inCbk_step = open_para_7807s.cbk_step;/* from mp.ini */
    int inCint = open_para_7807s.cint;/* from mp.ini */
    int inVdrv = open_para_7807s.tvch - open_para_7807s.tvcl;/* from mp.ini */
    int inGain = open_para_7807s.gain;/*  from mp.ini */
...
static void allnode_open_cdc_result(int index, int *buf, int *dac, int *raw)
{
    ...
    if (ipio_strcmp(desp, "open test(integration)_sp") == 0) {
        for (i = 0; i < core_mp.frame_len; i++) {
            buf[i] = open_sp_formula(dac[i], raw[i], open_para_7807s.tvch, open_para_7807s.tvcl);
        }
    } else if (ipio_strcmp(desp, "open test_c") == 0) {
        if (parser_get_int_data(desp, "accuracy", str, sizeof(str)) > 0) {
            accuracy = ili7807s_katoi(str);
        }
...
static int codeToOhm(s32 *ohm, s32 *Code, u16 *v_tdf, u16 *h_tdf)
{
    u16 id = core_mp.chip_id;
    u8 type = core_mp.chip_type;
    int inTVCH = short_para_7807s.tvch;
    int inTVCL = short_para_7807s.tvcl;
    int inVariation = short_para_7807s.variation;
    int inTDF1 = 0;
    int inTDF2 = 0;
    int inCint = short_para_7807s.cint;
    int inRinternal = short_para_7807s.rinternal;
    s32 temp = 0;
    int j = 0;
...
static int short_test(int index, int frame_index)
{
    ...
    short_para_7807s.tvch = ili7807s_katoi(str);
    parser_get_int_data("short test", "tvcl", str, sizeof(str));
    short_para_7807s.tvcl = ili7807s_katoi(str);
    parser_get_int_data("short test", "variation", str, sizeof(str));
    short_para_7807s.variation = ili7807s_katoi(str);
    parser_get_int_data("short test", "cint", str, sizeof(str));
    short_para_7807s.cint = ili7807s_katoi(str);
    parser_get_int_data("short test", "rinternal", str, sizeof(str));
    short_para_7807s.rinternal = ili7807s_katoi(str);
    ...
    if ((pid != 0x988117) && (pid != 0x988118) && (pid != 0x78071A) && (pid != 0x78071C)) {
        if ((short_para_7807s.cint == 0) || (short_para_7807s.rinternal == 0)) {
    ...
    ILI_INFO("TVCH = %d, TVCL = %d, Variation = %d, V_TDF1 = %d, V_TDF2 = %d, H_TDF1 = %d, H_TDF2 = %d, Cint = %d, Rinternal = %d\n"
             , short_para_7807s.tvch, short_para_7807s.tvcl, short_para_7807s.variation, tItems[index].v_tdf_1,
             tItems[index].v_tdf_2, tItems[index].h_tdf_1, tItems[index].h_tdf_2, short_para_7807s.cint,
             short_para_7807s.rinternal);

    if (core_mp.protocol_ver >= PROTOCOL_VER_540) {
        /* Calculate code to ohm and save to tItems[index].buf */
        ili7807s_dump_data(frame_buf, 10, core_mp.frame_len, core_mp.xch_len, "Short Raw 1");
        codeToOhm(&tItems[index].buf[frame_index * core_mp.frame_len], frame_buf, v_tdf, h_tdf);
        ili7807s_dump_data(&tItems[index].buf[frame_index * core_mp.frame_len], 10, core_mp.frame_len,
...
static int allnode_key_cdc_data(int index)
{
    ...
    ret = ilits7807s->wrapper(cmd, sizeof(cmd), ori, len, ON, ON);
    ...
    ili7807s_dump_data(ori, 8, len, 0, "Key CDC original");
    ...
    ili7807s_dump_data(key_buf, 32, core_mp.frame_len, core_mp.xch_len, "Key CDC combined data");
out:
    ili_kfree((void **)&ori);
    return ret;
}
...
static int allnode_open_cdc_data(int mode, int *buf)
{
    ...
    ili7807s_dump_data(cmd, 8, sizeof(cmd), 0, "Open SP command");
    /* Allocate a buffer for the original */
    ori = kcalloc(len, sizeof(u8), GFP_KERNEL);
	...
	if (!ilits7807s->eng_flow) {
		if (ilits7807s->wrapper(cmd, core_mp.cdc_len, ori, len, ON, ON) < 0) {
		...
		msleep(200);
		if (ilits7807s->wrapper(cmd, core_mp.cdc_len, ori, len, OFF, OFF) < 0) {
			ILI_ERR("Failed to get cdc data\n");
		}
    }

    ili7807s_dump_data(ori, 8, len, 0, "Open SP CDC original");
    ...
    ili7807s_dump_data(buf, 10, core_mp.frame_len,  core_mp.xch_len, "Open SP CDC combined");
out:
    ili_kfree((void **)&ori);
    return ret;
}
...
static int allnode_peak_to_peak_cdc_data(int index)
{
    ...
    ili7807s_dump_data(cmd, 8, core_mp.cdc_len, 0, "Mutual CDC command");
    ...
    for (k = 0; k < rd_frame_num; k++) {
        ...
        /* Get original frame(cdc) data */
		if (!ilits7807s->eng_flow) {
			if (ilits7807s->wrapper(cmd, cmd_len, ori, len, ON, ON) < 0) {
				ILI_ERR("Failed to get cdc data\n");
				ret = -EMP_GET_CDC;
				goto out;
			}
			...
			msleep(200);
			if (ilits7807s->wrapper(cmd, cmd_len, ori, len, OFF, OFF) < 0) {
				ILI_ERR("Failed to get cdc data\n");
			}
        }

        ili7807s_dump_data(ori, 8, len, 0, "Mutual CDC original");
    ...
    if (tItems[index].bch_mrk_multi) {
        ili7807s_dump_data(frm_buf[0], 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined[0]/frame1");
        ili7807s_dump_data(frm_buf[1], 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined[1]/frame2");
        ili7807s_dump_data(frm_buf[2], 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined[2]/frame3");
    } else {
        ili7807s_dump_data(frm_buf[0], 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined[0]/frame1");
    }
...
static int allnode_mutual_cdc_data(int index)
{
    ...
    ili7807s_dump_data(frame_buf, 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined");
out:
    ili_kfree((void **)&ori);
    return ret;
}
...
static int pin_test(int index)
{
    ...
    if (tItems[index].test_int_pin == ENABLE) {
        ...
        cmd[1] = 0x1;
        ret = ilits7807s->wrapper(cmd, 2, NULL, 0, OFF, OFF);
    ...
    if (tItems[index].int_pulse_test == ENABLE) {
        ...
        ret = ilits7807s->wrapper(cmd, 3, NULL, 0, OFF, OFF);
...
static int open_test_sp(int index)
{
    ...
    for (i = 0; i < tItems[index].frame_count; i++) {
        ...
        ili7807s_dump_data(open[i].cbk_700, 10, core_mp.frame_len, core_mp.xch_len, "cbk 700");
        ili7807s_dump_data(open[i].cbk_250, 10, core_mp.frame_len, core_mp.xch_len, "cbk 250");
        ili7807s_dump_data(open[i].cbk_200, 10, core_mp.frame_len, core_mp.xch_len, "cbk 200");
        ...
        ili7807s_dump_data(open[i].charg_rate, 10, core_mp.frame_len, core_mp.xch_len, "origin charge rate");
        ili7807s_dump_data(open[i].full_Open, 10, core_mp.frame_len, core_mp.xch_len, "origin full open");
        addr = 0;
        ...
        ili7807s_dump_data(&tItems[index].buf[(i * core_mp.frame_len)], 10, core_mp.frame_len, core_mp.xch_len,
...
static int open_test_cap(int index)
{
    ...
    open_para_7807s.gain = ili7807s_katoi(str);
    parser_get_int_data(tItems[index].desp, "tvch", str, sizeof(str));
    open_para_7807s.tvch = ili7807s_katoi(str);
    parser_get_int_data(tItems[index].desp, "tvcl", str, sizeof(str));
    open_para_7807s.tvcl = ili7807s_katoi(str);
    parser_get_int_data(tItems[index].desp, "cbk_step", str, sizeof(str));
    open_para_7807s.cbk_step = ili7807s_katoi(str);
    parser_get_int_data(tItems[index].desp, "cint", str, sizeof(str));
    open_para_7807s.cint = ili7807s_katoi(str);

    /* 9881N, 9881O, 7807Q, 7807S not support cbk_step and cint read from mp.ini */
    if ((pid != 0x988117) && (pid != 0x988118) && (pid != 0x78071A) && (pid != 0x78071C)) {
        if ((open_para_7807s.cbk_step == 0) || (open_para_7807s.cint == 0)) {
    ...
    ILI_INFO("gain = %d, tvch = %d, tvcl = %d, cbk_step = %d, cint = %d\n", open_para_7807s.gain,
             open_para_7807s.tvch, open_para_7807s.tvcl, open_para_7807s.cbk_step, open_para_7807s.cint);
    ...
    for (i = 0; i < tItems[index].frame_count; i++) {
        ...
        ili7807s_dump_data(open[i].dcl_cap, 10, core_mp.frame_len, core_mp.xch_len, "DCL_Cap");
        addr = 0;
    ...
    if (core_mp.final_result == MP_DATA_FAIL) {
        ...
        if (lcm_on) {
            ...
            snprintf(csv_name, sizeof(csv_name), "%s/%s_lcm_on_%s_%s.csv", CSV_LCM_ON_NG_PATH, \
                     ilits7807s->ts->panel_data.manufacture_info.manufacture, get_date_time_str(), ret_fail_name);
#else
        ...
        if (lcm_on) {
            ...
            snprintf(csv_name, sizeof(csv_name), "%s/%s_lcm_on_%s_%s.csv", CSV_LCM_ON_OK_PATH, \
                     ilits7807s->ts->panel_data.manufacture_info.manufacture, get_date_time_str(), ret_pass_name);
#else
...
static void ilitek_tddi_mp_init_item(void)
{
    ...
    memset(&open_para_7807s, 0, sizeof(open_para_7807s));
    memset(&short_para_7807s, 0, sizeof(short_para_7807s));
    core_mp.chip_pid = ilits7807s->chip->pid;
    core_mp.chip_id = ilits7807s->chip->id;
    core_mp.chip_type = ilits7807s->chip->type;
    core_mp.chip_ver = ilits7807s->chip->ver;
    core_mp.fw_ver = ilits7807s->chip->fw_ver;
    core_mp.protocol_ver = ilits7807s->protocol->ver;
    core_mp.core_ver = ilits7807s->chip->core_ver;
    core_mp.cdc_len = ilits7807s->protocol->cdc_len;
    core_mp.no_bk_shift = ilits7807s->chip->no_bk_shift;
    core_mp.xch_len = ilits7807s->xch_num;
    core_mp.ych_len = ilits7807s->ych_num;
    ...
    core_mp.retry = ilits7807s->mp_retry;
...
static void mp_test_run(int index)
{
    ...
    if (ipio_strcmp(tItems[i].desp, "tx/rx delta") == 0) {
        ...
        tItems[i].max = ili7807s_katoi(str);
        parser_get_int_data(tItems[i].desp, "min", str, sizeof(str));
        tItems[i].min = ili7807s_katoi(str);
    }
    ...
    if (ipio_strcmp(tItems[i].desp, "peak to peak_td (lcm off)") == 0 &&
        ...
        core_mp.td_retry = ili7807s_katoi(str);
...
static int mp_sort_item(bool lcm_on)
{
    ...
    for (i = 0; i < MAX_SECTION_NUM; i++) {
        for (j = 0; j < MP_TEST_ITEM; j++) {
            ...
            tItems[j].run = ili7807s_katoi(str);
...
static void ilitek_tddi_mp_all_pass_check(void)
{
    ...
    ilits7807s->wait_int_timeout = 20;

    do {
        ret = ilits7807s->wrapper(cmd, 1, temp, 3, ON, OFF);
    ...
    if (temp[2] == ili7807s_calc_packet_checksum(temp, sizeof(temp) - 1) &&
...
int ili7807s_mp_test_main(char *apk, bool lcm_on, struct seq_file *s, char *message)
{
    int i, ret = 0;
    char str[128] = {0}, ver[128] = {0};

    if (ilits7807s->xch_num <= 0 || ilits7807s->ych_num <= 0) {
    ...
    ret = ilitek_tddi_mp_ini_parser(ilits7807s->test_limit_name);
    ...
    if ((ili7807s_str2hex(ver)) != (core_mp.protocol_ver >> 8)) {
        ILI_ERR("ERROR! MP Protocol version is invaild, 0x%x\n", ili7807s_str2hex(ver));
    ...
    if (ili7807s_ic_int_trigger_ctrl(true) < 0) {
...
out:
    ...
    if (ili7807s_ic_int_trigger_ctrl(false) < 0) {
