...
static int ilitek_tddi_ic_check_support(u32 pid, u16 id)
{
    ...
    ilits7807s->chip->pid = pid;
    ilits7807s->chip->reset_key = 0x00019878;
    ilits7807s->chip->wtd_key = 0x9881;

    if (((pid & 0xFFFFFF00) == ILI9881N_AA) || ((pid & 0xFFFFFF00) == ILI9881O_AA)) {
        ilits7807s->chip->dma_reset = ENABLE;
    } else {
        ilits7807s->chip->dma_reset = DISABLE;
    }

    ilits7807s->chip->no_bk_shift = RAWDATA_NO_BK_SHIFT;
    ilits7807s->chip->max_count = 0x1FFFF;
    return 0;
}

int ili7807s_ice_mode_bit_mask_write(u32 addr, u32 mask, u32 value)
{
    int ret = 0;
    u32 data = 0;

    if (ili7807s_ice_mode_read(addr, &data, sizeof(u32)) < 0) {
    ...
    ret = ili7807s_ice_mode_write(addr, data, sizeof(u32));
...
int ili7807s_ice_mode_write(u32 addr, u32 data, int len)
{
    int ret = 0, i;
    u8 txbuf[64] = {0};
    ...
    ret = ilits7807s->wrapper(txbuf, len + 4, NULL, 0, OFF, OFF);
...
int ili7807s_ice_mode_read(u32 addr, u32 *data, int len)
{
    int ret = 0;
    u8 rxbuf[4] = {0};
    u8 txbuf[4] = {0};
    ...
    txbuf[2] = (char)((addr & 0x0000FF00) >> 8);
    txbuf[3] = (char)((addr & 0x00FF0000) >> 16);
    ret = ilits7807s->wrapper(txbuf, sizeof(txbuf), NULL, 0, OFF, OFF);

    if (ret < 0) {
        goto out;
    }

    ret = ilits7807s->wrapper(NULL, 0, rxbuf, len, OFF, OFF);
...
int ili7807s_ice_mode_ctrl(bool enable, bool mcu)
{
    int ret = 0;
    ...
    if (enable) {
        ...
        if (ilits7807s->wrapper(cmd_open, sizeof(cmd_open), NULL, 0, OFF, OFF) < 0) {
            ILI_ERR("write ice mode cmd error\n");
            atomic_set(&ilits7807s->ice_stat, DISABLE);
        }

        ilits7807s->pll_clk_wakeup = false;
    } else {
        if (!atomic_read(&ilits7807s->ice_stat)) {
        ...
        ret = ilits7807s->wrapper(cmd_close, sizeof(cmd_close), NULL, 0, OFF, OFF);
...
int ili7807s_ic_func_ctrl(const char *name, int ctrl)
{
    int i = 0, ret;

    if (strcmp(name, func_ctrl[1].name) == 0) {
        ilits7807s->sleep_type = ctrl;
    }
    ...
    if (ilits7807s->protocol->ver == PROTOCOL_VER_500) {
        ILI_ERR("Non support function ctrl with protocol v5.0\n");
        ret = -1;
        goto out;
    }

    if (ilits7807s->protocol->ver >= PROTOCOL_VER_560) {
    ...
    ret = ilits7807s->wrapper(func_ctrl[i].cmd, func_ctrl[i].len, NULL, 0, OFF, OFF);
...
int ili7807s_ic_code_reset(bool mcu)
{
    int ret;
    bool ice = atomic_read(&ilits7807s->ice_stat);

    if (!ice)
        if (ili7807s_ice_mode_ctrl(ENABLE, mcu) < 0) {
            ILI_ERR("Enable ice mode failed before code reset\n");
        }

    ret = ili7807s_ice_mode_write(0x40040, 0xAE, 1);
    ...
    if (!ice)
        if (ili7807s_ice_mode_ctrl(DISABLE, mcu) < 0) {
...
int ili7807s_ic_whole_reset(bool mcu)
{
    int ret = 0;
    bool ice = atomic_read(&ilits7807s->ice_stat);
    ...
    ILI_INFO("ic whole reset key = 0x%x, edge_delay = %d\n",
             ilits7807s->chip->reset_key, ilits7807s->rst_edge_delay);
    ret = ili7807s_ice_mode_write(ilits7807s->chip->reset_addr, ilits7807s->chip->reset_key, sizeof(u32));
...
static void ilitek_tddi_ic_wr_pack(int packet)
{
    ...
    while (retry--) {
        if (ili7807s_ice_mode_read(0x73010, &reg_data, sizeof(u8)) < 0) {
    ...
    if (ili7807s_ice_mode_write(0x73000, packet, 4) < 0) {
...
static u32 ilitek_tddi_ic_rd_pack(int packet)
{
    ...
    while (retry--) {
        if (ili7807s_ice_mode_read(0x4800A, &reg_data, sizeof(u8)) < 0) {
    ...
    if (ili7807s_ice_mode_write(0x4800A, 0x02, 1) < 0) {
        ILI_ERR("Write 0x2 at 0x4800A\n");
    }

    if (ili7807s_ice_mode_read(0x73016, &reg_data, sizeof(u8)) < 0) {
...
void ili7807s_ic_set_ddi_reg_onepage(u8 page, u8 reg, u8 data, bool mcu)
{
...
void ili7807s_ic_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data, bool mcu)
{
    u32 setpage = 0x1FFFFF00 | page;
...
void ili7807s_ic_get_pc_counter(int stat)
{
    bool ice = atomic_read(&ilits7807s->ice_stat);
    u32 pc = 0, pc_addr = ilits7807s->chip->pc_counter_addr;
    u32 latch = 0, latch_addr = ilits7807s->chip->pc_latch_addr;
    int ret = 0;
    ILI_DBG("stat = %d\n", stat);
    ...
    if (ili7807s_ice_mode_read(ilits7807s->chip->pc_counter_addr, &pc, sizeof(u32)) < 0) {
        ILI_ERR("Read pc conter error\n");
    }

    if (ili7807s_ice_mode_read(ilits7807s->chip->pc_latch_addr, &latch, sizeof(u32)) < 0) {
        ILI_ERR("Read pc latch error\n");
    }

    ilits7807s->fw_pc = pc;
    ilits7807s->fw_latch = latch;
    ILI_ERR("Read counter (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x\n",
            pc_addr, ilits7807s->fw_pc, latch_addr, ilits7807s->fw_latch);
...
int ili7807s_ic_int_trigger_ctrl(bool pulse)
{
    /* It's supported by fw, and the level will be kept at high until data was already prepared. */
    if (ili7807s_ic_func_ctrl("int_trigger", pulse) < 0) {
        ILI_ERR("Write CMD error, set back to <%s> trigger\n", ilits7807s->int_pulse ? "Level" : "Pulse");
        return -1;
    }

    ilits7807s->int_pulse = pulse;
    ILI_INFO("INT Trigger = %s\n", ilits7807s->int_pulse ? "Level" : "Pulse");
    return 0;
}

int ili7807s_ic_check_int_level(bool level)
{
    int timer = 3000;
    int gpio = ilits7807s->tp_int;
...
int ili7807s_ic_check_int_pulse(bool pulse)
{
    if (!wait_event_interruptible_timeout(ilits7807s->inq, !atomic_read(&ilits7807s->cmd_int_check),
                                          msecs_to_jiffies(ilits7807s->wait_int_timeout))) {
        ILI_ERR("Error! INT pulse no detected. Timeout = %d ms\n", ilits7807s->wait_int_timeout);
...
int ili7807s_ic_check_busy(int count, int delay)
{
    u8 cmd[2] = {0};
    u8 busy = 0, rby = 0;
    ...
    if (ilits7807s->actual_tp_mode == P5_X_FW_AP_MODE) {
        rby = 0x41;
    } else if (ilits7807s->actual_tp_mode == P5_X_FW_TEST_MODE) {
        rby = 0x51;
    } else {
    ...
    do {
        ...
        if (ilits7807s->wrapper(cmd, sizeof(cmd), NULL, 0, OFF, OFF) < 0) {
            ILI_ERR("Write check busy cmd failed\n");
        }

        if (ilits7807s->wrapper(&cmd[1], sizeof(u8), &busy, sizeof(u8), ON, OFF) < 0) {
...
int ili7807s_ic_get_core_ver(void)
{
    int i = 0, ret = 0;
    u8 cmd[2] = {0}, buf[10] = {0};
    ilits7807s->protocol->core_ver_len = P5_X_CORE_VER_FOUR_LENGTH;

    if (ilits7807s->info_from_hex) {
        buf[1] = ilits7807s->fw_info[68];
        buf[2] = ilits7807s->fw_info[69];
        buf[3] = ilits7807s->fw_info[70];
        buf[4] = ilits7807s->fw_info[71];
        goto out;
    }
    ...
    do {
        if (i == 0) {
            ...
            ilits7807s->protocol->core_ver_len = P5_X_CORE_VER_THREE_LENGTH;
        }
        ...
        if (ilits7807s->wrapper(&cmd[1], sizeof(u8), buf, ilits7807s->protocol->core_ver_len, ON, OFF) < 0) {
...
out:
    ...
    ilits7807s->chip->core_ver = buf[1] << 24 | buf[2] << 16 | buf[3] << 8 | buf[4];
    return ret;
}

void ili7807s_fw_uart_ctrl(u8 ctrl)
{
    u8 cmd[4] = {0};
...
int ili7807s_ic_get_fw_ver(void)
{
    int ret = 0;
    u8 cmd[2] = {0};
    u8 buf[10] = {0};
    ...
    if (ilits7807s->info_from_hex) {
        buf[1] = ilits7807s->fw_info[48];
        buf[2] = ilits7807s->fw_info[49];
        buf[3] = ilits7807s->fw_info[50];
        buf[4] = ilits7807s->fw_info[51];
        buf[5] = ilits7807s->fw_mp_ver[0];
        buf[6] = ilits7807s->fw_mp_ver[1];
        buf[7] = ilits7807s->fw_mp_ver[2];
        buf[8] = ilits7807s->fw_mp_ver[3];
        goto out;
    }
    ...
    if (ilits7807s->wrapper(&cmd[1], sizeof(u8), buf, ilits7807s->protocol->fw_ver_len, ON, OFF) < 0) {
...
out:
    ...
    ilits7807s->chip->fw_ver = buf[1] << 24 | buf[2] << 16 | buf[3] << 8 | buf[4];
    ...
    if (ilits7807s->ts->panel_data.manufacture_info.version) {
        ...
        if (ilits7807s->ts->panel_data.vid_len == 0) {
            ver_len = strlen(ilits7807s->ts->panel_data.manufacture_info.version);

            //strlcpy(&(ilits7807s->ts->panel_data.manufacture_info.version[12]), dev_version, 3);
            if (ver_len <= 11) {
                //snprintf(ilits7807s->ts->panel_data.manufacture_info.version + 9, sizeof(dev_version),"%s", dev_version);
                strlcpy(&ilits7807s->ts->panel_data.manufacture_info.version[9], dev_version, 3);
                ILI_ERR("melo version %s\n", ilits7807s->ts->panel_data.manufacture_info.version);
            } else {
                strlcpy(&ilits7807s->ts->panel_data.manufacture_info.version[12], dev_version, 3);
                ILI_ERR("melo version1 %s\n", ilits7807s->ts->panel_data.manufacture_info.version);
            }
        } else {
            ver_len = ilits7807s->ts->panel_data.vid_len;

            if (ver_len > MAX_DEVICE_VERSION_LENGTH - 4) {
                ver_len = MAX_DEVICE_VERSION_LENGTH - 4;
            }

            strlcpy(&ilits7807s->ts->panel_data.manufacture_info.version[ver_len],
...
int ili7807s_ic_get_panel_info(void)
{
    if ((ilits7807s->ts->resolution_info.max_x != 0 && ilits7807s->ts->resolution_info.max_y != 0)) {
        ILI_DBG("use kit default resolution\n");
        ilits7807s->panel_wid = ilits7807s->ts->resolution_info.max_x;
        ilits7807s->panel_hei = ilits7807s->ts->resolution_info.max_y;
        ilits7807s->trans_xy = (ilits7807s->chip->core_ver >= CORE_VER_1430
                           && (ilits7807s->rib.nReportByPixel > 0)) ? ON : OFF;
    } else {
        ILI_DBG("Invalid panel info, use default resolution\n");
        ilits7807s->panel_wid = TOUCH_SCREEN_X_MAX;
        ilits7807s->panel_hei = TOUCH_SCREEN_Y_MAX;
        ilits7807s->trans_xy = OFF;
    }

    ILI_DBG("Panel info: width = %d, height = %d\n", ilits7807s->panel_wid, ilits7807s->panel_hei);
    ILI_DBG("Transfer touch coordinate = %s\n", ilits7807s->trans_xy ? "ON" : "OFF");
    return 0;
}

int ili7807s_ic_get_tp_info(void)
{
    int ret = 0;
    u8 cmd[2] = {0};
    u8 buf[20] = {0};
    ...
    if (ilits7807s->info_from_hex  && (ilits7807s->chip->core_ver >= CORE_VER_1410)) {
        buf[1] = ilits7807s->fw_info[5];
        buf[2] = ilits7807s->fw_info[7];
        buf[3] = ilits7807s->fw_info[8];
        buf[4] = ilits7807s->fw_info[9];
        buf[5] = ilits7807s->fw_info[10];
        buf[6] = ilits7807s->fw_info[11];
        buf[7] = ilits7807s->fw_info[12];
        buf[8] = ilits7807s->fw_info[14];
        buf[11] = buf[7];
        buf[12] = buf[8];
        goto out;
    }
    ...
    ret = ilits7807s->wrapper(&cmd[1], sizeof(u8), buf, ilits7807s->protocol->tp_info_len, ON, OFF);
...
out:
    ilits7807s->min_x = buf[1];
    ilits7807s->min_y = buf[2];
    ilits7807s->max_x = buf[4] << 8 | buf[3];
    ilits7807s->max_y = buf[6] << 8 | buf[5];
    ilits7807s->xch_num = buf[7];
    ilits7807s->ych_num = buf[8];
    ilits7807s->stx = buf[11];
    ilits7807s->srx = buf[12];
    ILI_DBG("TP Info: min_x = %d, min_y = %d, max_x = %d, max_y = %d\n", ilits7807s->min_x, ilits7807s->min_y,
             ilits7807s->max_x, ilits7807s->max_y);
    ILI_DBG("TP Info: xch = %d, ych = %d, stx = %d, srx = %d\n", ilits7807s->xch_num, ilits7807s->ych_num,
             ilits7807s->stx, ilits7807s->srx);
    return ret;
}
...
static void ilitek_tddi_ic_check_protocol_ver(u32 pver)
{
    ...
    if (ilits7807s->protocol->ver == pver) {
    ...
    for (i = 0; i < PROTOCL_VER_NUM - 1; i++) {
        if (protocol_info[i].ver == pver) {
            ilits7807s->protocol = &protocol_info[i];
            ILI_INFO("update protocol version = %x\n", ilits7807s->protocol->ver);
            return;
        }
    }

    ILI_ERR("Not found a correct protocol version in list, use newest version\n");
    ilits7807s->protocol = &protocol_info[PROTOCL_VER_NUM - 1];
}

int ili7807s_ic_get_protocl_ver(void)
{
    int ret = 0;
    u8 cmd[2] = {0};
    u8 buf[10] = {0};
    ...
    if (ilits7807s->info_from_hex) {
        buf[1] = ilits7807s->fw_info[72];
        buf[2] = ilits7807s->fw_info[73];
        buf[3] = ilits7807s->fw_info[74];
        goto out;
    }
    ...
    if (ilits7807s->wrapper(&cmd[1], sizeof(u8), buf, ilits7807s->protocol->pro_ver_len, ON, OFF) < 0) {
...
out:
    ...
    ILI_DBG("Protocol version = %d.%d.%d\n", ilits7807s->protocol->ver >> 16,
             (ilits7807s->protocol->ver >> 8) & 0xFF, ilits7807s->protocol->ver & 0xFF);
    return ret;
}
int ili7807s_ic_get_info(void)
{
    int ret = 0;
    ...
    if (ili7807s_ice_mode_read(ilits7807s->chip->pid_addr, &ilits7807s->chip->pid, sizeof(u32)) < 0) {
        ILI_ERR("Read chip pid error\n");
    }

    if (ili7807s_ice_mode_read(ilits7807s->chip->otp_addr, &ilits7807s->chip->otp_id, sizeof(u32)) < 0) {
        ILI_ERR("Read otp id error\n");
    }

    if (ili7807s_ice_mode_read(ilits7807s->chip->ana_addr, &ilits7807s->chip->ana_id, sizeof(u32)) < 0) {
        ILI_ERR("Read ana id error\n");
    }

    //ilits7807s->chip->pid = ilits7807s->chip->pid;
    ilits7807s->chip->id = ilits7807s->chip->pid >> 16;
    ilits7807s->chip->type = (ilits7807s->chip->pid & 0x0000FF00) >> 8;
    ilits7807s->chip->ver = ilits7807s->chip->pid & 0xFF;
    ilits7807s->chip->otp_id &= 0xFF;
    ilits7807s->chip->ana_id &= 0xFF;
    ILI_INFO("CHIP: PID = %x\n", (ilits7807s->chip->pid >> 8));
    ret = ilitek_tddi_ic_check_support(ilits7807s->chip->pid, ilits7807s->chip->id);
    return ret;
}

int ili7807s_ic_dummy_check(void)
{
    int ret = 0;
	...
	for (i = 0; i < 3; i++) {
	    if (ili7807s_ice_mode_write(WDT9_DUMMY2, wdata, sizeof(u32)) < 0) {
	        ILI_ERR("Write dummy error\n");
	    }

	    if (ili7807s_ice_mode_read(WDT9_DUMMY2, &rdata, sizeof(u32)) < 0) {
	        ILI_ERR("Read dummy error\n");
	    }
		if (rdata == wdata || rdata == (u32)-wdata){
			if (rdata == (u32)-wdata) {
				ilits7807s->eng_flow = true;
			} else {
				ilits7807s->eng_flow = false;
			}
			break;
		}
		mdelay(30);
	...
	ILI_INFO("Ilitek IC check successe ilits7807s->eng_flow = %d\n", ilits7807s->eng_flow);
    return ret;
}

static struct ilitek_ic_info chip;

void ili7807s_ic_init(void)
{
