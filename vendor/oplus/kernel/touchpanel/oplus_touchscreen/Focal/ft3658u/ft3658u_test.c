...
#define FTS_TEST_SAVE_INFO(fmt, args...) do { \
	if (g_ft3658u_data->s) { \
		seq_printf(g_ft3658u_data->s, fmt, ##args); \
	} \
	TPD_INFO(fmt, ##args); \
} while (0)
...
void ft3658u_print_buffer(int *buffer, int length, int line_num)
{
	int i = 0;
	int j = 0;
	int tmpline = 0;
 ...
 *******************************************************************/
static int ft3658u_test_bus_read(u8 *cmd, int cmdlen, u8 *data, int datalen)
{
	int ret = 0;

	ret = ft3658u_read(cmd, cmdlen, data, datalen);
	if (ret < 0) {
		return ret;
	} else {
		return 0;
...
static int ft3658u_test_bus_write(u8 *writebuf, int writelen)
{
	int ret = 0;

	ret = ft3658u_write(writebuf, writelen);
	if (ret < 0) {
		return ret;
	} else {
		return 0;
...
static int ft3658u_test_read_reg(u8 addr, u8 *val)
{
	return ft3658u_test_bus_read(&addr, 1, val, 1);
}

static int ft3658u_test_write_reg(u8 addr, u8 val)
{
	int ret;
	u8 cmd[2] = {0};
	...
	cmd[0] = addr;
	cmd[1] = val;
	ret = ft3658u_test_bus_write(cmd, 2);

	return ret;
}

static int ft3658u_test_read(u8 addr, u8 *readbuf, int readlen)
{
	int ret = 0;
	int i = 0;
	int packet_length = 0;
	...
	ret = ft3658u_test_bus_read(&addr, 1, &readbuf[offset], packet_length);
...
static int wait_state_update(u8 retval)
{
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		state = 0xFF;
		ret = ft3658u_test_read_reg(addr, &state);
...
static int read_mass_data(u8 addr, int byte_num, int *buf)
{
	...
	ret = ft3658u_test_read(addr, data, byte_num);
...
static void ft3658u_test_save_data(char *name, int *data, int datacnt, int line, int fd)
{
	char *data_buf = NULL;
	u32 cnt = 0;
...
static int enter_work_mode(void)
{
	...
	TPD_INFO("%s +\n", __func__);
	ret = ft3658u_test_read_reg(DEVIDE_MODE_ADDR, &mode);
	...
	for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
		ret = ft3658u_test_write_reg(DEVIDE_MODE_ADDR, 0x00);
...
static int ft3658u_special_operation_for_samsung(struct chip_data_ft3658u *ts_data)
{
	int ret = 0;

	if (true ==
		ts_data->use_panelfactory_limit) {                      /*only for firmware released to samsung factory*/
		ret = ft3658u_test_write_reg(FT3658U_REG_SAMSUNG_SPECIFAL, 0x01);

		if (ret < 0) {
			FTS_TEST_SAVE_ERR("write FT3658U_REG_SAMSUNG_SPECIFAL fail at %s,ret=%d\n", __func__, ret);
			return -EIO;
		}
	}
...
#define FT3658U_FACTORY_MODE 0x40
static int enter_factory_mode(struct chip_data_ft3658u *ts_data)
{
	...
	if ((ret >= 0) && (FT3658U_FACTORY_MODE == mode)) {
		ft3658u_special_operation_for_samsung(ts_data);
		return 0;
	}

	for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
		ret = ft3658u_test_write_reg(DEVIDE_MODE_ADDR, 0x40);
...
static int get_channel_num(struct chip_data_ft3658u *ts_data)
{
	...
	ret = ft3658u_test_read_reg(FACTORY_REG_CHX_NUM, &tx_num);
	...
	ret = ft3658u_test_read_reg(FACTORY_REG_CHY_NUM, &rx_num);
			...
			int *data)
{
	...
	ret = ft3658u_test_write_reg(off_addr, off_val);
...
static int start_scan(void)
{
	...
	ret = ft3658u_test_write_reg(addr, val);
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		ret = ft3658u_test_read_reg(addr, &val);
 ...
 */
int ft3658u_start_scan(int frame_num)
{
	int ret = 0;
	u8 addr = 0;
	u8 val = 0;
...
static int get_cb_ft5652(int *cb_buf, int byte_num, bool is_cf)
{
	int ret = 0;
	int i = 0;
	u8 cb[SC_NUM_MAX] = { 0 };
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_MC_SC_CB_H_ADDR_OFF, 0);
	if (ret < 0) {
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_MC_SC_CB_ADDR_OFF, 0);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("write cb addr offset fail\n");
		return ret;
	}

	ret = ft3658u_test_read(FACTORY_REG_MC_SC_CB_ADDR, cb, byte_num);
	if (ret < 0) {
...
static int short_get_adc_data_mc(u8 retval, int byte_num, int *adc_buf, u8 mode)
{
	...
	ret = ft3658u_test_write_reg(FACTROY_REG_SHORT2_TEST_EN, mode);
	...
	for (i = 0; i < FACTORY_TEST_RETRY; i++) {
		...
		ret = ft3658u_test_read_reg(FACTROY_REG_SHORT2_TEST_STATE, &short_state);
	...
	/*    TPD_DEBUG("adc data:\n");*/
	/*    ft3658u_print_buffer(adc_buf, byte_num / 2, 0);*/
test_err:
	FTS_TEST_FUNC_EXIT();
	return ret;
}
...
static int short_test_ch_to_all(struct chip_data_ft3658u *ts_data,
				int *res, u8 *ab_ch, int offset, bool *result)
{
	int ret = 0;
	int i = 0;
	int tx_num = ts_data->hw_res->TX_NUM;
	...
	ret = ft3658u_test_write_reg(FACTROY_REG_SHORT2_RES_LEVEL, 1);
	if (ret < 0) {
	...
	*result = true;
	...
	if (ab_ch_num) {
		ft3658u_print_buffer(res, ch_num, ch_num);
		ab_ch[0] = ab_ch_num;
...
static void ft3658u_show_null_noise(int *null_noise, int rx_num)
{
	int i = 0;
	int cnt = 0;
	char tmpbuf[512] = { 0 };
...
static void ft3658u_get_null_noise(struct chip_data_ft3658u *ts_data)
{
	...
	if (ret < 0) {
		...
		ft3658u_show_null_noise(&null_noise[1], ts_data->hw_res->RX_NUM);
	}
...
static int ft3658u_auto_preoperation(struct chip_data_ft3658u *ts_data)
{
...
static int ft3658u_noise_autotest(struct chip_data_ft3658u *ts_data, bool *test_result)
{
	int ret = 0;
	int i = 0;
	u8 fre = 0;
	...
	if (!ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P || !ts_data->ft3658u_autotest_offset->ft3658u_noise_data_N) {
		TPD_INFO("ft3658u_noise_data_P || ft3658u_noise_data_N is NULL");
		return 0;
	}
	...
	ret = ft3658u_test_read_reg(FACTORY_REG_TOUCH_THR, &reg0d_val);
	...
	/* save origin value */
	ret = ft3658u_test_read_reg(FACTORY_REG_DATA_SELECT, &reg06_val);
	...
	ret = ft3658u_test_read_reg(FACTORY_REG_FRE_LIST, &fre);
	...
	ret = ft3658u_test_read_reg(0x1A, &reg1a_val);
	...
	ret = ft3658u_test_read_reg(0x1B, &reg1b_val);
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_DATA_SELECT, 0x01);
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_FRE_LIST, 0x00);
	...
	ret = ft3658u_test_write_reg(0x1A, 0x01);
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_FRAME_NUM, (frame_num >> 8) & 0xFF);
	ret = ft3658u_test_write_reg(FACTORY_REG_FRAME_NUM + 1, frame_num & 0xFF);
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_MAX_DIFF, 0x01);
	...
	if (ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P && ts_data->ft3658u_autotest_offset->ft3658u_noise_data_N) {
		for (i = 0; i < node_num; i++) {
			if (ts_data->noise_rawdata[i] > ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P[i]) {
				TPD_INFO("noise data ERR [%d]: [%d] > [%d] > [%d] \n", i, ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P[i],
					ts_data->noise_rawdata[i], ts_data->ft3658u_autotest_offset->ft3658u_noise_data_N[i]);
				FTS_TEST_SAVE_ERR("test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
						i / rx_num + 1, i % rx_num + 1, ts_data->noise_rawdata[i], ts_data->ft3658u_autotest_offset->ft3658u_noise_data_N[i],
						ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P[i]);
				result = false;
			}
		}
	} else {
		TPD_INFO("ft3658u_raw_data_P || ft3658u_raw_data_N is null \n");
		result = false;
	}

	ft3658u_get_null_noise(ts_data);
	/*ft3658u_test_save_data("Noise Test", rawdata, node_num, rx_num, ts_data->csv_fd);*/

restore_reg:
	ret = ft3658u_test_write_reg(0x1B, reg1b_val);
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_DATA_SELECT, reg06_val);
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_FRE_LIST, fre);
	...
	ret = ft3658u_test_write_reg(0x1A, reg1a_val);
...
static int ft3658u_rawdata_autotest(struct chip_data_ft3658u *ts_data, bool *test_result)
{
	int ret = 0;
	int i = 0;
	u8 fre = 0;
	...
	if (!ts_data->ft3658u_autotest_offset->ft3658u_raw_data_P || !ts_data->ft3658u_autotest_offset->ft3658u_raw_data_N) {
		TPD_INFO("ft3658u_raw_data_P || ft3658u_raw_data_N is NULL");
		return 0;
	}
	...
	ret = ft3658u_test_read_reg(0x5B, &reg5b_val);
	...
	/* set frequecy high */
	ret = ft3658u_test_write_reg(FACTORY_REG_FRE_LIST, 0x81);
	...
	ret = ft3658u_test_write_reg(0x5B, 1);
	...
	ret = ft3658u_test_write_reg(0x06, 0);
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((ts_data->rawdata[i] < ts_data->ft3658u_autotest_offset->ft3658u_raw_data_N[i]) ||
			(ts_data->rawdata[i] > ts_data->ft3658u_autotest_offset->ft3658u_raw_data_P[i])) {
			TPD_INFO("raw data ERR [%d]: [%d] > [%d] > [%d] \n", i, ts_data->ft3658u_autotest_offset->ft3658u_raw_data_P[i],
				ts_data->rawdata[i], ts_data->ft3658u_autotest_offset->ft3658u_raw_data_N[i]);
			FTS_TEST_SAVE_ERR("test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
						i / rx_num + 1, i % rx_num + 1, ts_data->rawdata[i],
							ts_data->ft3658u_autotest_offset->ft3658u_raw_data_N[i], ts_data->ft3658u_autotest_offset->ft3658u_raw_data_P[i]);
			result = false;
		}
	}
...
restore_reg:
	/* set the origin value */
	ret = ft3658u_test_write_reg(0x5B, reg5b_val);
...
static int ft3658u_uniformity_autotest(struct chip_data_ft3658u *ts_data, bool *test_result)
{
	int ret = 0;
	int row = 0;
	int col = 1;
	...
	if (!ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P || !ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N) {
		TPD_INFO("ft3658u_uniformity_data_P || ft3658u_uniformity_data_N is NULL");
		return 0;
	}
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((rl_tmp[i] < ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N[i]) ||
			(rl_tmp[i] > ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P[i])) {
			TPD_INFO("uniformity data ERR [%d]: [%d] > [%d] > [%d] \n", i, ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P[i],
				rl_tmp[i], ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N[i]);
			FTS_TEST_SAVE_ERR("test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
					  i / rx_num + 1, i % rx_num + 1, rl_tmp[i],
					  ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N[i], ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P[i]);
			result = false;
		}
	}
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((rl_tmp[i] < ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N[i]) ||
			(rl_tmp[i] > ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P[i])) {
			TPD_INFO("uniformity data ERR [%d]: [%d] > [%d] > [%d] \n", i, ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P[i], rl_tmp[i],
			ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N[i]);
			FTS_TEST_SAVE_ERR("test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
					  i / rx_num + 1, i % rx_num + 1, rl_tmp[i],
					  ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N[i], ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P[i]);
			result2 = false;
...
static int ft3658u_scap_cb_autotest(struct chip_data_ft3658u *ts_data, bool *test_result)
{
	int ret = 0;
	...
	if (!ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_P || !ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_N ||
		!ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_N || !ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_P) {
		TPD_INFO("ft3658u_scap_cb_data_P || ft3658u_scap_cb_data_N || ft3658u_scap_cb_data_waterproof_N || ft3658u_scap_cb_data_waterproof_P is NULL");
		return 0;
	}
	...
	ret = ft3658u_test_read_reg(FACTORY_REG_WC_SEL, &wc_sel);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("read water_channel_sel fail,ret=%d\n", ret);
		goto test_err;
	}

	ret = ft3658u_test_read_reg(FACTORY_REG_MC_SC_MODE, &sc_mode);
	if (ret < 0) {
	...
	if (fw_wp_check) {
		...
		ret = ft3658u_test_write_reg(FACTORY_REG_MC_SC_MODE, WATER_PROOF_ON);
		...
		ret = get_cb_ft5652(scb_tmp, byte_num, false);
		if (ret < 0) {
		...
		tmp_result = compare_mc_sc(ts_data, tx_check, rx_check, scb_tmp,
					   ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_N,
					   ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_P);
	...
	if (fw_wp_check) {
		scb_tmp = ts_data->scap_cb + ts_data->scb_cnt;
		/* 1:waterproof 0:non-waterproof */
		ret = ft3658u_test_write_reg(FACTORY_REG_MC_SC_MODE, WATER_PROOF_OFF);
		if (ret < 0) {
			FTS_TEST_SAVE_ERR("set mc_sc mode fail\n");
			goto restore_reg;
		}
		...
		tmp2_result = compare_mc_sc(ts_data, tx_check, rx_check, scb_tmp,
					    ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_N,
					    ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_P);
...
restore_reg:
	ret = ft3658u_test_write_reg(FACTORY_REG_MC_SC_MODE, sc_mode);
...
static int ft3658u_scap_rawdata_autotest(struct chip_data_ft3658u *ts_data, bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
	bool tmp2_result = false;
	...
	if (!ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_P || !ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_N ||
		!ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_N || !ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_P) {
		TPD_INFO("ft3658u_scap_raw_data_P || ft3658u_scap_raw_data_N || ft3658u_scap_raw_waterproof_data_N || ft3658u_scap_raw_waterproof_data_P is NULL");
		return 0;
	}
	...
	ret = ft3658u_test_read_reg(FACTORY_REG_HC_SEL, &hc_sel);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("read high_channel_sel fail,ret=%d\n", ret);
		goto test_err;
	}

	ret = ft3658u_test_read_reg(0x5B, &data_type);
	...
	ret = ft3658u_test_write_reg(0x5B, 0x01);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("set raw type fail,ret=%d\n", ret);
		goto restore_reg;
	}
	...
	if (fw_wp_check) {
		...
		tmp_result = compare_mc_sc(ts_data, tx_check, rx_check, srawdata_tmp,
					   ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_N,
					   ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_P);
	...
	if (fw_wp_check) {
		...
		tmp2_result = compare_mc_sc(ts_data, tx_check, rx_check, srawdata_tmp,
					    ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_N,
					    ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_P);
		ts_data->srawdata_cnt += channel_num;
...
restore_reg:
	ret = ft3658u_test_write_reg(FACTORY_REG_WC_SEL, wc_sel);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("restore water_channel_sel fail,ret=%d\n", ret);
	}

	ret = ft3658u_test_write_reg(FACTORY_REG_HC_SEL, hc_sel);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("restore high_channel_sel fail,ret=%d\n", ret);
	}

	ret = ft3658u_test_write_reg(FACTORY_REG_DATA_TYPE, data_type);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("set raw type fail,ret=%d\n", ret);
	}
...
static int ft3658u_short_test(struct chip_data_ft3658u *ts_data, bool *test_result)
{
	int ret = 0;
	int adc[256] = { 0 };
	u8 ab_ch[256] = { 0 };
...
static int ft3658u_panel_differ_test(struct chip_data_ft3658u *ts_data, bool *test_result)
{
	int ret = 0;
	int i = 0;
	u8 fre = 0;
	...
	if (!ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_P
	    || !ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_N) {
		TPD_INFO("ft3658u_panel_differ_data_P || ft3658u_panel_differ_data_N is NULL");
		return 0;
	}
	...
	/* save origin value */
	ret = ft3658u_test_read_reg(FACTORY_REG_NORMALIZE, &normalize);
	if (ret < 0) {
	...
	ret = ft3658u_test_read_reg(FACTORY_REG_DATA_TYPE, &data_type);
	if (ret) {
		FTS_TEST_SAVE_ERR("read 0x5B fail,ret=%d\n", ret);
		goto test_err;
	}

	ret = ft3658u_test_read_reg(FACTORY_REG_FIR, &fir);
	if (ret) {
		FTS_TEST_SAVE_ERR("read 0xFB fail,ret=%d\n", ret);
		goto test_err;
	}

	ret = ft3658u_test_read_reg(FACTORY_REG_DATA_SELECT, &data_sel);
	if (ret) {
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_NORMALIZE, 0x00);
	if (ret < 0) {
	...
	/* fir disable */
	ret = ft3658u_test_write_reg(FACTORY_REG_FIR, 0);
	if (ret < 0) {
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_DATA_TYPE, 0x01);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("set raw type fail,ret=%d\n", ret);
		goto restore_reg;
	}

	ret = ft3658u_test_write_reg(FACTORY_REG_DATA_SELECT, 0x00);
	if (ret < 0) {
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((ts_data->panel_differ[i] < ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_N[i])
			|| (ts_data->panel_differ[i] > ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_P[i])) {
			TPD_INFO("raw data ERR [%d]: [%d] > [%d] > [%d] \n", i,
					ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_P[i], ts_data->panel_differ[i],
					ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_N[i]);
			FTS_TEST_SAVE_ERR("test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
							i / rx_num + 1, i % rx_num + 1, ts_data->panel_differ[i],
							ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_N[i],
							ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_P[i]);
			result = false;
		}
	}
...
restore_reg:
	...
	/* set the origin value */
	ret = ft3658u_test_write_reg(FACTORY_REG_NORMALIZE, normalize);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("restore normalize fail,ret=%d\n", ret);
	}
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_FIR, fir);
	if (ret < 0) {
	...
	ret = ft3658u_test_write_reg(FACTORY_REG_DATA_SELECT, data_sel);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("set data sel fail,ret=%d\n", ret);
	}
	ret = wait_state_update(TEST_RETVAL_AA);
...
static void ft3658u_auto_write_result(struct chip_data_ft3658u *ts_data, int failed_count)
{
	...
	ft3658u_test_save_data("Noise Test", ts_data->noise_rawdata, node_num, rx_num, ts_data->csv_fd);
	ft3658u_test_save_data("Null Noise Test", &ts_data->null_noise_value, 1, 1, ts_data->csv_fd);
	ft3658u_test_save_data("Rawdata Test", ts_data->rawdata, node_num, rx_num, ts_data->csv_fd);
	ft3658u_test_save_data("Rawdata Uniformity Test", ts_data->rawdata_linearity, ts_data->rl_cnt, rx_num, ts_data->csv_fd);
	ft3658u_test_save_data("SCAP CB Test", ts_data->scap_cb, channel_num, rx_num, ts_data->csv_fd);
	ft3658u_test_save_data("SCAP CB Test", ts_data->scap_cb + channel_num, channel_num, rx_num, ts_data->csv_fd);
	ft3658u_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata, channel_num, rx_num, ts_data->csv_fd);
	ft3658u_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata + channel_num, channel_num, rx_num, ts_data->csv_fd);
	ft3658u_test_save_data("Panel Differ Test", ts_data->panel_differ, node_num, rx_num, ts_data->csv_fd);
...
static int ft3658u_auto_endoperation(struct chip_data_ft3658u *ts_data)
{
	TPD_INFO("%s +\n", __func__);
	if (ts_data->rawdata_linearity) {
		kfree(ts_data->rawdata_linearity);
...
static int ft3658u_start_test(struct chip_data_ft3658u *ts_data)
{
	int ret = 0;
	...
	FTS_TEST_FUNC_ENTER();
	TPD_INFO("%s +\n", __func__);
	ft3658u_auto_preoperation(ts_data);

	/*noise test*/
	ret = ft3658u_noise_autotest(ts_data, &temp_result);
	...
	/* rawdata test */
	ret = ft3658u_rawdata_autotest(ts_data, &temp_result);
	if ((ret < 0) || (false == temp_result)) {
		test_result = false;
		test_result |= (1 << 2);
		failed_count += 1;
	...
	/* uniformity test */
	ret = ft3658u_uniformity_autotest(ts_data, &temp_result);
	if ((ret < 0) || (false == temp_result)) {
		test_result = false;
		test_result |= (1 << 3);
		failed_count += 1;
	...
	/* scap_cb test */
	ret = ft3658u_scap_cb_autotest(ts_data, &temp_result);
	if ((ret < 0) || (false == temp_result)) {
		test_result = false;
		test_result |= (1 << 4);
		failed_count += 1;
	...
	/* scap_rawdata test */
	ret = ft3658u_scap_rawdata_autotest(ts_data, &temp_result);
	if ((ret < 0) || (false == temp_result)) {
		test_result = false;
		test_result |= (1 << 5);
		failed_count += 1;
	...
	/* short test */
	ret = ft3658u_short_test(ts_data, &temp_result);
	if ((ret < 0) || (false == temp_result)) {
		test_result = false;
		test_result |= (1 << 6);
		failed_count += 1;
	...
	/* panel differ test */
	ret = ft3658u_panel_differ_test(ts_data, &temp_result);
	if ((ret < 0) || (false == temp_result)) {
		test_result = false;
		test_result |= (1 << 7);
		failed_count += 1;
	...
	ft3658u_auto_write_result(ts_data, failed_count);
	ft3658u_auto_endoperation(ts_data);
...
static void ft3658u_threshold_free(struct chip_data_ft3658u *ts_data)
{
	struct mc_sc_threshold *thr = &ts_data->mpt.thr;
...
static void ft3658u_autotest_endoperation(struct chip_data_ft3658u *ts_data, const struct firmware *limit_fw)
{
	TPD_INFO("%s +\n", __func__);
	if (ts_data->ft3658u_autotest_offset) {
		kfree(ts_data->ft3658u_autotest_offset);
		ts_data->ft3658u_autotest_offset = NULL;
	}
...
static int ft3658u_threshold_malloc(struct chip_data_ft3658u *ts_data)
{
	int tx_num = ts_data->hw_res->TX_NUM;
	int rx_num = ts_data->hw_res->RX_NUM;
	int node_num = tx_num * rx_num;
...
thr_free:
	ft3658u_threshold_free(ts_data);
	return -ENOMEM;
}

static int ft3658u_get_threshold(struct chip_data_ft3658u *ts_data, char *data)
{
	int i = 0;
	int ret = 0;
	int tx_num = ts_data->hw_res->TX_NUM;
	...
	ret = ft3658u_threshold_malloc(ts_data);
	if (ret < 0) {
...
static int ft3658u_get_threshold_from_img(struct chip_data_ft3658u *ts_data, char *data, const struct firmware *limit_fw)
{
	int ret = 0;
	int i = 0;
	int item_cnt = 0;
	...
	ret = ft3658u_read_reg(FT3658U_REG_FW_VER, &fwver);
	if (fwver >= 0x20) {
	...
	ts_data->ft3658u_autotest_offset = kzalloc(sizeof(struct ft3658u_autotest_offset), GFP_KERNEL);
...
*/
	...
	TPD_INFO("%s: populating ft3658u_test_offset \n", __func__);
		...
		if (item_head->item_limit_type == LIMIT_TYPE_NO_DATA) {
			...
			if (false == ts_data->use_panelfactory_limit) {
				...
				if (item_head->item_bit == TYPE_NOISE_DATA) {
					ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P = (int32_t *)(
								limit_fw->data + item_head->top_limit_offset);
					ts_data->ft3658u_autotest_offset->ft3658u_noise_data_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("item_head->top_limit_offset = %d, ft3658u_noise_data_N = %p \n",
						 item_head->top_limit_offset,
						 ts_data->ft3658u_autotest_offset->ft3658u_noise_data_N);

				} else if (item_head->item_bit == TYPE_RAW_DATA) {
					ts_data->ft3658u_autotest_offset->ft3658u_raw_data_P = (int32_t *)(
								limit_fw->data + item_head->top_limit_offset);
					ts_data->ft3658u_autotest_offset->ft3658u_raw_data_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("ft3658u_raw_data_P = %p, ft3658u_raw_data_N = %p \n",
						 ts_data->ft3658u_autotest_offset->ft3658u_raw_data_P,
						 ts_data->ft3658u_autotest_offset->ft3658u_raw_data_N);

				} else if (item_head->item_bit == TYPE_UNIFORMITY_DATA) {
					ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P = (int32_t *)(
								limit_fw->data + item_head->top_limit_offset);
					ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("ft3658u_uniformity_data_P = %p, ft3658u_uniformity_data_N = %p \n",
						 ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P,
						 ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N);

				} else if (item_head->item_bit == TYPE_PANEL_DIFFER_DATA) {
					ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_P = (int32_t *)(
								limit_fw->data + item_head->top_limit_offset);
					ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("ft3658u_panel_differ_data_P = %p, ft3658u_panel_differ_data_N = %p \n",
						 ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_P,
						 ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_N);
				}
				...
				if (item_head->item_bit == TYPE_FACTORY_NOISE_DATA) {
					...
					ts_data->ft3658u_autotest_offset->ft3658u_noise_data_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("ft3658u_noise_data_P = %p, ft3658u_noise_data_N = %p \n",
						 ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P,
						 ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P);
			...
			if (false == ts_data->use_panelfactory_limit) {
				...
				if (item_head->item_bit == TYPE_SCAP_CB_DATA) {
					ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_P = (int32_t *)(
								limit_fw->data + item_head->top_limit_offset);
					ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("ft3658u_scap_cb_data_P = %p, ft3658u_scap_cb_data_P = %p \n",
						 ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_P,
						 ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_N);

				} else if (item_head->item_bit == TYPE_SCAP_RAW_DATA) {
					ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_P = (int32_t *)(
								limit_fw->data + item_head->top_limit_offset);
					ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("ft3658u_scap_raw_data_P = %p, ft3658u_scap_raw_data_N = %p \n",
						 ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_P,
						 ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_N);

				} else if (item_head->item_bit == TYPE_SCAP_CB_WATERPROOF_DATA) {
					ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_P = (int32_t *)(
								limit_fw->data + item_head->top_limit_offset);
					ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("ft3658u_scap_cb_data_waterproof_P = %p, ft3658u_scap_cb_data_waterproof_N = %p \n",
						 ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_P,
						 ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_N);

				} else if (item_head->item_bit == TYPE_SCAP_RAW_WATERPROOF_DATA) {
					ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_P = (int32_t *)(
								limit_fw->data + item_head->top_limit_offset);
					ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_N = (int32_t *)(
								limit_fw->data + item_head->floor_limit_offset);
					TPD_INFO("ft3658u_scap_raw_waterproof_data_P = %p, ft3658u_scap_raw_waterproof_data_N = %p \n",
						 ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_P,
						 ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_N);
				}
...
static void ft3658u_print_threshold(struct chip_data_ft3658u *ts_data)
{
	int tx_num = ts_data->hw_res->TX_NUM;
	int rx_num = ts_data->hw_res->RX_NUM;
	int node_num = tx_num * rx_num;
	...
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_noise_data_P, node_num, rx_num);
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_noise_data_N, node_num, rx_num);

	TPD_INFO("rawdata threshold max/min:");
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_raw_data_P, node_num, rx_num);
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_raw_data_N, node_num, rx_num);

	TPD_INFO("uniformity threshold max/min:");
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_P, node_num, rx_num);
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_uniformity_data_N, node_num, rx_num);

	TPD_INFO("scap cb normal threshold max/min:");
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_P, channel_num, channel_num);
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_N, channel_num, channel_num);

	TPD_INFO("scap cb waterproof threshold max/min:");
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_P, channel_num, channel_num);
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_scap_cb_data_waterproof_N, channel_num, channel_num);

	TPD_INFO("scap rawdata threshold max/min:");
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_P, channel_num, channel_num);
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_data_N, channel_num, channel_num);

	TPD_INFO("panel differ threshold max/min:");
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_P, node_num, rx_num);
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_panel_differ_data_N, node_num, rx_num);

	TPD_INFO("scap rawdata waterproof threshold max/min:");
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_P, channel_num, channel_num);
	ft3658u_print_buffer(ts_data->ft3658u_autotest_offset->ft3658u_scap_raw_waterproof_data_N, channel_num, channel_num);
}
...
int ft3658u_test_entry(struct chip_data_ft3658u *ts_data)
{
	...
	ret = ft3658u_get_threshold(ts_data, NULL);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("get threshold fail,ret=%d\n", ret);
		return 0xFF;
	}

	ret = ft3658u_get_threshold_from_img(ts_data, NULL, limit_fw);
	if (ret < 0) {
		FTS_TEST_SAVE_ERR("get threshold from img fail,ret=%d\n", ret);
		return 0xFF;
	}

	ft3658u_print_threshold(ts_data);
	...
	ret = ft3658u_test_read_reg(0x14, &ts_data->fre_num);
	if (ret < 0) {
	...
	ret = ft3658u_start_test(ts_data);
	seq_printf(ts_data->s, "%d error(s). %s\n", ret, ret ? "" : "All test passed.");

test_err:
	enter_work_mode();
	ft3658u_threshold_free(ts_data);
	ft3658u_autotest_endoperation(ts_data, limit_fw);
	TPD_INFO("%s -\n", __func__);
	return ret;
}
