...
static struct chip_data_s3706 *g_chip_info[TP_SUPPORT_MAX] = {NULL};
static int synaptics_get_chip_info(void *chip_data);
static int synaptics_mode_switch(void *chip_data, work_mode mode, int flag);
				      ...
				      struct point_info *points, int max_num)
{
	...
	char buf[8 * MAX_FINGER_NUM];

	memset(buf, 0, sizeof(buf));
	obj_attention = touch_i2c_read_word(chip_info->client,
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.F12_2D_DATA_BASE, 8 * fingers_to_process, buf);
	...
	for (i = 0; i < fingers_to_process; i++) {
		...
		points[i].tx_press = (buf[i * 8 + 5] >> 4) & 0x0F;
		points[i].rx_press = buf[i * 8 + 5] & 0x0F;
	}
...
static int synaptics_read_F54_base_reg(struct chip_data_s3706 *chip_info)
{
	...
	TPD_INFO("F54_QUERY_BASE = %x \n\
                        ...
                        F54_CTRL_BASE   = %x \n\
                        F54_DATA_BASE   = %x \n",
		 chip_info->reg_info.F54_ANALOG_QUERY_BASE,
		 chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
		 chip_info->reg_info.F54_ANALOG_CONTROL_BASE,
...
static void synaptics_health_zone_init(struct chip_data_s3706 *chip_info)
{
	...
	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.F51_CUSTOM_QUERY_BASE + 0x04);
...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	reg_info->F12_2D_DATA04 = reg_info->F12_2D_DATA_BASE +
				  2;   /*where is gesture type register*/
	...
	reg_info->F12_2D_CTRL27 = reg_info->F12_2D_CTRL_BASE +
				  0x09;        /*where is gesture type register*/
	...
	reg_info->F51_CUSTOM_CTRL50 = reg_info->F51_CUSTOM_CTRL_BASE +
				      0x0A;        /*where is edge register*/
	...
	reg_info->F51_CUSTOM_CTRL04_08 = reg_info->F51_CUSTOM_CTRL_BASE +
					 0x10;         /*switch for corner mode*/
	...
	reg_info->F51_CUSTOM_CTRL30 = reg_info->F51_CUSTOM_CTRL_BASE +
				      0x38;    /*switch for game mode*/
...
static uint32_t synaptics_get_fw_id(struct chip_data_s3706 *chip_info)
{
	...
	touch_i2c_read_block(chip_info->client, chip_info->reg_info.F34_FLASH_CTRL_BASE,
			     4, buf);
...
static uint8_t synaptics_get_noise_level(struct chip_data_s3706 *chip_info)
{
	...
	touch_i2c_write_byte(chip_info->client, 0xff, 0x0);
	ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F12_2D_CTRL11,
				   13, &(game_buffer[0]));
...
static uint8_t synaptics_get_lock_point_level(struct chip_data_s3706 *chip_info)
{
	...
	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.F51_CUSTOM_CTRL30);
		...
		struct resolution_info *resolution_info, struct panel_info *panel_data)
{
	...
	touch_i2c_read_block(chip_info->client, chip_info->reg_info.F12_2D_CTRL08, 4,
			     buf);
	...
	bootloader_mode = touch_i2c_read_byte(chip_info->client,
				      ...
				      bool enable)
{
	...
	if (enable) {
		...
		ret = touch_i2c_read_byte(chip_info->client,
					  chip_info->reg_info.F01_RMI_DATA01);
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F01_RMI_CTRL01, abs_status_int);
...
static u32 synaptics_u32_trigger_reason(void *chip_data, int gesture_enable,
					int is_suspended)
{
	int ret = 0;
					...
					bool config)
{
	...
	if (config) { /*configed  && out of sleep mode*/
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F01_RMI_CTRL00, (ret & 0xf8) | 0x80);
		...
		if (ctrl_buf[2] & 0x02) {
			...
			retval = touch_i2c_read_byte(chip_info->client,
						     chip_info->reg_info.F51_CUSTOM_CTRL13);
			...
			if (retval != 0) {
				touch_i2c_write_byte(chip_info->client,
						     chip_info->reg_info.F51_CUSTOM_CTRL00 + 3, 0x00);
				touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
				touch_i2c_write_byte(chip_info->client,
						     chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);    /* force update*/
		...
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F01_RMI_CTRL00, (ret & 0xf8) | 0x81);
...
static void synaptics_enable_fingerprint_underscreen(void *chip_data,
		uint32_t enable)
{
	int ret = 0;
	struct chip_data_s3706 *chip_info = (struct chip_data_s3706 *)chip_data;
	...
	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.F51_CUSTOM_CTRL20);
...
static int synaptics_corner_limit_handle(struct chip_data_s3706 *chip_info,
		int enable)
{
	int ret = -1;
	...
	pre_set_1 = touch_i2c_read_byte(chip_info->client,
					chip_info->reg_info.F51_CUSTOM_CTRL04_05);
	pre_set_2 = touch_i2c_read_byte(chip_info->client,
					chip_info->reg_info.F51_CUSTOM_CTRL04_06);
	pre_set_3 = touch_i2c_read_byte(chip_info->client,
					chip_info->reg_info.F51_CUSTOM_CTRL04_08);
	...
	if (LANDSCAPE_SCREEN_90 == enable) {
		...
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F51_CUSTOM_CTRL04_05, 0xFF);        /*x part*/
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F51_CUSTOM_CTRL04_06, 0x44);        /*y part*/
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F51_CUSTOM_CTRL04_08, 0x03);
		...
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F51_CUSTOM_CTRL04_06, 0x44);        /*y part*/
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F51_CUSTOM_CTRL04_08, 0x0C);
		...
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F51_CUSTOM_CTRL04_05, 0x24);        /*x part*/
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F51_CUSTOM_CTRL04_06, 0xF5);        /*y part*/
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F51_CUSTOM_CTRL04_08, 0x05);
	}
				       ...
				       int enable)
{
	...
	if (!enable) {
		...
		touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL50,
				     ret);
					...
					bool enable)
{
	...
	if (enable) { /*enable nosleep flag*/
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F01_RMI_CTRL00, (ret | 0x04));
		...
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F01_RMI_CTRL00, (ret & 0xFB));
		...
		bool enable)
{
	...
	if (enable) {
		...
		touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F01_RMI_CTRL00,
				     arg);
				       ...
				       bool enable)
{
	...
	if (chip_info->report_120hz_support) {
		...
		ret = touch_i2c_read_byte(chip_info->client,
					  chip_info->reg_info.F51_CUSTOM_CTRL30);  /* read the strong lock point level related register*/
		...
		/* set the strong lock point level*/
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F51_CUSTOM_CTRL30, tmp);
	...
	ret = touch_i2c_write_block(chip_info->client,
				    chip_info->reg_info.F12_2D_CTRL11, 13, &game_buffer[0]);
...
static int synaptics_mode_switch(void *chip_data, work_mode mode, int flag)
{
	...
	case MODE_GAME:
		...
		synaptics_set_report_point_first(chip_info, flag);
		break;
				      ...
				      struct gesture_info *gesture)
{
	...
	ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F12_2D_DATA04,
				   5, &(gesture_buffer[0]));      /*get gesture type*/
	gesture_sign = gesture_buffer[0];
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x4);
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.F51_CUSTOM_DATA_BASE, 25,
	...
	case SWIPE_DETECT:
		gesture->gesture_type = (regswipe == 0x41) ? LEFT2RIGHT_SWIP   :
					(regswipe == 0x42) ? RIGHT2LEFT_SWIP   :
					(regswipe == 0x44) ? UP2DOWN_SWIP          :
					(regswipe == 0x48) ? DOWN2UP_SWIP          :
					(regswipe == 0x80) ? DOU_SWIP                  :
					UNKOWN_GESTURE;
		break;
	...
	case VEE_DETECT:
		gesture->gesture_type = (gesture_buffer[2] == 0x01) ? DOWN_VEE  :
					(gesture_buffer[2] == 0x02) ? UP_VEE        :
					(gesture_buffer[2] == 0x04) ? RIGHT_VEE :
					(gesture_buffer[2] == 0x08) ? LEFT_VEE  :
					UNKOWN_GESTURE;
		break;

	case UNICODE_DETECT:
		gesture->gesture_type = (gesture_buffer[2] == 0x77
					 && gesture_buffer[3] == 0x00) ? W_GESTURE :
					(gesture_buffer[2] == 0x6d && gesture_buffer[3] == 0x00) ? M_GESTRUE :
					UNKOWN_GESTURE;
		break;
...
/*************************************auto test Funtion**************************************/
unsigned char GetLogicalPin(unsigned char p_pin, uint8_t rx_num,
			    uint8_t *rx_physical)
{
	unsigned char i = 0;
...
static int synaptics_nocbc_capacity_test(struct seq_file *s, void *chip_data,
		...
		struct test_item_info *p_test_item_info)
{
	...
	/*for limit top and floor data*/
	int32_t *baseline_data_test_top = NULL, *baseline_data_test_floor = NULL;
	struct chip_data_s3706 *chip_info = (struct chip_data_s3706 *)chip_data;

	if (!syna_testdata || !p_test_item_info) {
		error_count++;
		return error_count;
	}

	memset(chip_info->raw_data, 0, chip_info->raw_data_size);

	if (p_test_item_info->para_num) {
		if (p_test_item_info->p_buffer[0]) {
			if (p_test_item_info->item_limit_type == LIMIT_TYPE_TX_RX_DATA) {
				baseline_data_test_top = (int32_t *)(syna_testdata->fw->data +
								     p_test_item_info->top_limit_offset);
				baseline_data_test_floor = (int32_t *)(syna_testdata->fw->data +
	...
	if (!baseline_data_test_top || !baseline_data_test_floor) {
		error_count++;
		return error_count;
	}
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x14);/*select report type 0x14*/
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0X02);/*force Cal*/
	checkCMD(chip_info, 30);
	TPD_DEBUG("Force Cal oK\n");
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	checkCMD(chip_info, 30);
	TPD_INFO("key_tx is %d, key_rx is %d\n", syna_testdata->key_tx,
		 syna_testdata->key_rx);
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
				   syna_testdata->tx_num * syna_testdata->rx_num * 2,
				   chip_info->raw_data);     /*read data*/
	...
	for (x = 0; x < syna_testdata->tx_num; x++) {
		...
		for (y = 0; y < syna_testdata->rx_num; y++) {
			...
			tmp_arg1 = chip_info->raw_data[z * 2] & 0xff;
			tmp_arg2 = chip_info->raw_data[z * 2 + 1] & 0xff;
			...
			if ((y < syna_testdata->rx_num - syna_testdata->key_rx)
				...
				if ((baseline_data < * (baseline_data_test_floor + count))
						|| (baseline_data > *(baseline_data_test_top + count))) {
					TPD_INFO("Synaptic:touchpanel failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
						 x, y, baseline_data, *(baseline_data_test_floor + count),
						 *(baseline_data_test_top + count));
					seq_printf(s,
				...
				TPD_DEBUG("touchkey test xy(%d, %d), baseline_data=%d, limite( %d, %d)\n ", x,
					  y, baseline_data,
					  *(baseline_data_test_floor + count), *(baseline_data_test_top + count));
...
static int synaptics_withcbc_capacity_test(struct seq_file *s, void *chip_data,
		struct auto_testdata *syna_testdata,
		struct test_item_info *p_test_item_info)
{
	int ret = 0;
	...
	memset(chip_info->raw_data, 0, chip_info->raw_data_size);
	memset(chip_info->data_buf, 0, chip_info->data_buf_size);
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x03);/*select report type 0x03*/
				    ...
				    struct test_item_info *p_test_item_info)
{
	...
	buffer_rx = tp_devm_kzalloc(&chip_info->client->dev,
				    syna_testdata->rx_num * (sizeof(uint8_t)), GFP_KERNEL);

	if (!buffer_rx) {
		error_count++;
		TPD_INFO("buffer_rx kzalloc error\n");
		goto ERROR;
	}

	buffer_tx = tp_devm_kzalloc(&chip_info->client->dev,
				    syna_testdata->tx_num * (sizeof(uint8_t)), GFP_KERNEL);

	if (!buffer_tx) {
		error_count++;
		TPD_INFO("buffer_tx kzalloc error\n");
		goto ERROR;
	}
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x19);/*select report type 25*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x0);
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	/*msleep(100);*/
	checkCMD(chip_info, 30);
					...
					for (j = 0; j < syna_testdata->rx_num; j++) {
						if (sum_line_error == buffer_rx[j]) {
							...
							TPD_INFO(" step 3 :check TRx-to-Ground, with rt25 error,  error_line is rx = %d\n",
								 buffer_rx[j]);
							seq_printf(s,
								   " step 3 :check TRx-to-Ground, with rt25,  error_line is rx = %d\n",
								   buffer_rx[j]);
							goto ERROR;
						}
					}
...
ERROR:
	...
	msleep(50);
	tp_devm_kfree(&chip_info->client->dev, (void **)&buffer_rx,
		      syna_testdata->rx_num * sizeof(uint8_t));
	tp_devm_kfree(&chip_info->client->dev, (void **)&buffer_tx,
		      syna_testdata->tx_num * sizeof(uint8_t));
	return error_count;
}
				    ...
				    struct test_item_info *p_test_item_info)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x1A);/*select report type 26*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x0);
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	...
	for (x = 0; x < 7; x++) {
		...
		if (buffer[x] != 0) {
			if ((x == 0)&& (buffer[x] & 0x01)) {
			    /*pin - 0, these four pins should be test in RT100*/
			} else if ((x == 0) && (buffer[x] & 0x02)) {
			   /*pin - 1*/
			} else if ((x == 4) && (buffer[x] & 0x01)) {
			   /*pin -32*/
			} else if ((x == 4) && (buffer[x] & 0x02)) {
			   /*pin -33*/
			} else {
				error_count++;
				     ...
				     struct test_item_info *p_test_item_info)
{
	...
	p_data_baseline1 = tp_devm_kzalloc(&chip_info->client->dev,
					   syna_testdata->tx_num * syna_testdata->rx_num * 2 * (sizeof(uint8_t)),
					   GFP_KERNEL);
	...
	p_data_baseline2 = tp_devm_kzalloc(&chip_info->client->dev,
					   syna_testdata->tx_num * syna_testdata->rx_num * 2 * (sizeof(uint8_t)),
					   GFP_KERNEL);
	...
	p_data_delta = tp_devm_kzalloc(&chip_info->client->dev,
				       syna_testdata->tx_num * syna_testdata->rx_num * 2 * (sizeof(uint8_t)),
				       GFP_KERNEL);
	...
	rx_assignment = tp_devm_kzalloc(&chip_info->client->dev,
					syna_testdata->rx_num * (sizeof(uint8_t)), GFP_KERNEL);

	if (!rx_assignment) {
		error_count++;
		TPD_INFO("rx_assignment kzalloc error\n");
		goto END;
	}

	rx_physical = tp_devm_kzalloc(&chip_info->client->dev,
				      syna_testdata->rx_num * (sizeof(uint8_t)), GFP_KERNEL);

	if (!rx_assignment) {
		error_count++;
		TPD_INFO("rx_physical kzalloc error\n");
		goto END;
	}

	minRX = tp_devm_kzalloc(&chip_info->client->dev,
				(syna_testdata->rx_num + 1) * (sizeof(uint8_t)), GFP_KERNEL);

	if (!rx_assignment) {
		error_count++;
		TPD_INFO("minRX kzalloc error\n");
		goto END;
	}

	maxRX = tp_devm_kzalloc(&chip_info->client->dev,
				(syna_testdata->rx_num + 1) * (sizeof(uint8_t)), GFP_KERNEL);

	if (!rx_assignment) {
		error_count++;
		TPD_INFO("maxRX kzalloc error\n");
		goto END;
	}
	...
	ret |= 0x02;
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.F54_ANALOG_CONTROL_BASE, ret);           /*set no scan*/

	/*set all local cbc to 0*/
	touch_i2c_write_block(chip_info->client,
			      chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x1D, syna_testdata->rx_num,
			      &temp_data[0]);
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);    /* force update*/
	checkCMD(chip_info, 30);
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x64);/*select report type 100*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x0);
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
				   syna_testdata->tx_num * syna_testdata->rx_num * 2,
				   chip_info->raw_data);         /*read raw data1*/
	...
	for (x = 0, z = 0; x < syna_testdata->tx_num; x++) {
		...
		for (y = 0; y < syna_testdata->rx_num; y++) {
			baseline_data = chip_info->raw_data[z] | (chip_info->raw_data[z + 1] << 8);
	...
	touch_i2c_read_block(chip_info->client, 0x01, syna_testdata->rx_num,
			     rx_assignment);
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
	...
	for (i = 0; i < 4; i++) {
		...
		logical_pin = GetLogicalPin(ExtendRT26_pin[i], syna_testdata->rx_num,
					    rx_physical);
		...
		ret = touch_i2c_read_block(chip_info->client,
					   chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
					   syna_testdata->tx_num * syna_testdata->rx_num * 2,
					   chip_info->raw_data);         /*read raw data2*/
		...
		for (x = 0; x < syna_testdata->rx_num; x++) {
			if (x == logical_pin) {
				if (minRX[x] < 600) {
					...
					seq_printf(s,
						   "step 5:check RT100 for pin 0,1,32,33 test failed: minRX[%-2d] = %4d when test pin %d (RX Logical pin [%d])\n",
						   x, minRX[x], ExtendRT26_pin[i], logical_pin);
					goto END;
				}
...
END:
	synaptics_reset(chip_info);
	msleep(50);
	tp_devm_kfree(&chip_info->client->dev, (void **)&p_data_baseline1,
		      syna_testdata->tx_num * syna_testdata->rx_num * 2 * sizeof(uint8_t));
	tp_devm_kfree(&chip_info->client->dev, (void **)&p_data_baseline2,
		      syna_testdata->tx_num * syna_testdata->rx_num * 2 * sizeof(uint8_t));
	tp_devm_kfree(&chip_info->client->dev, (void **)&p_data_delta,
		      syna_testdata->tx_num * syna_testdata->rx_num * 2 * sizeof(uint8_t));
	tp_devm_kfree(&chip_info->client->dev, (void **)&rx_assignment,
		      syna_testdata->rx_num * sizeof(uint8_t));
	tp_devm_kfree(&chip_info->client->dev, (void **)&rx_physical,
		      syna_testdata->rx_num * sizeof(uint8_t));
	tp_devm_kfree(&chip_info->client->dev, (void **)&minRX,
		      (syna_testdata->rx_num + 1) * sizeof(uint8_t));
	tp_devm_kfree(&chip_info->client->dev, (void **)&maxRX,
		      (syna_testdata->rx_num + 1) * sizeof(uint8_t));
				     ...
				     struct test_item_info *p_test_item_info)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x85);/*select report type 0x85*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	...
	for (y = 0; y < syna_testdata->rx_num; y++) {
		...
		if (baseline_data > 100) {
			...
			seq_printf(s,
				   "step 6 :check the broken line, error_line is y =%d, data[%d] = %d\n", y, y,
				   baseline_data);
			return error_count;
		}
	}
				     ...
				     struct test_item_info *p_test_item_info)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x96);/*select report type RT150*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	...
	for (y = 0;
			y < syna_testdata->rx_num + syna_testdata->tx_num - syna_testdata->key_tx -
			syna_testdata->key_rx; y++) {
		ret = touch_i2c_read_byte(chip_info->client,
					  chip_info->reg_info.F54_ANALOG_DATA_BASE + 3);
		tmp_arg1 = ret & 0xff;
		ret = touch_i2c_read_byte(chip_info->client,
		...
		if (unsigned_baseline_data < 7000) {
			...
			seq_printf(s,
				   "Step 7 : Check RT150 for random touch event, error_line is y =%d, data[%d] = %hu\n",
				   y, y, unsigned_baseline_data);
			return error_count;
		}
	}
				     ...
				     struct test_item_info *p_test_item_info)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x9A);/*select report type 0xFE*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	...
	for (y = 0; y < chip_info->hw_res->rx_num + chip_info->hw_res->tx_num; y++) {
		...
		if ((syna_testdata->key_rx != 0 || syna_testdata->key_tx != 0)
				&& (((y > chip_info->hw_res->rx_num - syna_testdata->key_rx - 1)
				    ...
				    || (y > chip_info->hw_res->rx_num + chip_info->hw_res->tx_num -
					syna_testdata->key_tx - 1))) {
			TPD_INFO("y = %d\n", y);
			continue;
		}
		...
		if (unsigned_baseline_data < 50 || unsigned_baseline_data > 975) {
			...
			TPD_INFO("Step 8 : Check RT154 for self raw data check, error_line is y =%d, data[%d] = %hu\n",
				 y, y, unsigned_baseline_data);
			seq_printf(s,
				   "Step 8 : Check RT154 for self raw data check, error_line is y =%d, data[%d] = %hu\n",
				   y, y, unsigned_baseline_data);
			return error_count;
		}
	}
				     ...
				     struct test_item_info *p_test_item_info)
{
	...
	ret |= 0x01;
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_CONTROL_BASE, ret);       /*set no relax*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);    /* force update*/
	checkCMD(chip_info, 30);
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x9B);/*select report type 0x9B*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	...
	for (y = 0; y < chip_info->hw_res->rx_num + chip_info->hw_res->tx_num; y++) {
		...
		if (baseline_data > 2000) {
			...
			TPD_INFO("Step 9 : Check RT155 for abs delta error, error_line is y =%d, data[%d] = %d\n",
				 y, y, baseline_data);
			seq_printf(s,
				   "Step 9 : Check RT155 for abs delta error, error_line is y =%d, data[%d] = %d\n",
				   y, y, baseline_data);
			return error_count;
		}
	}
				    ...
				    struct test_item_info *p_test_item_info)
{
	...
	int error_count = 0;
	int i = 0, j = 0;
	const int data_cycle = 20,
		  node_num = syna_testdata->tx_num + syna_testdata->rx_num, judge_threshold = 300;
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x3B);/*select report type 59*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0X02);/*force Cal*/
	checkCMD(chip_info, 30);
	...
	for (i = 0; i < data_cycle; i++) {
		...
		checkCMD(chip_info, 30);
		ret = touch_i2c_read_block(chip_info->client,
					   chip_info->reg_info.F54_ANALOG_DATA_BASE + 3, node_num * 4,
					   chip_info->raw_data);     /*read data*/

		for (j = 0; j < node_num; j++) {
			node_data = (chip_info->raw_data[j * 4] | (chip_info->raw_data[j * 4 + 1]  << 8)
				     | (chip_info->raw_data[j * 4 + 2]  << 16) | (chip_info->raw_data[j * 4 + 3]  <<
						     24));
				    ...
				    struct test_item_info *p_test_item_info)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x3F);/*select report type 63*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0X02);/*force Cal*/
	checkCMD(chip_info, 30);
	ret = touch_i2c_write_word(chip_info->client,
	...
	for (i = 0; i < syna_testdata->rx_num; i++) {
		node_data = (chip_info->raw_data[i * 4] | (chip_info->raw_data[i * 4 + 1]  << 8)
			     | (chip_info->raw_data[i * 4 + 2]  << 16) | (chip_info->raw_data[i * 4 + 3]  <<
					     24));
...
static int synaptics_test_enable_interrupt(void *chip_data, bool enable)
{
	int ret = 0;
	struct chip_data_s3706        *chip_info = (struct chip_data_s3706 *)chip_data;

	ret = synaptics_enable_interrupt(chip_info, enable);
		...
		struct test_item_info *p_test_item_info)
{
	...
	chip_info->raw_data = kzalloc(syna_testdata->tx_num * syna_testdata->rx_num * 2
				      * (sizeof(uint8_t)), GFP_KERNEL);

	if (!chip_info->raw_data) {
		TPD_INFO("raw_data kzalloc error\n");
		return -1;
	}

	chip_info->raw_data_size = syna_testdata->tx_num * syna_testdata->rx_num * 2 *
				   (sizeof(uint8_t));
	chip_info->data_buf = kzalloc(64 * (sizeof(uint8_t)), GFP_KERNEL);
...
static void synaptics_baseline_read(struct seq_file *s, void *chip_data)
{
	...
	raw_data = tp_devm_kzalloc(&chip_info->client->dev,
	...
	do {
		...
		if (enable_cbc) {
			...
			ret = touch_i2c_write_byte(chip_info->client,
						   ...
						   0x00);          /* Disable No SignalClarity*/
			ret = touch_i2c_write_byte(chip_info->client,
						   chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x17, (tmp_arg1 | 0x10));
			ret = touch_i2c_write_word(chip_info->client,
						   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);
			checkCMD(chip_info, 30);
			TPD_DEBUG("Test open cbc\n");
			...
			ret = touch_i2c_write_byte(chip_info->client,
						   chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x17, (tmp_arg1 & 0xef));
			ret = touch_i2c_write_word(chip_info->client,
						   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04); /* force update*/
			ret = touch_i2c_write_byte(chip_info->client,
...
END:
	tp_devm_kfree(&chip_info->client->dev, (void **)&raw_data,
		      chip_info->hw_res->tx_num * chip_info->hw_res->rx_num * 2 * (sizeof(uint8_t)));
	return;
}
...
static void synaptics_reserve_read(struct seq_file *s, void *chip_data)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x04);        /* page 4*/
	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.F51_CUSTOM_DATA_BASE + 0x1A);
...
static void synaptics_RT76_read(struct seq_file *s, void *chip_data)
{
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x4C);/*select report type 0x4C*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
...
static void synaptics_RT251_read(struct seq_file *s, void *chip_data)
{
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x96);/*select report type 0xFC*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	...
	for (y = 0;
			y < chip_info->hw_res->rx_num + chip_info->hw_res->tx_num -
			chip_info->hw_res->key_tx - chip_info->hw_res->key_rx; y++) {
		ret = touch_i2c_read_byte(chip_info->client,
					  chip_info->reg_info.F54_ANALOG_DATA_BASE + 3);
		tmp_arg1 = ret & 0xff;
		ret = touch_i2c_read_byte(chip_info->client,
...
static void synaptics_delta_read(struct seq_file *s, void *chip_data)
{
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0x02);/*select report type 0x02*/
	ret = touch_i2c_write_word(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
	...
	if (tp_debug != 0 && chip_info->health_zone.health_support == true) {
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F54_ANALOG_DATA_BASE, 0xC8);/*select report type 0xC8*/
		ret = touch_i2c_write_word(chip_info->client,
					   chip_info->reg_info.F54_ANALOG_DATA_BASE + 1, 0x00);/*set fifo 00*/
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x01);/*get report*/
...
static void synaptics_get_delta_data(void *chip_data, int32_t *delta_data)
{
	int ret = 0, x = 0, y = 0, z = 0;
	uint8_t *raw_data = NULL;
	struct chip_data_s3706 *chip_info = (struct chip_data_s3706 *)chip_data;

	if (!chip_info || !delta_data) {
		return;
	}
	...
	for (x = 0; x < chip_info->hw_res->tx_num; x++) {
		for (y = 0; y < chip_info->hw_res->rx_num; y++) {
			z = chip_info->hw_res->rx_num * x + y;
			delta_data[z] = (raw_data[z * 2 + 1] << 8) | raw_data[z * 2];
...
{
	TPD_INFO("%s.\n", __func__);
	synaptics_resetgpio_set(g_chip_info[rmi4_data->tp_index]->hw_res,
				false); /* reset gpio*/
	msleep(10);
	synaptics_resetgpio_set(g_chip_info[rmi4_data->tp_index]->hw_res,
				true); /* reset gpio*/
...
static int fwu_scan_pdt(struct chip_data_s3706 *chip_info)
{
	...
	rmi4_data->intr_mask[0] |=
		chip_info->fwu->intr_mask;           /*Roland interrupts mode for write flash, like checkFlashState in 3508 notice*/
...
static int fwu_write_f34_v7_command_single_transaction(
						struct chip_data_s3706 *chip_info, unsigned char cmd)
{
	int retval;
	unsigned char data_base;
...
static int fwu_read_f34_v7_queries(struct chip_data_s3706 *chip_info)
{
	...
	for (offset = 0; offset < V7_PARTITION_SUPPORT_BYTES;
			offset++) {       /*count partition nums*/
		for (ii = 0; ii < 8; ii++) {
	...
	ptable = tp_devm_kzalloc(&chip_info->client->dev,
				 chip_info->fwu->partition_table_bytes, GFP_KERNEL);
	...
	retval = fwu_read_f34_v7_partition_table(chip_info,
			ptable);            /*partition table*/

	if (retval < 0) {
		TPD_INFO("%s: Failed to read partition table\n", __func__);
		tp_devm_kfree(&chip_info->client->dev, (void **)&ptable,
			      chip_info->fwu->partition_table_bytes);
		return retval;
	}
				   ...
				   bool force)
{
	...
	chip_info->fwu = tp_devm_kzalloc(&chip_info->client->dev,
					 sizeof(struct synaptics_rmi4_fwu_handle), GFP_KERNEL);
	...
	retval = touch_i2c_read_block(chip_info->client, PDT_PROPS,
				      sizeof(pdt_props.data), pdt_props.data);
	...
	chip_info->fwu->rmi4_data = tp_devm_kzalloc(&chip_info->client->dev,
	...
	chip_info->fwu->rmi4_data->tp_index = chip_info->tp_index;

	chip_info->fwu->rmi4_data->reset_device =
		s3706_reset_device;                /*no need*/
...
exit_free_rmi:
	tp_devm_kfree(&chip_info->client->dev, (void **)&chip_info->fwu->rmi4_data,
		      sizeof(struct synaptics_rmi4_data));
	chip_info->fwu->rmi4_data = NULL;

exit_free_mem:
	tp_devm_kfree(&chip_info->client->dev, (void **)&chip_info->fwu,
		      sizeof(struct synaptics_rmi4_fwu_handle));
	chip_info->fwu = NULL;
exit:
	return retval;
}
...
static void fwu_parse_image_header_10_bootloader(
			struct chip_data_s3706 *chip_info, const unsigned char *image)
{
	unsigned char ii;
...
static int fwu_parse_image_info(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->bl_version == BL_V7
		...
		fwu_compare_partition_tables(
			chip_info);        /*compare two firmware img between ic and emmc, compare addr , if not equal, return or reflash partition*/
				     ...
				     unsigned int *fw_id)
{
	...
	if (chip_info->fwu->img.contains_firmware_id) {
		*fw_id = chip_info->fwu->img.firmware_id;
		...
		strptr += 2;
		firmware_id = tp_devm_kzalloc(&chip_info->client->dev, MAX_FIRMWARE_ID_LEN,
					      GFP_KERNEL);
		...
		tp_devm_kfree(&chip_info->client->dev, (void **)&firmware_id,
			      MAX_FIRMWARE_ID_LEN);
...
static enum flash_area fwu_go_nogo(struct chip_data_s3706 *chip_info)
{
	...
	device_fw_id =
		rmi4_data->firmware_id;          /*this is firmware id and control whether to update*/
	TPD_INFO("%s: Device firmware ID = %d\n", __func__, device_fw_id);

	/* Get image firmware ID */
	retval = fwu_get_image_firmware_id(chip_info,
	...
	if (image_fw_id != device_fw_id) {
		flash_area =
			UI_FIRMWARE;           /*UI_FIRMWARE means to update firmware and config, no need to judge config*/
		goto exit;
	}
...
static int fwu_enter_flash_prog(struct chip_data_s3706 *chip_info)
{
	...
	retval = fwu_write_f34_command(chip_info,
				       CMD_ENABLE_FLASH_PROG);       /*enter flash program mode*/

	if (retval < 0) {
		return retval;
	}

	retval = fwu_wait_for_idle(chip_info, ENABLE_WAIT_MS,
				   false);   /*wait for complete*/
...
static int fwu_write_lockdown(struct chip_data_s3706 *chip_info)
{
	...
	lockdown_block_count = chip_info->fwu->img.lockdown.size /
			       chip_info->fwu->block_size;

	return fwu_write_f34_blocks(chip_info,
				    (unsigned char *)chip_info->fwu->img.lockdown.data,
...
{
	return fwu_write_f34_blocks(chip_info,
				    (unsigned char *)chip_info->fwu->config_data,
...
static int fwu_write_ui_configuration(struct chip_data_s3706 *chip_info)
{
	...
	chip_info->fwu->config_block_count = chip_info->fwu->config_size /
					     chip_info->fwu->block_size;
...
static int fwu_write_bootloader(struct chip_data_s3706 *chip_info)
{
	...
	bootloader_block_count = chip_info->fwu->img.bl_image.size /
				 chip_info->fwu->block_size;

	chip_info->fwu->write_bootloader = true;
	retval = fwu_write_f34_blocks(chip_info,
				      (unsigned char *)chip_info->fwu->img.bl_image.data,
					...
					unsigned int count)
{
	if (count > chip_info->fwu->read_config_buf_size) {
		tp_devm_kfree(&chip_info->client->dev,
			      (void **)&chip_info->fwu->read_config_buf,
			      chip_info->fwu->read_config_buf_size);
		chip_info->fwu->read_config_buf = tp_devm_kzalloc(&chip_info->client->dev,
						  count, GFP_KERNEL);
...
static int fwu_write_utility_parameter(struct chip_data_s3706 *chip_info)
{
	...
	utility_param_size = chip_info->fwu->blkcount.utility_param *
			     chip_info->fwu->block_size;
	...
	retval = fwu_write_f34_blocks(chip_info,
				      (unsigned char *)chip_info->fwu->read_config_buf,
...
static int fwu_write_guest_code(struct chip_data_s3706 *chip_info)
{
	...
	guest_code_block_count = chip_info->fwu->img.guest_code.size /
				 chip_info->fwu->block_size;

	retval = fwu_write_f34_blocks(chip_info,
				      (unsigned char *)chip_info->fwu->img.guest_code.data,
...
static int fwu_write_firmware(struct chip_data_s3706 *chip_info)
{
	...
	firmware_block_count = chip_info->fwu->img.ui_firmware.size /
			       chip_info->fwu->block_size;

	return fwu_write_f34_blocks(chip_info,
				    (unsigned char *)chip_info->fwu->img.ui_firmware.data,
...
static int fwu_do_reflash(struct chip_data_s3706 *chip_info)
{
	...
	if (do_bl_update) {
		...
		retval = fwu_write_partition_table_v7(
				 chip_info);       /*reflash partition table*/
		...
		retval = fwu_write_partition_table_v8(
				 chip_info);       /*partition table is earsed in fwu_erase_all, rewrite FLASH_CONFIG_AREA partition*/
...
static int fwu_start_reflash(struct chip_data_s3706 *chip_info)
{
	...
	if (!monitor_data) {
		TPD_INFO("%s: monitor_data is NULL\n", __func__);
	}

	if (rmi4_data->sensor_sleep) {
		TPD_INFO("%s: Sensor sleeping\n", __func__);
		tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Sensor error sleeping");
		return -ENODEV;
	}
	...
	if (retval < 0) {
		tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Parse image info failed");
		goto exit;
	}
	...
	/*lots of info, large function* *enter bootloader mode*/
		...
		if (retval < 0) {
			tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Enter flash prog failed");
			chip_info->fwu->rmi4_data->reset_device(rmi4_data, false);
			goto exit;
		}
	}
	...
	case UI_CONFIG:
		...
		if (retval < 0) {
			tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Check UI config failed");
			break;
		}
		...
		if (retval < 0) {
			tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Erase UI config failed");
			break;
		}
		...
		if (retval < 0) {
			tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Write UI config failed");
		}
	...
	if (chip_info->fwu->do_lockdown
			&& (chip_info->fwu->img.lockdown.data !=
			    NULL)) {         /*lockdown mode , only can write once, use for save i2c addr, pull up, pull down info*/
		...
		case BL_V8:
			...
			if (retval < 0) {
				tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Lockdown failed");
		...
		const struct firmware *fw, bool force)
{
	...
	if (retval <
			0) {        /*less zero means the space has been released in synaptics_rmi4_fwu_init*/
...
static void synaptics_health_report(void *chip_data,
				    struct monitor_data *mon_data)
{
	...
	if (data_length < 1 || data_length > 255) {
		...
		ret = touch_i2c_read_block(chip_info->client,
					   ((uint8_t)(chip_info->health_zone.loglength_addr & 0xFF) + 1), data_length,
					   log_data);         /*read log*/
		...
		if (health_info->reset_reason != 0) {
			...
			touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL13,
					     ret | 0x80);        /* page 4*/
			TPD_DEBUG("chip_info->reg_info.F51_CUSTOM_CTRL13= %x  ret=%x\n",
				  chip_info->reg_info.F51_CUSTOM_CTRL_BASE + 0x0C, ret | 0x80);
		}

		tp_memcpy(health_local, sizeof(chip_info->health_zone.health_info), \
			  health_info, sizeof(struct health_info), \
			  sizeof(struct health_info));

		if (tp_debug != 0) {
			printk("health report:");
		...
		uint16_t register_addr, uint8_t *result, uint8_t length)
{
	...
	ret = touch_i2c_read_block(chip_info->client, l_tmp, length,
				   result);         /*read data*/
}
...
static int synaptics_get_face_state(void *chip_data)
{
	...
	state = touch_i2c_read_byte(chip_info->client,
				    chip_info->reg_info.F51_CUSTOM_DATA_BASE + 0x05);
		...
		struct fp_underscreen_info *fp_tpinfo)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x0);
	ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F12_2D_DATA04,
				   5, &(buf[0]));      /*get gesture type*/
	fp_tpinfo->touch_state = buf[0];

	touch_i2c_write_byte(chip_info->client, 0xff, 0x4);
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.F51_CUSTOM_DATA_BASE, 8, &(buf[0]));
...
static int synaptics_specific_resume_operate(void *chip_data,
...
static void synaptics_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	touch_i2c_write_byte(chip_info->client, 0xff, 0x4);
	gesture_mask = touch_i2c_read_byte(chip_info->client,
					   chip_info->reg_info.F51_CUSTOM_CTRL20);
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F51_CUSTOM_CTRL20, gesture_mask);
	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.F51_CUSTOM_CTRL20);
	TPD_INFO("%s :F51_CUSTOM_CTRL20 is %d\n", __func__, ret);
	touch_i2c_write_byte(chip_info->client, 0xff, 0x0);
...
static int synaptics_set_report_point_first(void *chip_data, uint32_t enable)
{
	...
	if (enable) { /*enable game mode control flag*/
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F51_CUSTOM_CTRL13, (ret | 0x04));
		...
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F51_CUSTOM_CTRL13, (ret & 0xFB));
...
static struct oplus_touchpanel_operations synaptics_ops = {
	...
	.trigger_reason         = synaptics_u32_trigger_reason,
...
static struct debug_info_proc_operations debug_info_proc_ops = {
	...
	.reserve1                 = synaptics_RT251_read,
	.reserve2                  = synaptics_RT76_read,
	.get_delta_data        = synaptics_get_delta_data,
};
			      ...
			      const struct i2c_device_id *id)
{
	...
	chip_info =  devm_kzalloc(&client->dev, sizeof(struct chip_data_s3706),
				  GFP_KERNEL);
	...
	chip_info->report_120hz_support = of_property_read_bool(ts->dev->of_node,
					  "report_120hz_support");
	/*get ts tp_index to chip info*/
	chip_info->tp_index = ts->tp_index;
	g_chip_info[chip_info->tp_index] = chip_info;
	...
	if (premote_data) {
		...
		premote_data->tp_index            = ts->tp_index;
...
ts_malloc_failed:
	devm_kfree(&client->dev, (void **)&chip_info);
	ret = -1;
...
struct synaptics_bootst_data {
	struct delayed_work work;
	struct workqueue_struct *workqueue;
	struct i2c_client *client;
	const struct i2c_device_id *dev_id;
};
static struct synaptics_bootst_data bootst_data;

static void synaptics_tp_probe_func(struct work_struct *w)
{
	struct i2c_client *client_boost = bootst_data.client;
	const struct i2c_device_id *dev_id = bootst_data.dev_id;

	TPD_INFO("after on cpu [%d]\n", smp_processor_id());
	probe_ret = synaptics_tp_probe(client_boost, dev_id);
}

static int boost_synaptics_tp_probe(struct i2c_client *client,
				    const struct i2c_device_id *id)
{
	int i;
	bootst_data.client = client;
	bootst_data.dev_id = id;
	bootst_data.workqueue = create_workqueue("tpd_probe_boost");

	INIT_DELAYED_WORK(&(bootst_data.work), synaptics_tp_probe_func);
	TPD_INFO("before on cpu [%d]\n", smp_processor_id());

	for (i = 0; i < NR_CPUS; i++) {
		TPD_INFO("check CPU[%d] is [%s]\n", i,
	...
	queue_delayed_work_on(i != NR_CPUS ? i : 0, bootst_data.workqueue,
			      &(bootst_data.work), msecs_to_jiffies(300));
...
#ifdef CONFIG_SYNAPTIC_RED
	unregister_remote_device(g_chip_info[ts->tp_index]->premote_data);
#endif

	if (ts) {
		synaptics_remove_proc(ts, g_chip_info[ts->tp_index]->syna_ops);
		unregister_common_touch_device(ts);
		common_touch_data_free(ts);
		devm_kfree(&client->dev, (void **)&g_chip_info[ts->tp_index]);
		kfree(ts);
	}
...
#ifdef TP_BOOST_UP
	.probe          = boost_synaptics_tp_probe,
#else
...
int __init tp_driver_init_3706(void)
{
	TPD_INFO("%s is called\n", __func__);
...
void __exit tp_driver_exit_3706(void)
{
