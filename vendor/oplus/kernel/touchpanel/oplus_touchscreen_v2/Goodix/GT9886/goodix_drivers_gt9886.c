...
static void goodix_special_register_info_read(struct seq_file *s, void *chip_data);
...
#define DOZE_CLOSE_OK_FLAG    0xBB
	...
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)ts->chip_data;

	mutex_lock(&chip_info->doze_mode_lock);

	if (enable) {
		if (chip_info->doze_mode_set_count != 0) {
			chip_info->doze_mode_set_count--;
		}
		...
		if (chip_info->doze_mode_set_count == 1) {
...
exit:
	mutex_unlock(&chip_info->doze_mode_lock);
	return result;
}
 ...
 *
 * */
static int gt9886_i2c_read(struct i2c_client *client, u16 addr, s32 len,
			   u8 *buffer)
{
	int ret = -EINVAL;
...
static int gt9886_i2c_write(struct i2c_client *client, u16 addr, s32 len,
			    u8 *buffer)
{
	int ret = -EINVAL;
...
*/
static int goodix_i2c_read_dbl_check(struct i2c_client *client, u16 addr,
				     u8 *buffer, s32 len)
{
	u8 buf[16] = {0};
	u8 confirm_buf[16] = {0};
...
s32 goodix_send_cmd(void *chip_data, u8 cmd, u8 data)
{
	...
	mutex_lock(&chip_info->cmd_mutex);
	...
	ret = touch_i2c_write_block(chip_info->client, chip_info->reg_info.GTP_REG_CMD,
				    3, &buffer[0]);
	...
	while (retry--) {
		ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.GTP_REG_CMD,
					   1, &buf_read_back);
		...
		if (retry_times > 5) {
			...
			tp_healthinfo_report(monitor_data, HEALTH_REPORT, "send_cmd_err_count");
		}
...
exit:
	...
	mutex_unlock(&chip_info->cmd_mutex);
...
static int goodix_clear_irq(void *chip_data)
{
	...
	if (!chip_info->gt8x_rawdiff_mode) {
		ret = touch_i2c_write_block(chip_info->client,
					    chip_info->reg_info.GTP_REG_READ_COOR, 1, &clean_flag);
...
static void goodix_esd_check_enable(struct chip_data_gt9886 *chip_info,
				    bool enable)
{
...
static int goodix_enable_gesture(struct chip_data_gt9886 *chip_info, int enable)
{
	int ret = -1;
...
static int goodix_enable_edge_limit(struct chip_data_gt9886 *chip_info,
				    int state)
{
	int ret = -1;
...
static int goodix_enable_charge_mode(struct chip_data_gt9886 *chip_info,
				     bool enable)
{
	int ret = -1;
...
static int goodix_enable_game_mode(struct chip_data_gt9886 *chip_info,
				   bool enable)
{
	int ret = 0;
...
static int goodix_reset_esd_status(struct chip_data_gt9886 *chip_info)
{
	...
	ret = gt9886_i2c_write(chip_info->client, chip_info->reg_info.GTP_REG_ESD_WRITE,
			       1, &value);
...
static s32 goodix_read_version(struct chip_data_gt9886 *chip_info,
			       struct panel_info *panel_data)
{
	...
	while (retry--) {
		ret = gt9886_i2c_read(chip_info->client,
				      chip_info->reg_info.GTP_REG_PRODUCT_VER, sizeof(buf), buf);
		...
		if (ret > 0) {
			...
			if (checksum == 0 &&    /* first 3 bytes must be number or char */
					IS_NUM_OR_CHAR(buf[9]) && IS_NUM_OR_CHAR(buf[10]) && IS_NUM_OR_CHAR(buf[11])
					&& buf[21] != 0xFF) {    /*sensor id == 0xFF, retry */
				break;
...
static int goodix_send_large_config(struct chip_data_gt9886 *chip_info,
				    u8 *config, int cfg_len)
{
	int r = 0;
	int try_times = 0;
	u8 buf = 0;
...
RETRY:
	...
	for (try_times = 0; try_times < TS_WAIT_CMD_FREE_RETRY_TIMES; try_times++) {
		if (gt9886_i2c_read(chip_info->client, command_reg, 1, &buf) >= 0
	...
	if (try_times >= TS_WAIT_CMD_FREE_RETRY_TIMES) {
		TPD_INFO("Send large cfg FAILED, before send, reg:0x%04x is not 0xff",
			 command_reg);
	...
	if (goodix_wait_cfg_cmd_ready(chip_info, COMMAND_SEND_CFG_PREPARE_OK,
				      COMMAND_START_SEND_LARGE_CFG)) {
	...
	if (try_times >= TS_WAIT_CMD_FREE_RETRY_TIMES) {
		TPD_INFO("Send large cfg FAILED, after send, reg:0x%04x is not 0xff",
			 command_reg);
		r = -EINVAL;
		goto exit;
	}
...
static s32 goodix_send_config(struct chip_data_gt9886 *chip_info, u8 *config,
			      int cfg_len)
{
	int ret = 0;
	...
	mutex_lock(&chip_info->config_lock);
	...
	mutex_unlock(&chip_info->config_lock);
...
static s32 goodix_request_event_handler(struct chip_data_gt9886 *chip_info)
{
	...
	ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.GTP_REG_RQST,
				   1, &rqst_data);
	...
	ret = touch_i2c_write_block(chip_info->client, chip_info->reg_info.GTP_REG_RQST,
				    1, &rqst_data);
...
int goodix_get_channel_num(struct i2c_client *client, u32 *sen_num,
			   u32 *drv_num)
{
	int ret = -1;
	u8 buf[2] = {0};
				      ...
				      struct resolution_info *resolution_info, struct panel_info *panel_data)
{
	...
	do {
		goodix_i2c_read_dbl_check(chip_info->client,
					  chip_info->reg_info.GTP_REG_FW_CHK_MAINSYS, reg_val, 1);
...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	for (i = 0; i < fw_info->subsys_num; i++) {
		...
		fw_info->subsys[i].size = be32_to_cpup((__be32 *)&firmware->data[info_offset +
						       1]);
		fw_info->subsys[i].flash_addr = be16_to_cpup((__be16 *)
						&firmware->data[info_offset + 5]);
	...
	TPD_INFO("Fimware VID:%02X%02X%02X%02X", fw_info->fw_vid[0], fw_info->fw_vid[1],
		 fw_info->fw_vid[2], fw_info->fw_vid[3]);
				    ...
				    unsigned int addr, unsigned char *data, unsigned int len)
{
	...
	if (len > sizeof(cfm_buf)) {
		cfm = tp_devm_kzalloc(&client->dev, len, GFP_KERNEL);
...
exit:
	...
	if (cfm != &cfm_buf[0]) {
		tp_devm_kfree(&client->dev, (void **)&cfm, len);
	}
 ...
 * return 0 ok, <0 error
 */
static int goodix_load_isp(struct i2c_client *client,
			   struct firmware_data *fw_data)
{
	...
	r = goodix_reg_write_confirm(client, HW_REG_ISP_ADDR, (u8 *)fw_isp->data,
				     fw_isp->size);
	...
	TPD_DEBUG("Success forbit bank0 accedd, set 0x%x -->0x00",
...
static int goodix_update_prepare(void *chip_data,
				 struct fw_update_ctrl *fwu_ctrl)
{
	...
	reg_val[0] = 0x00;
	r = goodix_reg_write_confirm(chip_info->client, HW_REG_DSP_MCU_POWER, reg_val,
				     1);
	...
	TPD_DEBUG("Success enabled DSP&MCU power,set 0x%x-->0x00",
				   ...
				   u16 len, const u8 *data)
{
	...
	pkt[1] = len & 0xff;
	pkt[2] = (flash_addr >> 16) &
		 0xff; /* u16 >> 16bit seems nosense but really important */
				 ...
				 u8 *pkt, u32 len)
{
	...
	while (total_size > 0) {
		data_size = total_size > I2C_DATA_MAX_BUFFERSIZE ? I2C_DATA_MAX_BUFFERSIZE :
			    total_size;
		TPD_INFO("I2C firmware to %08x,size:%u bytes all len:%u bytes", offset,
			 data_size, len);
		r = goodix_reg_write_confirm(client, HW_REG_ISP_BUFFER + offset, &pkt[offset],
					     data_size);
	...
	for (i = 0; i < TS_READ_FLASH_STATE_RETRY_TIMES; i++) {
		...
		if (reg_val[0] == ISP_FLASH_SUCCESS && reg_val[1] == ISP_FLASH_SUCCESS) {
			...
			if (r >= 0 && reg_val[0] == ISP_FLASH_SUCCESS
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_flash_subsystem(struct i2c_client *client,
				  struct fw_subsys_info *subsys)
{
	...
	fw_packet = tp_devm_kzalloc(&client->dev, ISP_MAX_BUFFERSIZE + 6, GFP_KERNEL);
	...
	while (total_size > 0) {
		...
		TPD_INFO("Flash firmware to %08x,size:%u bytes", subsys_base_addr + offset,
			 data_size);

		/* format one firmware packet */
		r = goodix_format_fw_packet(fw_packet, subsys_base_addr + offset, data_size,
					    &subsys->data[offset]);
...
exit:
	tp_devm_kfree(&client->dev, (void **)&fw_packet, ISP_MAX_BUFFERSIZE + 6);
	return r;
}
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_flash_firmware(struct i2c_client *client,
				 struct firmware_data *fw_data)
{
	...
	for (i = 1; i < fw_num && retry;) {
		...
		if (r == 0) {
			...
			retry--;
			TPD_INFO("--- End flash subsystem%d: Fail, errno:%d, retry:%d ---", i, r,
				 GOODIX_BUS_RETRY_TIMES - retry);
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_update_finish(void *chip_data,
				struct fw_update_ctrl *fwu_ctrl)
{
	u8 reg_val[8] = {0};
	int r = 0;
...
static int gtx8_parse_cfg_data(const struct firmware *cfg_bin, char *cfg_type,
			       u8 *cfg, int *cfg_len, u8 sid)
{
	int i = 0, config_status = 0, one_cfg_count = 0;
	int cfg_package_len = 0;
	...
	cfg_package_len = getU32(cfg_bin->data) + BIN_CFG_START_LOCAL;

	if (cfg_package_len > cfg_bin->size) {
		TPD_INFO("%s:Bad firmware!,cfg package len:%d,firmware size:%d\n",
			 __func__, cfg_package_len, (int)cfg_bin->size);
		goto exit;
	}
	...
	for (i = BIN_CFG_START_LOCAL; i < (cfg_package_len); i++) {
	...
	for (i = 0; i < bin_group_num * bin_cfg_num; i++) {
		...
		one_cfg_count = getU16(&cfg_bin->data[CFG_HEAD_BYTES + 2 + i *
						      CFG_INFO_BLOCK_BYTES]);

		if (sid == (cfg_bin->data[CFG_HEAD_BYTES + i * CFG_INFO_BLOCK_BYTES])) {
			sid_is_exist = GTX8_EXIST;

			if (config_status == (cfg_bin->data[CFG_HEAD_BYTES + 1 + i *
							    CFG_INFO_BLOCK_BYTES])) {
...
static int gtx8_get_cfg_data(void *chip_data_info,
			     const struct firmware *cfg_bin,
			     char *config_name, struct gtx8_ts_config *config)
{
	...
	cfg_data = tp_devm_kzalloc(&chip_info->client->dev, GOODIX_CFG_MAX_SIZE,
				   GFP_KERNEL);
	...
	TPD_DEBUG("%s: %s  version:%d , size:%d\n", __func__, config_name, cfg_data[0],
		  cfg_len);
...
exit:
	tp_devm_kfree(&chip_info->client->dev, (void **)&cfg_data, GOODIX_CFG_MAX_SIZE);
	TPD_DEBUG("%s exit\n", __func__);
	return ret;
}


static int gtx8_get_cfg_parms(void *chip_data_info,
			      const struct firmware *firmware)
{
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data_info;
	int ret = 0;
	...
	ret = gtx8_get_cfg_data(chip_info, firmware, GTX8_NORMAL_CONFIG,
				&chip_info->normal_cfg);
	...
	ret = gtx8_get_cfg_data(chip_info, firmware, GTX8_TEST_CONFIG,
				&chip_info->test_cfg);
	...
	ret = gtx8_get_cfg_data(chip_info, firmware, GTX8_NORMAL_NOISE_CONFIG,
				&chip_info->normal_noise_cfg);
	...
	ret = gtx8_get_cfg_data(chip_info, firmware, GTX8_NOISE_TEST_CONFIG,
				&chip_info->noise_test_cfg);
...
/*      0: operate success*/
/*      other: failed*/
static int gtx8_get_fw_parms(void *chip_data_info,
			     const struct firmware *firmware, struct firmware *fw_firmware)
{
	/*struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data_info;*/
	int ret = 0;
	int cfg_package_len = 0;
	int fw_package_len = 0;
	...
	cfg_package_len = getU32(firmware->data) + BIN_CFG_START_LOCAL;
	TPD_DEBUG("%s cfg package len:%d\n", __func__, cfg_package_len);

	if (firmware->size <= (cfg_package_len + 16)) {
	...
	if (!(firmware->data[cfg_package_len + 0] == 'G'
			&& firmware->data[cfg_package_len + 1] == 'X' &&
			firmware->data[cfg_package_len + 2] == 'F'
			&& firmware->data[cfg_package_len + 3] == 'W')) {
		TPD_INFO("%s can't find fw package\n", __func__);
		TPD_INFO("Data type:%c %c %c %c,dest type is:GXFW\n",
			 firmware->data[cfg_package_len + 0],
			 firmware->data[cfg_package_len + 1], firmware->data[cfg_package_len + 2],
			 firmware->data[cfg_package_len + 3]);
		ret = -1;
		goto exit;
	}

	if (firmware->data[cfg_package_len + 4] != 1) {
		TPD_INFO("%s can't support this ver:%d\n", __func__,
			 firmware->data[cfg_package_len + 4]);
		ret = -1;
		goto exit;
	}

	fw_package_len =  getU32(firmware->data + cfg_package_len + 8);

	TPD_DEBUG("%s fw package len:%d\n", __func__, fw_package_len);

	if ((fw_package_len + 16 + cfg_package_len) > firmware->size) {
		TPD_INFO("%s bad firmware,need len:%d,actual firmware size:%d\n",
			 __func__, fw_package_len + 16 + cfg_package_len, (int)firmware->size);
		ret = -1;
		goto exit;
	}

	fw_firmware->size = fw_package_len;
	fw_firmware->data = firmware->data + cfg_package_len + 16;

	TPD_DEBUG("success get fw,len:%d\n", fw_package_len);
	TPD_DEBUG("fw head info:%*ph\n", 32, fw_firmware->data);
	TPD_DEBUG("fw tail info:%*ph\n", 4, &fw_firmware->data[fw_package_len - 4 - 1]);
	ret = 0;
...
#define FW_UPDATE_RETRY        2
	...
	fwu_ctrl = tp_devm_kzalloc(&chip_info->client->dev,
				   sizeof(struct fw_update_ctrl), GFP_KERNEL);
...
err_parse_fw:
	...
	tp_devm_kfree(&chip_info->client->dev, (void **)&fwu_ctrl,
		      sizeof(struct fw_update_ctrl));

	return r;
}
/*****************end of GT9886's update function********************/

static u32 goodix_trigger_reason(void *chip_data, int gesture_enable,
				 int is_suspended)
{
	int ret = -1;
	u8 touch_num = 0;
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_READ_COOR,  4 + BYTES_PER_COORD,
	...
	if ((chip_info->touch_data[0] & GOODIX_TOUCH_EVENT) == GOODIX_TOUCH_EVENT) {
		...
		if (chip_info->kernel_grip_support) {
			if (touch_num > 0) {
				ret = touch_i2c_read_block(chip_info->client,
							   chip_info->reg_info.GTP_REG_EDGE_INFO, BYTES_PER_EDGE * touch_num,
							   chip_info->edge_data);
		...
		if (touch_num > 1) {
			ret = touch_i2c_read_block(chip_info->client,
						   chip_info->reg_info.GTP_REG_READ_COOR + 2, \
						   BYTES_PER_COORD * touch_num + 2,
						   &chip_info->touch_data[2]); /*read out point data*/
	...
	} else if (unlikely(((chip_info->touch_data[0] & GOODIX_GESTURE_EVENT) !=
			     GOODIX_GESTURE_EVENT)
			    && ((chip_info->touch_data[0] &GOODIX_FINGER_IDLE_EVENT) !=
				GOODIX_FINGER_IDLE_EVENT))) {
		check_sum = 0;
	...
	if (gesture_enable && is_suspended
			&& (chip_info->touch_data[0] & GOODIX_GESTURE_EVENT) ==
			GOODIX_GESTURE_EVENT) {   /*check whether the gesture trigger*/
		return IRQ_GESTURE;
	...
	if (unlikely((chip_info->touch_data[0] & (GOODIX_TOUCH_EVENT |
			GOODIX_FINGER_IDLE_EVENT)) == GOODIX_FINGER_IDLE_EVENT)) {
		goto IGNORE_IDLETOACTIVE_IRQ;
	}

	if (unlikely((chip_info->touch_data[0] & GOODIX_REQUEST_EVENT) ==
			GOODIX_REQUEST_EVENT)) {     /*int request*/
		return IRQ_FW_CONFIG;
	}

	if (unlikely((chip_info->touch_data[0] & GOODIX_FINGER_STATUS_EVENT) ==
			GOODIX_FINGER_STATUS_EVENT)) {
	...
	if ((chip_info->touch_data[0] & GOODIX_TOUCH_EVENT) == GOODIX_TOUCH_EVENT) {
		...
		if ((chip_info->touch_data[0] & GOODIX_FINGER_PRINT_EVENT) ==
				GOODIX_FINGER_PRINT_EVENT &&
				!is_suspended && (chip_info->fp_down_flag == false)) {
			chip_info->fp_down_flag = true;
			SET_BIT(result_event, IRQ_FINGERPRINT);

		} else if (!is_suspended
				&& (chip_info->touch_data[0] & GOODIX_FINGER_PRINT_EVENT) !=
				GOODIX_FINGER_PRINT_EVENT &&
				(chip_info->fp_down_flag == true)) {
...
static int goodix_get_touch_points(void *chip_data, struct point_info *points,
				   int max_num)
{
	int ret, i;
	int touch_map = 0;
...
static int goodix_get_gesture_info(void *chip_data,
				   struct gesture_info *gesture)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_WAKEUP_GESTURE, sizeof(doze_buf), doze_buf);
	...
	if (gesture_id == 0
			|| ((doze_buf[0] & GOODIX_GESTURE_EVENT)  == 0)) {
		/*no gesture type, no need handle*/
	...
	case FP_DOWN_DETECT:
		...
		chip_info->fp_coor_report.fp_y_coor = (doze_buf[6] & 0xFF) |
						      (doze_buf[7] & 0x0F) << 8;
		chip_info->fp_coor_report.fp_area = (doze_buf[12] & 0xFF) |
						    (doze_buf[13] & 0x0F) << 8;
	...
	point_num = point_num < MAX_GESTURE_POINT_NUM ? point_num :
		    MAX_GESTURE_POINT_NUM;

	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_GESTURE_COOR, point_num * 4 + 1,
				   point_data); /*havn't add checksum here.*/
...
REPORT_GESTURE:
	TPD_INFO("%s: gesture_id = 0x%x, gesture_type = %d, clockWise = %d, point:(%d %d)(%d %d)(%d %d)(%d %d)(%d %d)(%d %d)\n",
		 ...
		 gesture->Point_end.y,
		 gesture->Point_1st.x, gesture->Point_1st.y, gesture->Point_2nd.x,
		 gesture->Point_2nd.y,
...
static void goodix_get_health_info(void *chip_data,
				   struct monitor_data *mon_data)
{
	...
	if (health_info->shield_freq != 0) {
		...
		if (tp_debug != 0) {
			TPD_INFO("%s: freq before: %d HZ, freq after: %d HZ\n", __func__,
				 health_info->freq_before, health_info->freq_after);
		}
	}
	...
	if (health_info->fw_rst != 0) {
		...
		case RST_MAIN_REG:
			tp_healthinfo_report(mon_data, HEALTH_REPORT, "RST_MAIN_REG");
			break;

		case RST_OVERLAY_ERROR:
			tp_healthinfo_report(mon_data, HEALTH_REPORT, "RST_OVERLAY_ERROR");
			break;

		case RST_LOAD_OVERLAY:
			tp_healthinfo_report(mon_data, HEALTH_REPORT, "RST_LOAD_OVERLAY");
			break;

		case RST_CHECK_PID:
			tp_healthinfo_report(mon_data, HEALTH_REPORT, "RST_CHECK_PID");
			break;

		case RST_CHECK_RAM:
			tp_healthinfo_report(mon_data, HEALTH_REPORT, "RST_CHECK_RAM");
			break;

		case RST_CHECK_RAWDATA:
			tp_healthinfo_report(mon_data, HEALTH_REPORT, "RST_CHECK_RAWDATA");
			break;
...
static int goodix_esd_handle(void *chip_data) /*Jarvis:have not finished*/
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_ESD_READ, 1, &esd_buf);
	...
	if ((ret < 0) || esd_buf == 0xAA) {
		...
		goodix_reset(
			chip_data); /* reset function have rewrite the esd reg no need to do it again*/

		tp_touch_btnkey_release(chip_info->tp_index);

		enable_irq(chip_info->client->irq);
		TPD_INFO("%s: Goodix esd reset over.", __func__);
		tp_healthinfo_report(monitor_data, HEALTH_REPORT, "esd_err_count");
		return -1;

	} else {
		esd_buf = 0xAA;
		ret = touch_i2c_write_block(chip_info->client,
					    chip_info->reg_info.GTP_REG_ESD_WRITE, 1, &esd_buf);
...
static void goodix_enable_fingerprint_underscreen(void *chip_data,
		uint32_t enable)
{
	int ret = 0;
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;
...
static void goodix_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	if (enable) {
		ret = goodix_send_cmd(chip_info, GTP_CMD_GESTURE_MASK,
				      1);/* enable all gesture */
...
static void goodix_register_info_read(void *chip_data, uint16_t register_addr,
				      uint8_t *result, uint8_t length)
{
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;

	gt9886_i2c_read(chip_info->client, register_addr, length,
			result);         /*read data*/
}
...
static int  goodix_smooth_lv_set(void *chip_data, int level)
{
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;

	TPD_DEBUG("set smooth lv to %d", level);

	return goodix_send_cmd(chip_info, GTP_CMD_SMOOTH_LEVEL, level);
}

static int  goodix_sensitive_lv_set(void *chip_data, int level)
{
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;

	TPD_DEBUG("set sensitive lv to %d", level);

	return goodix_send_cmd(chip_info, GTP_CMD_SENSITIVE_LEVEL, level);
}

static int goodix_set_high_frame_rate(void *chip_data, int level, int time)
{
	int ret = 0,high_frame_time = 0;
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;

	if(level)
		high_frame_time = time;

	TPD_INFO("set high_frame_rate to %d, keep %ds", level, high_frame_time);
	ret = goodix_send_cmd(chip_info, GTP_CMD_HIGH_FRAME_TIME, high_frame_time/5);
...
struct oplus_touchpanel_operations goodix_ops = {
	...
	.smooth_lv_set               = goodix_smooth_lv_set,
	.sensitive_lv_set            = goodix_sensitive_lv_set,
...
static void goodix_detailed_debug_info_read(struct seq_file *s, void *chip_data, debug_type debug_type)
{
	...
	goodix_config_info_read(s,chip_data);
	goodix_special_register_info_read(s,chip_data);

read_data_exit:
	goodix_send_cmd(chip_info, GTP_CMD_NORMAL, 0);
	chip_info->gt8x_rawdiff_mode = 0;
	gt9886_i2c_write(chip_info->client, chip_info->reg_info.GTP_REG_READ_COOR, 1, &clear_state);
	/*be normal in idle*/
...
static void goodix_debug_info_read(struct seq_file *s, void *chip_data,
				   debug_type debug_type)
{
	...
	kernel_buf = tp_devm_kzalloc(&chip_info->client->dev, tx_num * rx_num * 2 > PAGE_SIZE ? tx_num * rx_num  * 2 : PAGE_SIZE, GFP_KERNEL);
	...
	goodix_send_cmd(chip_info, GTP_CMD_RAWDATA, 0);
	msleep(20);
	gt9886_i2c_write(chip_info->client, chip_info->reg_info.GTP_REG_READ_COOR, 1,
			 &clear_state);

	/*wait for data ready*/
	while (i++ < 10) {
		ret = gt9886_i2c_read(chip_info->client, chip_info->reg_info.GTP_REG_READ_COOR,
				      1, kernel_buf);
		TPD_INFO("ret = %d  kernel_buf = %d\n", ret, kernel_buf[0]);
	...
	for (i = 0; i < rx_num; i++) {
		...
		for (j = 0; j < tx_num; j++) {
			data = (kernel_buf[j * rx_num * 2 + i * 2] << 8) + kernel_buf[j * rx_num * 2 + i
					* 2 + 1];
			seq_printf(s, "%4d ", data);
		}
...
read_data_exit:
	...
	/*enable doze mode*/
	goodix_set_i2c_doze_mode(chip_info->client, true);
	tp_devm_kfree(&chip_info->client->dev, (void **)&kernel_buf, PAGE_SIZE);
	return;
}
...
static void goodix_down_diff_info_read(struct seq_file *s, void *chip_data)
{
	...
	kernel_buf = tp_devm_kzalloc(&chip_info->client->dev, PAGE_SIZE, GFP_KERNEL);
	...
	ret = gt9886_i2c_read(chip_info->client,
			      chip_info->reg_info.GTP_REG_DOWN_DIFFDATA, 2, kernel_buf);
	...
	ret = gt9886_i2c_read(chip_info->client,
			      chip_info->reg_info.GTP_REG_DOWN_DIFFDATA, tx_num * rx_num * 2 + 2, kernel_buf);
	...
	for (i = 0; i < rx_num; i++) {
		...
		for (j = 0; j < tx_num; j++) {
			diff_data = (kernel_buf[j * rx_num * 2 + i * 2 + 2] << 8) + kernel_buf[j *
					rx_num * 2 + i * 2 + 1 + 2];
...
static void goodix_special_register_info_read(struct seq_file *s, void *chip_data)
{
    ...
    TPD_INFO("%s enter %d\n",__func__);
    seq_printf(s, "\ngoodix_special_register_info_read:\n");

    reg = 0x5254;
    ret = gt9886_i2c_read(chip_info->client, reg , 88,reg_buff);
    if (ret < 0) {
        TPD_INFO("%s failed to read 0x5254 data.\n", __func__);
        goto exit;
    }

    seq_printf(s, "\nreg =0x5254 data:\n");
	for(i= 0;i < 88;i++)
	    seq_printf(s, "%4d ", reg_buff[i]);

    reg = 0x45EC;
    ret = gt9886_i2c_read(chip_info->client, reg , 88,reg_buff);
    if (ret < 0) {
        TPD_INFO("%s failed to read 0x45EC data.\n", __func__);
        goto exit;
    }
    ...
    ret = gt9886_i2c_read(chip_info->client, reg , 88,reg_buff);
    if (ret < 0) {
        TPD_INFO("%s failed to read 0x56E3  data.\n", __func__);
        goto exit;
    }
...
{
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;
	TPD_INFO("%s detail_debug_info_support = %d\n",__func__,chip_info->detail_debug_info_support);
...
static void goodix_main_register_read(struct seq_file *s,
				      void *chip_data)/*Jarvis:need change to GT9886's register*/
{
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;
	int ret = 0;
	u8 *touch_data = NULL;
	...
	touch_data = tp_devm_kzalloc(&chip_info->client->dev, 4 + BYTES_PER_COORD,
				     GFP_KERNEL);
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_READ_COOR,  4 + BYTES_PER_COORD, touch_data);
	...
	seq_printf(s, "====================================================\n");
out:
	tp_devm_kfree(&chip_info->client->dev, (void **)&touch_data,
		      4 + BYTES_PER_COORD);
	return;
}
...
static int goodix_do_read_config(void *chip_data, u32 base_addr, u8 *buf)
{
	...
	if (checksum) {
		TPD_INFO("Config head checksum err:0x%x,data:%*ph", checksum, TS_CFG_HEAD_LEN,
			 buf);
		ret = -EINVAL;
		goto err_out;
	}
	...
	for (i = 0; i < sub_bags; i++) {
		...
		ret = gt9886_i2c_read(chip_info->client, base_addr + offset + 2, subbag_len + 1,
				      buf + offset + 2);
		...
		offset += subbag_len + 3;
		TPD_DEBUG("sub bag %d, data:%*ph", buf[offset], buf[offset + 1] + 3,
			  buf + offset);
	}
...
int goodix_read_config(void *chip_data, u8 *config_data, u32 config_len)
{
	...
	if (goodix_wait_cfg_cmd_ready(chip_info, COMMAND_READ_CFG_PREPARE_OK,
...
static void goodix_config_info_read(struct seq_file *s, void *chip_data)
{
	...
	gt9886_i2c_read(chip_info->client, chip_info->reg_info.GTP_REG_PRODUCT_VER, 72,
			temp_data);
	seq_printf(s, "ProductID: GT%c%c%c%c\n", temp_data[9], temp_data[10],
		   temp_data[11], temp_data[12]);
	seq_printf(s, "PatchID: %02X%02X%02X%02X\n", temp_data[17], temp_data[18],
		   temp_data[19], temp_data[20]);
...
*******************************************************************************/
static int gtx8_get_cfg_value(u8 *config, u8 *buf, u8 len, u8 sub_bag_num,
			      u8 offset)
{
	u8 *sub_bag_ptr = NULL;
	u8 i = 0;
...
static int init_test_config(void *chip_data)
{
	int ret = 0;
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;
	struct gtx8_ts_test *ts_test = chip_info->p_gtx8_ts_test;
...
static int gtx8_init_params(void *chip_data,
			    struct auto_testdata *goodix_testdata)
{
	int ret = 0;
	struct gtx8_ts_test *ts_test = NULL;
	struct ts_test_params *test_params = NULL;
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;

	TPD_INFO("%s run\n", __func__);
	ts_test = chip_info->p_gtx8_ts_test;
	...
	test_params->drv_num = goodix_testdata->tx_num;
	test_params->sen_num = goodix_testdata->rx_num;
...
/* init cmd data*/
static int gtx8_init_cmds(void *chip_data)
{
	int ret = 0;
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;
	struct gtx8_ts_test *ts_test = NULL;
 ...
 */
static int gtx8_cache_origconfig(void *chip_data)
{
	int ret = -ENODEV;
	u8 checksum = 0;
	struct gtx8_ts_test *ts_test = NULL;
...
/* gtx8_auto_test_preoperation
 *
 * preparation before tp test
 */
static int gtx8_auto_test_preoperation(struct seq_file *s,
				       void *chip_data,
				       struct auto_testdata *goodix_testdata,
				       struct test_item_info *p_test_item_info)
{
	...
	ts_test = tp_devm_kzalloc(&chip_info->client->dev, sizeof(struct gtx8_ts_test),
				  GFP_KERNEL);
	...
	chip_info->p_gtx8_ts_test = ts_test;

	TPD_INFO("TP test preparation\n");
	ret = gtx8_cache_origconfig(chip_data);
	...
	ret = gtx8_init_params(chip_info, goodix_testdata);
	...
	init_test_config(chip_info);

	ret = goodix_send_config(chip_info, chip_info->noise_test_cfg.data,
				 chip_info->noise_test_cfg.length);
 ...
 */
static int gtx8_cache_rawdata(void *chip_data)
{
	int j = 0;
	int i = 0;
	int ret = -EINVAL;
 ...
 */
static int gtx8_cache_self_rawdata(void *chip_data)
{
	int i = 0, j = 0;
	int ret = -EINVAL;
 ...
 */
static int gtx8_noisetest_prepare(void *chip_data,
				  struct auto_testdata *goodix_testdata)
{
	int ret = 0;
	...
	TPD_INFO("%s run\n", __func__);
	noise_data_size = goodix_testdata->tx_num * goodix_testdata->rx_num;
	self_noise_data_size = goodix_testdata->tx_num + goodix_testdata->rx_num;
 ...
 */
static int gtx8_cache_noisedata(void *chip_data)
{
	int ret = 0;
	int ret1 = 0;
 ...
 */
static int gtx8_analyse_noisedata(struct gtx8_ts_test *ts_test)
{
	int i = 0;
	int ret = 0;
	u32 find_bad_node = 0;
 ...
 */
static int gtx8_analyse_self_noisedata(struct gtx8_ts_test *ts_test)
{
	int i = 0;
	int ret = 0;
...
/* test noise data */
static int gtx8_noisedata_test(struct seq_file *s, void *chip_data,
			       struct auto_testdata *goodix_testdata, struct test_item_info *p_test_item_info)
{
	int ret = 0;
	int ret1 = 0;
	int i2c_ret = 0;
	...
	p_noisedata = tp_devm_kzalloc(&chip_info->client->dev,
				      sizeof(u16) * (goodix_testdata->tx_num * goodix_testdata->rx_num), GFP_KERNEL);
	...
	p_self_noisedata = tp_devm_kzalloc(&chip_info->client->dev,
					   sizeof(u16) * (goodix_testdata->tx_num + goodix_testdata->rx_num), GFP_KERNEL);
	...
	ts_test->noisedata.data = p_noisedata;
	ts_test->self_noisedata.data = p_self_noisedata;

	if (p_test_item_info->para_num) {
		if (p_test_item_info->p_buffer[0]) {
			ts_test->is_item_support[TYPE_TEST1] = p_test_item_info->p_buffer[0];
			test_params->noise_threshold = p_test_item_info->p_buffer[1];
			test_params->self_noise_threshold = p_test_item_info->p_buffer[2];
			TPD_INFO("noise_threshold:%d,self_noise_threshold:%d\n",
				 test_params->noise_threshold, test_params->self_noise_threshold);
		}
	}

	ret = gtx8_noisetest_prepare(chip_info, goodix_testdata);
	...
	for (test_cnt = 0; test_cnt < RAWDATA_TEST_TIMES; test_cnt++) {
		ret = gtx8_cache_noisedata(chip_info);
		...
		ret = gtx8_analyse_noisedata(ts_test);

		ret1 = gtx8_analyse_self_noisedata(ts_test);
		break;
	}
...
noise_test_exit:
	...
	i2c_ret = touch_i2c_write_block(chip_info->client, ts_test->normal_cmd.cmd_reg,
					ts_test->normal_cmd.length, ts_test->normal_cmd.cmds);

	buf[0] = 0x00;
	i2c_ret = touch_i2c_write_block(chip_info->client, GTP_REG_COOR, 1, buf);
	return ret | ret1;
}
 ...
 */
static int gtx8_captest_prepare(void *chip_data)
{
	int ret = -EINVAL, result = 0;
	char temp_data[TS_CFG_MAX_LEN] = {0};
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;

	TPD_INFO("%s run\n", __func__);
	ret = goodix_send_config(chip_info, chip_info->test_cfg.data,
				 chip_info->test_cfg.length);
	...
	result = goodix_read_config(chip_info, temp_data, 0);
	if (result) {
		TPD_INFO("Failed to read test config:%d\n", result);
	} else {
		TPD_INFO("read test config ver:%x cfg.data: %x\n",  temp_data[0],  chip_info->test_cfg.data[0]);
		if (temp_data[0] != chip_info->test_cfg.data[0]) {
			ret = goodix_send_config(chip_info, chip_info->test_cfg.data,
					 chip_info->test_cfg.length);
			if (ret) {
				TPD_INFO("Failed to send test config again :%d\n", ret);
			} else {
				TPD_INFO("Success send test config again");
			}
	    }
	}
	TPD_INFO("%s exit:%d\n", __func__, ret);
...
static int gtx8_deltacapacitance(struct seq_file *s, void *chip_data,
				 ...
				 struct test_item_info *p_test_item_info)
{
	...
	if (p_test_item_info->para_num) {
		if (p_test_item_info->p_buffer[0]) {
			ts_test->is_item_support[TYPE_TEST3] = p_test_item_info->p_buffer[0];

			if (p_test_item_info->item_limit_type == LIMIT_TYPE_TX_RX_DATA) {
				test_params->deviation_limits = (uint32_t *)(p_testdata->fw->data +
 ...
 */
static int gtx8_rawdata_test(void *chip_data)
{
	int ret = 0;
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;
	struct gtx8_ts_test *ts_test = NULL;
	...
	ret = gtx8_cache_rawdata(chip_info);
...
static int gtx8_capacitance_test(struct seq_file *s, void *chip_data,
				 struct auto_testdata *p_testdata,
				 struct test_item_info *p_test_item_info)
{
	int ret = 0;
	...
	p_rawdata = tp_devm_kzalloc(&chip_info->client->dev,
				    sizeof(u16) * (p_testdata->tx_num * p_testdata->rx_num), GFP_KERNEL);
	...
	ts_test->rawdata.data = p_rawdata;

	if (p_test_item_info->para_num) {
		if (p_test_item_info->p_buffer[0]) {
			ts_test->is_item_support[TYPE_TEST2] = p_test_item_info->p_buffer[0];

			if (p_test_item_info->item_limit_type == LIMIT_TYPE_TX_RX_DATA) {
				test_params->max_limits = (uint32_t *)(p_testdata->fw->data +
								       p_test_item_info->top_limit_offset);
				test_params->min_limits = (uint32_t *)(p_testdata->fw->data +
	...
	ret = gtx8_captest_prepare(chip_info);
	...
	ret = gtx8_rawdata_test(chip_info);
	if (ret < 0) {
		TPD_INFO("%s:Failed gtx8_rawdata_test:ret%d\n", __func__, ret);
                goto capac_test_exit;

	} else {
		TPD_INFO("%s: Success gtx8_rawdata_test\n", __func__);
	}
...
static int gtx8_self_rawcapacitance(struct seq_file *s, void *chip_data,
				    struct auto_testdata *p_testdata,
				    struct test_item_info *p_test_item_info)
{
	struct chip_data_gt9886 *chip_info = (struct chip_data_gt9886 *)chip_data;
	...
	p_self_rawdata = tp_devm_kzalloc(&chip_info->client->dev,
					 sizeof(u16) * (p_testdata->tx_num + p_testdata->rx_num), GFP_KERNEL);
	...
	ts_test->self_rawdata.data = p_self_rawdata;

	if (p_test_item_info->para_num) {
		if (p_test_item_info->p_buffer[0]) {
			ts_test->is_item_support[TYPE_TEST4] = p_test_item_info->p_buffer[0];

			if (p_test_item_info->item_limit_type == LIMIT_TYPE_SLEF_TX_RX_DATA) {
				TPD_INFO("get IMIT_TYPE_SLEFRAW_DATA data\n");
				test_params->self_max_limits = (int32_t *)(p_testdata->fw->data +
							       p_test_item_info->top_limit_offset);
				test_params->self_min_limits = (int32_t *)(p_testdata->fw->data +
							       p_test_item_info->floor_limit_offset);
	...
	ret = gtx8_cache_self_rawdata(chip_info);
...
/* short test */
static int gtx8_short_test_prepare(void *chip_data)
{
				      ...
				      struct short_record *r_data, u16 self_capdata, u8 flag)
{
	...
	if (flag == 0) {
		if (r_data->group1 != r_data->group2) {    /* different Group */
			...
			DieNumber1 = ((r_data->master & 0x80) == 0x80) ? (r_data->master + lineSenNum) :
				     r_data->master;
			DieNumber2 = ((r_data->slave & 0x80) == 0x80) ? (r_data->slave + lineSenNum) :
				     r_data->slave;
			DieNumber1 = (DieNumber1 >= DieNumber2) ? (DieNumber1 - DieNumber2) :
...
static int gtx8_shortcircut_analysis(void *chip_data)
{
	int ret = 0, err = 0;
	...
	data_buf = tp_devm_kzalloc(&chip_info->client->dev,
				   (MAX_DRV_NUM + MAX_SEN_NUM) * 2, GFP_KERNEL);
	...
	/* read self-capdata+ */
	size = (MAX_DRV_NUM + MAX_SEN_NUM) * 2;
	ret = touch_i2c_read_block(chip_info->client, DRV_SELF_CODE_REG, size,
				   data_buf);
	...
	*/
	ret = touch_i2c_read_block(chip_info->client, TX_SHORT_NUM, 3,
				   &short_status[0]);
	...
	tp_devm_kfree(&chip_info->client->dev, (void **)&data_buf,
		      (MAX_DRV_NUM + MAX_SEN_NUM) * 2);
	TPD_INFO("%s exit:%d\n", __func__, err);
...
static int gtx8_shortcircut_test(struct seq_file *s, void *chip_data,
				 struct auto_testdata *goodix_testdata,
				 struct test_item_info *p_test_item_info)
{
	int i = 0;
	...
	if (p_test_item_info->para_num) {
		if (p_test_item_info->p_buffer[0]) {
			ts_test->is_item_support[TYPE_TEST5] = p_test_item_info->p_buffer[0];
	...
	n_num = test_params->drv_num;
	m_num = test_params->sen_num;
	wait_time = ((n_num * (n_num + 1)) + (m_num * (m_num + 1))) / 2 + m_num * n_num + n_num + m_num;
	wait_time = (int)(86 * wait_time / 100 + 100);
	TPD_INFO("wait_time:%d(ms),tx:%d,rx:%d\n", wait_time, test_params->drv_num,
		 test_params->sen_num);

	ts_test->test_result[GTP_SHORT_TEST] = GTP_TEST_PASS;
	ret = gtx8_short_test_prepare(chip_info);
	...
	if (i < 150) {
		ret = gtx8_shortcircut_analysis(chip_info);
...
static void gtx8_put_test_result(struct seq_file *s, void *chip_data,
				 struct auto_testdata *p_testdata,
				 struct test_item_info *p_test_item_info)
{
	uint8_t  data_buf[64];
...
/* auto_test_endoperation
 *
 * finish test
 */
static int gtx8_auto_test_endoperation(struct seq_file *s, void *chip_data,
				       struct auto_testdata *p_testdata,
				       struct test_item_info *p_test_item_info)
{
	int ret = RESULT_ERR;
	...
	ret = goodix_send_config(chip_info, ts_test->orig_config.data,
				 ts_test->orig_config.length);
	...
	cfg_ver = goodix_config_version_read(chip_info);
	p_testdata->dev_tp_fw = cfg_ver | (fw_ver & 0xFF) << 8;
	gtx8_put_test_result(s, chip_data, p_testdata, p_test_item_info);

	tp_devm_kfree(&chip_info->client->dev,
		      (void **)&ts_test->rawdata.data,
		      sizeof(u16) * (p_testdata->tx_num * p_testdata->rx_num));
	tp_devm_kfree(&chip_info->client->dev,
		      (void **)&ts_test->noisedata.data,
		      sizeof(u16) * (p_testdata->tx_num * p_testdata->rx_num));
	tp_devm_kfree(&chip_info->client->dev,
		      (void **)&ts_test->self_noisedata.data,
		      sizeof(u16) * (p_testdata->tx_num + p_testdata->rx_num));
	tp_devm_kfree(&chip_info->client->dev,
		      (void **)&ts_test->self_rawdata.data,
		      sizeof(u16) * (p_testdata->tx_num + p_testdata->rx_num));
	tp_devm_kfree(&chip_info->client->dev, (void **)&ts_test,
...
static struct goodix_auto_test_operations goodix_test_ops = {
	.auto_test_preoperation = gtx8_auto_test_preoperation,
	.test1 = gtx8_noisedata_test,
	.test2 = gtx8_capacitance_test,
	.test3 = gtx8_deltacapacitance,
	.test4 = gtx8_self_rawcapacitance,
	.test5 = gtx8_shortcircut_test,
	.auto_test_endoperation = gtx8_auto_test_endoperation,
};
...
static int goodix_tp_probe(struct i2c_client *client,
			   const struct i2c_device_id *id)
{
	...
	/* 1. Alloc chip_info */
	chip_info = devm_kzalloc(&client->dev, sizeof(struct chip_data_gt9886),
				 GFP_KERNEL);
	...
	chip_info->touch_data = devm_kzalloc(&client->dev, MAX_GT_IRQ_DATA_LENGTH,
					     GFP_KERNEL);
	...
	chip_info->edge_data = devm_kzalloc(&client->dev, MAX_GT_EDGE_DATA_LENGTH,
					    GFP_KERNEL);
	...
	chip_info->doze_mode_set_count = 0;
	mutex_init(&chip_info->doze_mode_lock);
	mutex_init(&chip_info->cmd_mutex);
	mutex_init(&chip_info->config_lock);
	...
	gtx8_init_tool_node(ts, &chip_info->gt8x_rawdiff_mode);
...
static void goodix_tp_shutdown(struct i2c_client *client)
{
	struct touchpanel_data *ts = i2c_get_clientdata(client);
...
int __init tp_driver_init_gt9886(void)
{
	TPD_INFO("%s is called\n", __func__);
...
void __exit tp_driver_exit_gt9886(void)
{
...
late_initcall(tp_driver_init_gt9886);
#else
module_init(tp_driver_init_gt9886);
#endif
module_exit(tp_driver_exit_gt9886);

MODULE_DESCRIPTION("Touchscreen GT9886 Driver");
MODULE_LICENSE("GPL");
