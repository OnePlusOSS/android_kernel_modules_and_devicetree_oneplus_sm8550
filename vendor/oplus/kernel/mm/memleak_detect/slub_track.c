...
#define ALL_KMALLOC_HIGH (KMALLOC_SHIFT_HIGH * NR_KMALLOC_TYPES + 1)
...
#define KMALLOC_DEBUG_DMA_NAME(sz)     .name[KMALLOC_DMA] = "dma-kmalloc-debug-" #sz,
#else
#define KMALLOC_DEBUG_DMA_NAME(sz)
#endif

#ifdef CONFIG_MEMCG_KMEM
#define KMALLOC_DEBUG_CGROUP_NAME(sz)  .name[KMALLOC_CGROUP] = "kmalloc-cg-debug-" #sz,
#else
#define KMALLOC_DEBUG_CGROUP_NAME(sz)
#endif
...
{								\
	...
	KMALLOC_DEBUG_CGROUP_NAME(__short_size)                 \
	KMALLOC_DEBUG_DMA_NAME(__short_size)                            \
	.size = __size,						\
}

#define get_track_hash(track) ((u32)((track)->when))
#define set_track_hash(track, hash) ((track)->when = hash)

/* to make consistent with slab.h */
struct static_key_false slub_debug_enabled = { .key = STATIC_KEY_INIT_TRUE,  };

/* to make consistent with memcontrol.h */
struct static_key_false memcg_kmem_enabled_key = { .key = STATIC_KEY_INIT_TRUE,  };
...
static void save_track_hash_hook(void *data, bool alloc, struct track *p)
{
		...
		unsigned long flags, enum kmalloc_cache_type kmalloc_type)
{
	...
	} else if (IS_ENABLED(CONFIG_MEMCG_KMEM) && (kmalloc_type == KMALLOC_CGROUP)) {
		/* equal to if (cgroup_memory_nokmem) */
		if (!memcg_kmem_enabled())
			return (struct kmem_cache *)atomic64_read(
					&kmalloc_debug_caches[KMALLOC_NORMAL][index]);
		flags |= SLAB_ACCOUNT;
	}
		...
		char __user *buffer, size_t count, loff_t *off)
{
	...
	for (i = 0; i <= KMALLOC_SHIFT_HIGH; i++) {
		for(type = KMALLOC_NORMAL; type < NR_KMALLOC_TYPES; type++) {
			...
			if (IS_ENABLED(CONFIG_MEMCG_KMEM) && (type == KMALLOC_CGROUP)) {
				/* equal to if (cgroup_memory_nokmem) */
				if (!memcg_kmem_enabled())
					continue;
			}
...
#ifdef CONFIG_MEMCG_KMEM
	if (kmalloc_caches[KMALLOC_NORMAL][KMALLOC_SHIFT_LOW]
			== kmalloc_caches[KMALLOC_CGROUP][KMALLOC_SHIFT_LOW])
		static_branch_disable(&memcg_kmem_enabled_key);
#endif
