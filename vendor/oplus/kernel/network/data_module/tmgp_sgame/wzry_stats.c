...
#define SKB_MARK_MASK 0x80000000
...
static int s_get_intvl = 5; /* unit:s */
...
static void wzry_record_delay(struct sk_buff *skb, enum wzry_event_type type)
{
	...
	case WZRY_ENTRY_RX_DEV:
		if (!s_down_calc) {
			...
			skb->mark |= SKB_MARK_MASK;
			logt("WZRY_ENTRY_RX_DEV enter");
		}
		break;
	case WZRY_ENTRY_RX_IP:
		if (skb->mark & SKB_MARK_MASK) {
			save->rx_ip_time = cur_time;
			logt("WZRY_ENTRY_RX_IP enter");
		}
		break;
	case WZRY_ENTRY_RX_UDP:
		if (skb->mark & SKB_MARK_MASK) {
			save->rx_udp_time = cur_time;
			logt("WZRY_ENTRY_RX_UDP enter");
		}
		break;
	case WZRY_ENTRY_RX_USER:
		if (skb->mark & SKB_MARK_MASK) {
			save->rx_user_time = cur_time;
			logt("WZRY_ENTRY_RX_USER enter");
		}
		save->down_packets += 1;
	...
	case WZRY_ENTRY_TX_UDP:
		if (skb->mark & SKB_MARK_MASK) {
			...
			logt("WZRY_ENTRY_TX_UDP enter");
		}
		break;
	case WZRY_ENTRY_TX_IP:
		if (skb->mark & SKB_MARK_MASK) {
			save->tx_ip_time = cur_time;
			logt("WZRY_ENTRY_TX_IP enter");
		}
		break;
...
static void probe_net_dev_xmit(void *data, struct sk_buff *skb, int rc, struct net_device *dev, unsigned int skb_len)
{
...
static int probe_func_init(void)
{
	...
	ret = register_trace_net_dev_xmit(probe_net_dev_xmit, NULL);
	logt("register_trace_net_dev_xmit return %d", ret);
	if (ret) {
...
static void probe_func_deinit(void)
{
	...
	ret = unregister_trace_net_dev_xmit(probe_net_dev_xmit, NULL);
	logt("unregister_trace_net_dev_xmit return %d", ret);
#endif
}
...
static void wzry_delay_timer_function(struct timer_list *t)
{
	...
	item->timestamp = get_time_ts64_ns();
	...
	logt("copy item data %llu,%llu,%llu,%llu,%llu,%llu,%llu,%u,%u", item->timestamp, item->rx_ip_delay, item->rx_udp_delay, item->rx_user_delay,
