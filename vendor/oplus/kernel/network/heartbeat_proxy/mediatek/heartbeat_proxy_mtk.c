...
static int hba_check_seq_and_reinject(struct hba_st *hba, hba_seq_sync_msg_struct seq_sync)
{
	...
	if (seq_sync.tcp_snd_nxt == tcp_snd_nxt && seq_sync.tcp_rcv_nxt == tcp_rcv_nxt) {
		for (i = 0; i < HBA_SKB_QUEUE_LEN; i++) {
			if (hba->skb_queue[i]) {
				...
				if (hba->skb_queue[i]->protocol != htons(ETH_P_IP) || (!iph) || iph->protocol != IPPROTO_TCP || !tcph) {
					...
					if (hba->ip_local_deliver_finish) {
						ret = hba->ip_local_deliver_finish(net, sk, hba->skb_queue[i]);
						printk("[heartbeat_proxy_mtk] hba_check_seq_and_reinject proxy_key=%s id=%d index=%d reinject=%d.\n",
						       hba->proxy_key, iph->id, i, ret);
					} else {
						kfree_skb(hba->skb_queue[i]);
						printk("[heartbeat_proxy_mtk] hba_check_seq_and_reinject reinject failed, proxy_key=%s id=%d index=%d, fun is invalid!\n",
						       hba->proxy_key, iph->id, i);
					}
				}
			}
			hba->skb_queue[i] = NULL;
		...
		printk("[heartbeat_proxy_mtk] hba_check_seq_and_reinject failed, proxy_key=%s, seq not match!\n", hba->proxy_key);
		goto error_with_free;
	}
...
static int hba_netlink_nlmsg_handle(struct sk_buff *skb, struct genl_info *info)
{
	...
	case NETLINK_REQUEST_HBA_ESTABLISH:
		...
		ret = request_hba_establish(nla);
		break;
	case NETLINK_REQUEST_HBA_PAUSE:
		ret = request_hba_pause(nla);
		break;
	case NETLINK_REQUEST_HBA_RESUME:
		ret = request_hba_resume(nla);
		break;
	case NETLINK_REQUEST_HBA_STOP:
		ret = request_hba_stop(nla);
		break;
	case NETLINK_REQUEST_HBA_SEND_NOW:
		ret = request_hba_send_now(nla);
		break;
	case NETLINK_REQUEST_HBA_DELETE:
		ret = request_hba_delete(nla);
		break;
	default:
		printk("[heartbeat_proxy_mtk] hba_netlink_nlmsg_handle failed, unknown nla type=%d.\n", nla->nla_type);
		ret = -EINVAL;
		break;
	}
