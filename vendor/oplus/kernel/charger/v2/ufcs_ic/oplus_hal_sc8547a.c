...
static void sc8547_i2c_error(struct oplus_voocphy_manager *voocphy, bool happen)
{
...
static s32 sc8547_read_word(struct i2c_client *client, u8 reg)
{
	...
	if (ret < 0) {
		sc8547_i2c_error(voocphy, true);
		chg_err("i2c read word fail: can't read reg:0x%02X \n", reg);
		mutex_unlock(&chip->i2c_rw_lock);
		return ret;
	}
	sc8547_i2c_error(voocphy, false);
	mutex_unlock(&chip->i2c_rw_lock);
	return ret;
}
...
static int sc8547_svooc_hw_setting(struct sc8547a_device *chip)
{
	...
	sc8547_write_byte(chip->client, SC8547_REG_09, 0x10); /*WD:1000ms TODO:disable*/
...
static int sc8547_vooc_hw_setting(struct sc8547a_device *chip)
{
	...
	sc8547_write_byte(chip->client, SC8547_REG_09, 0x90); /*WD:1000ms TODO:disable*/
...
static int sc8547a_ufcs_hard_reset(struct ufcs_dev *ufcs)
{
	struct sc8547a_device *chip = ufcs->drv_data;
	int rc;
	int retry_count = 0;
...
static struct ufcs_dev_ops ufcs_ops = {
	...
	.source_hard_reset = sc8547a_ufcs_hard_reset,
...
static int sc8547a_parse_dt(struct sc8547a_device *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,ovp_reg", &chip->ovp_reg);
	if (rc)
		chip->ovp_reg = 0xe;
	chg_err("ovp_reg=0x%2x\n", chip->ovp_reg);

	rc = of_property_read_u32(node, "oplus,ocp_reg", &chip->ocp_reg);
	if (rc)
		chip->ocp_reg = 0x8;
