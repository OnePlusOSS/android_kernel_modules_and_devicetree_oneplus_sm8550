...
{
	int index = 0;
	struct oplus_chg_track_status temp_track_status = { 0 };
...
int oplus_chg_track_upload_mmi_chg_info(struct oplus_chg_track *chip)
{
	int index = 0;
	union mms_msg_data data = { 0 };
	int rc = 0;
...
int oplus_chg_track_upload_slow_chg_info(struct oplus_chg_track *chip)
{
	int index = 0;
	union mms_msg_data data = { 0 };
	int rc = 0;
	...
	if (track_status->curr_soc == -EINVAL) {
		...
		if (abs(track_status->curr_uisoc - judge_curr_soc) >OPLUS_CHG_TRACK_UI_SOC_LOAD_JUMP_THD) {
			...
			ret = snprintf(g_track_chip->uisoc_load_trigger.crux_info, OPLUS_CHG_TRACK_CURX_INFO_LEN,
				       ...
				       curr_fcc, curr_rm, monitor->ibat_ma);
