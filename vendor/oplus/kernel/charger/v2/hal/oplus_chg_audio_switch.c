...
enum TYPEC_AUDIO_SWITCH_STATE {
	...
	TYPEC_AUDIO_SWITCH_STATE_SUPPORT	= 0x1 << 3,
...
__maybe_unused void lock_audio_switch(void)
{
	mutex_lock(&g_audio_switch.audio_switch_lock);
}

__maybe_unused void unlock_audio_switch(void)
{
...
__maybe_unused static bool oplus_get_audio_switch_callback(state)
{
	lock_audio_switch();
	g_audio_switch.audio_switch_state = state;
