...
int ufcs_msg_handler(struct ufcs_dev *ufcs)
{
	...
	queue_work(class->wq, &class->recv_work);
...
EXPORT_SYMBOL(ufcs_config_watchdog);
...
void ufcs_clr_error_flag(struct ufcs_dev *ufcs)
{
	...
	memset(&ufcs->err_info, 0, sizeof(struct ufcs_error_info));
}
...
EXPORT_SYMBOL(ufcs_get_device_info);
...
EXPORT_SYMBOL(ufcs_get_error_info);
...
EXPORT_SYMBOL(ufcs_get_source_info);
...
EXPORT_SYMBOL(ufcs_get_cable_info);
...
EXPORT_SYMBOL(ufcs_get_pdo_info);
...
int ufcs_verify_adapter(struct ufcs_dev *ufcs, u8 key_index, u8 *auth_data, u8 data_len)
{
	...
	rc = READ_ONCE(class->state.err);
	if (rc < 0) {
		ufcs_err("adapter verify fail, rc=%d\n", rc);
		return rc;
	}

	return 0;
}
EXPORT_SYMBOL(ufcs_verify_adapter);
...
EXPORT_SYMBOL(ufcs_get_power_change_info);
...
EXPORT_SYMBOL(ufcs_get_emark_info);
...
EXPORT_SYMBOL(ufcs_get_power_info_ext);
