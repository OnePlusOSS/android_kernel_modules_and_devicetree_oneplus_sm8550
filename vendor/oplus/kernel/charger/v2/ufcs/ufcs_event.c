...
int ufcs_send_msg(struct ufcs_class *class, struct ufcs_msg *msg)
{
	...
	queue_work(class->wq, &class->msg_send_work);
	wait_for_completion(&sender->ack);
	cancel_work_sync(&class->msg_send_work);
	sender->msg = NULL;
...
{
	struct ufcs_receive_error *rcv_err = &class->ufcs->err_info.rcv_error;
	struct ufcs_hardware_error *hw_err = &class->ufcs->err_info.hd_error;
	struct ufcs_communication_error *comm_err = &class->ufcs->err_info.commu_error;
	...
	if (hw_err->hardreset) {
		if (class->start_cable_detect) {
			ufcs_send_state(class, UFCS_NOTIFY_CABLE_HW_RESET);
			ufcs_err("cable hard reset\n");
		} else {
			class->handshake_success = false;
			ufcs_send_state(class, UFCS_NOTIFY_SOURCE_HW_RESET);
...
static void ufcs_recv_work(struct work_struct *work)
{
	...
	class->recv_msg = msg;
	queue_work(class->wq, &class->event_work);
}
...
static void ufcs_event_work(struct work_struct *work)
{
...
static void ufcs_msg_send_work(struct work_struct *work)
{
...
int ufcs_event_init(struct ufcs_class *class)
{
	...
	INIT_WORK(&class->event_work, ufcs_event_work);
	INIT_WORK(&class->msg_send_work, ufcs_msg_send_work);
	INIT_WORK(&class->recv_work, ufcs_recv_work);
	class->recv_msg = NULL;
