	...
	struct oplus_param_head *param_head, struct oplus_comm_config *config)
{
	...
	while (data_head != NULL) {
		...
		config->reserve_chg_soc = le32_to_cpu(buf);
		chg_info("[TEST]:reserve_chg_soc = %d\n", config->reserve_chg_soc);
		break;
	}

	data_head = oplus_cfg_find_param_by_name(param_head, "oplus,reserve_dis_soc");
	while (data_head != NULL) {
		rc = oplus_cfg_get_data(data_head, (u8 *)&buf, sizeof(buf));
		if (rc < 0) {
			chg_err("get oplus,reserve_dis_soc data error, rc=%d\n", rc);
			break;
		}
		config->reserve_dis_soc = le32_to_cpu(buf);
		chg_info("[TEST]:reserve_dis_soc = %d\n", config->reserve_dis_soc);
		break;
	}
}
