...
static int oplus_pps_action_status_start(struct oplus_pps_chip *chip)
{
	...
	if (chip->cp_mode == PPS_SC_MODE) {
		chip->target_charger_volt =
			((vbat * 2 * batt_num) / 100) * 100 + PPS_ACTION_START_DIFF_VOLT_V1 * batt_num;
		if (chip->pps_adapter_type == PPS_ADAPTER_THIRD)
			chip->target_charger_current = PPS_ACTION_3RD_CURR_MIN;
		else
...
static int oplus_3rd_pps_action_volt_change(struct oplus_pps_chip *chip)
{
	...
	batt_num = oplus_chg_get_batt_num();
	charger_volt_min = ((vbat * 2 * batt_num) / 100) * 100 + PPS_ACTION_START_DIFF_VOLT_V1 * batt_num;
...
static void oplus_pps_stop_work(struct work_struct *work)
{
	...
	if (chip->pps_adapter_type == PPS_ADAPTER_THIRD && chip->pps_stop_status == PPS_STOP_VOTER_CHOOSE_CURVE) {
		pps_err("switch curve because of temp change.");
		/* decrease voltage to avoid disconnect after disabled CP */
		batt_num = oplus_chg_get_batt_num();
		chip->ask_charger_volt =
			(chip->data.ap_batt_volt * 2 * batt_num / 100) * 100 + PPS_ACTION_START_DIFF_VOLT_V1 * batt_num;
		oplus_pps_set_pdo(chip);
	}
