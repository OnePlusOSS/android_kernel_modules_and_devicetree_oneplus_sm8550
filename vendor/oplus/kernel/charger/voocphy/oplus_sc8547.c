...
irqreturn_t sc8547_protect_interrupt_handler(
...
static int sc8547_parse_dt(struct oplus_voocphy_manager *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,pps_ocp_max",
	                          &chip->pps_ocp_max);
	if (rc) {
...
static int sc8547_cp_reg_reset(struct i2c_client *client)
{
	...
	chip->cp_work_mode = CP_WORKMODE_INVALID;
...
static int sc8547_set_cp_enable(struct i2c_client *client, int enable)
{
	...
	if (ret < 0)
		...
		chip->cp_work_mode = enable ? CP_WORKMODE_PPS : CP_WORKMODE_INVALID;
...
static int sc8547_pps_check_and_register(struct oplus_voocphy_manager *chip)
{
	...
	if (index < CP_MAX && index >= CP_MASTER) {
		...
		sc8547_cp->dev_name = devm_kzalloc(chip->dev, PPS_CP_NAME_MAX, GFP_KERNEL);
		of_property_read_string(node, "oplus,pps_dev-name", &sc8547_cp->dev_name);
