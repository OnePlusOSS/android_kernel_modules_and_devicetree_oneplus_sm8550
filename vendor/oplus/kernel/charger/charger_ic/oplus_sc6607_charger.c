...
static int oplus_sc6607_enable_otg(void)
{
	...
	if (ret > 0) {
		...
		sc6607_set_watchdog_timer(g_chip, SC6607_WD_TIMEOUT_S(20000));
...
void oplus_chg_set_camera_on(bool val)
{
	...
	if (val) {
		if (chg_chip->charger_exist) {
			if (chg_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC) {
			} else if (chg_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
				oplus_sc6607_set_pd_config();
			} else {
				oplus_chg_set_flash_led_status(chg_chip->camera_on);
			}
		} else if (!chg_chip->charger_exist) {
			oplus_chg_set_flash_led_status(chg_chip->camera_on);
		}
	} else {
		if (chg_chip->charger_exist && POWER_SUPPLY_TYPE_USB_DCP == chg_chip->charger_type) {
			if (chg_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC) {
				pr_err("oplus_chg_enable_qc_detect\n");
			}
			if (chg_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
				pr_err("oplus_mt6360_pd_setup\n");
				oplus_sc6607_set_pd_config();
			}
		}
		oplus_chg_set_flash_led_status(chg_chip->camera_on);
