...
static int oplus_ufcs_parse_charge_strategy(struct oplus_ufcs_chip *chip)
{
	...
	chg_err("mos_switch_enable:%d\n", chip->mos_switch_enable);
	...
	ufcs_err(" ufcs_btb_limit[%d %d %d %d %d]\n", chip->btb_limit.limit_v0_diff, chip->btb_limit.limit_v1_diff,
		 chip->btb_limit.limit_v2_diff, chip->btb_limit.limit_v3_diff, chip->btb_limit.limit_v4_diff);
		...
		if (curve_volt_change) {
			for (index = chip->batt_curve_index; index < chip->batt_curves.batt_curve_num - 1; index++) {
				if ((chip->batt_curves.batt_curves[index - 1].target_vbus ==
					...
					ufcs_err("entry ibus =%d change batt_curve_index from %d to %d\n",
...
static void oplus_ufcs_circuit_current_check(struct oplus_ufcs_chip *chip)
{
	...
	if (chip->data.ap_batt_current > UFCS_CIRCUIT_PATH_CURR_MIN) {
		...
		ufcs_debug("[%d, %d]\n", chip->data.ap_batt_current, chip->ufcs_stop_status);
	}
...
static void oplus_ufcs_circuit_switch_check(struct oplus_ufcs_chip *chip)
{
	...
	if ((chip->ask_charger_current >= UFCS_MOS0_SWITCH_IBUS_ENABLE_MIN) &&
		...
		ufcs_err("[%d, %d]\n", ret_switch1, chip->ufcs_mos1_switch);
