...
static noinline int gen_insert_fatal_info(int type, u64 time)
{
	...
	info = (struct fatal_info*)kmalloc(sizeof(*info), GFP_KERNEL);
	if (!info)
		return -ENOMEM;
...
static noinline struct top_node *gen_top_node(int type, int grp_idx)
{
	...
	node = (struct top_node *)kmalloc(sizeof(*node), GFP_KERNEL);
	if (!node)
		return NULL;
...
static __always_inline void lk_contended(int type)
{
	...
	if (unlikely(IS_ERR_OR_NULL(ots))) {
		trace_printk("kern_lock_stat : error start, but ots == NULL !\n");
		return;
	}
...
static __always_inline void lk_acquired(int type)
{
	...
	if (unlikely(IS_ERR_OR_NULL(ots))) {
		trace_printk("kern_lock_stat : erro end, but ots == NULL !\n");
		return;
	}
	if (ots->lkinfo.waittime_stamp) {
		now = lockstat_clock();
