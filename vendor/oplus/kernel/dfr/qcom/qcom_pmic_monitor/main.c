			...
			struct kobj_attribute *attr, char *buf)
{
	...
	for (pmic_device_index = 0;pmic_device_index<8;pmic_device_index++) {
		...
		len += snprintf(&page[len], 256-len, "PMIC|%u|0x%08X%02X%02X\n",
				pmic_device_index,
...
static ssize_t poff_reason_gen3_show(struct kobject *kobj,
	...
	pmic_first_record = pmic_history_ptr->pmic_record[show_count];
	/* Fundamental Reset */
...
static int __init pmic_info_init(void)
{
	...
	if(PMIC_GEN3_INFO_MAGIC == *magic || PMIC_GEN3_NVMEM_MAGIC == *magic) {
