...
{
	if (g_init_watchdog_debug_enabled) {
		return false;
	}
	if(!strncmp(t->comm, "shortc_thread", TASK_COMM_LEN)||
		!strncmp(t->comm, "bcc_kthread", TASK_COMM_LEN)||
		!strncmp(t->comm, "usbtemp_kthread", TASK_COMM_LEN)||
		!strncmp(t->comm, "hang_detect", TASK_COMM_LEN)||
		!strncmp(t->comm, "wdtk-0", TASK_COMM_LEN)||
		!strncmp(t->comm, "wdtk-1", TASK_COMM_LEN)||
		!strncmp(t->comm, "wdtk-2", TASK_COMM_LEN)||
		!strncmp(t->comm, "wdtk-3", TASK_COMM_LEN)||
		!strncmp(t->comm, "wdtk-4", TASK_COMM_LEN)||
		!strncmp(t->comm, "wdtk-5", TASK_COMM_LEN)||
		!strncmp(t->comm, "wdtk-6", TASK_COMM_LEN)||
		!strncmp(t->comm, "wdtk-7", TASK_COMM_LEN)||
		!strncmp(t->comm, "disp_echk", TASK_COMM_LEN)||
		!strncmp(t->comm, "tee_irq_bh", TASK_COMM_LEN)||
		!strncmp(t->comm, "ccci_poll1", TASK_COMM_LEN)) {
		return true;
	}
	return false;
}
...
static inline bool
state_filter_match_ext(unsigned long state_filter, struct task_struct *p)
{
	unsigned int task_state;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	task_state = READ_ONCE(p->__state);
#else
	task_state = p->state;
#endif

	/* doesn't match */
	if (!(task_state & state_filter))
		return false;
	 ...
	 * TASK_KILLABLE).
	 */
	if (state_filter == TASK_UNINTERRUPTIBLE && task_state == TASK_IDLE)
		return false;
...
static void show_state_filter_ext(unsigned long state_filter)
{
	...
	for_each_process_thread(g, p) {
		...
		touch_nmi_watchdog();
#endif
		if (state_filter_match_ext(state_filter, p) && !is_ignore_process(p))
			sched_show_task(p);
	}
	rcu_read_unlock();
}
...
static void init_check_work_handler(struct work_struct *work)
{
	...
	if (!g_init_watchdog_debug_enabled && g_init_watchdog_data->state != STATE_CHECKED) {
		pr_debug(INIT_WATCHDOG_LOG_TAG "init check is not ready or already blocked or sleeping, just return\n");
		return;
	}

	/* if kicked, init process work normal, clear block count, exit to next loop */
	if (!g_init_watchdog_debug_enabled && g_init_watchdog_data->kick_state == STATE_KICKED) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
	if (!g_init_watchdog_debug_enabled && (taskp->__state == TASK_FROZEN)) {
#else
	if (!g_init_watchdog_debug_enabled && (taskp->flags & PF_FROZEN)) {
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	if (g_init_watchdog_debug_enabled || READ_ONCE(taskp->__state) == TASK_UNINTERRUPTIBLE) {
#else
	if (g_init_watchdog_debug_enabled || taskp->state == TASK_UNINTERRUPTIBLE) {
#endif
		...
		pr_debug(INIT_WATCHDOG_LOG_TAG "init process sleep for more than 30s\n");
