				...
				const phx_keyinfo *kinfo)
{
	...
	opfile = filp_open(keyinfo_file, O_CREAT | O_WRONLY | O_TRUNC,
			   FILE_MODE_0666);
	if (IS_ERR(opfile)) {
...
{
	PHX_KLOG_INFO(
		"Due to calling filp_open is forbidden in GKI, kernel log  "
		"will not be dumped here in driver");
#if FILP_OPEN_FUNCTION_CLOSE
