...
static int irqno_sort[IRQ_MAX_RECORD_NUM];
...
static unsigned int oplus_kstat_irqs(unsigned int irq)
{
	...
	if (!desc || !desc->kstat_irqs) {
                printk(KERN_ERR "desc or desc->kstat_irqs null!\n");
		return 0;
        }
...
static void update_irq_counter(void)
{
	...
	if (!o_irq_counter.irqs_delta || !o_irq_counter.irqs_last) {
                printk(KERN_ERR "irqs_delta or irqs_last null!\n");
		return;
        }
