...
#include <soc/oplus/boot/oplus_project_oldcdt.h>
    ...
    if(np){
        ...
        for (i = 0; i < FEATURE_COUNT; i++) {
            sprintf(feature_node, "nFeature%d", i);
                ret = of_property_read_u8_array(np, feature_node, tmp_feature, 4);
                local_ProjectInfoCDT.nDataBCDT.Feature[i] = tmp_feature[3];
                if (ret) {
                        pr_err("init_project_version_newcdt nFeature[%d]: %s %u fail",
	    ...
	    if(g_project->nDataBCDT.ProjectNo < 100000){
			pr_err("KE new cdt newcdt:%u nVersion:%u Project:%u, Dtsi:%u, Audio:%u, nRF:%u, PCB:%u eng_version:%u confidential:%u\n",
				newcdt,
				g_project->Version,
			...
			pr_err("Feature is: ");
			for (i = 0; i < FEATURE_COUNT; i++) {
				pr_err("[%d]:%u\t", i, g_project->nDataBCDT.Feature[i]);
			}
			pr_err("\n");
		}else{
			pr_err("KE new cdt newcdt:%u nVersion:%u Project:%X, Dtsi:%X, Audio:%X, nRF:%u, PCB:%u eng_version:%u confidential:%u\n",
				newcdt,
				g_project->Version,
				g_project->nDataBCDT.ProjectNo,
				g_project->nDataBCDT.DtsiNo,
