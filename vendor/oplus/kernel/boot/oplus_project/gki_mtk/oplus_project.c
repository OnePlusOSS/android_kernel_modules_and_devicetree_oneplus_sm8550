...
static void init_project_version(void)
{
	...
	if (np) {
		...
		if (g_project->nDataBCDT.ProjectNo < 100000) {
			pr_err("KE new cdt newcdt:%u nVersion:%u Project:%u, Dtsi:%u, Audio:%u, nRF:%u, "\
				"PCB:%u eng_version:%u confidential:%u\n",
				newcdt,
				g_project->Version,
			...
			pr_err("KE new cdt newcdt:%u nVersion:%u Project:%X, Dtsi:%X, Audio:%X, nRF:%u, "\
				"PCB:%u eng_version:%u confidential:%u\n",
				newcdt,
				g_project->Version,
				g_project->nDataBCDT.ProjectNo,
...
static int __init oplus_project_init(void)
{
	...
	pr_err("oplus_project_init start\n");
