...
static struct relation_event_limit g_relate[] = {
	{10001, 0, 2500, {10003, 10004, 10008, 10041, 10042, 10046, 10047}},
	{10003, 0, 1000, {10008, 10046, 0, 0, 0, 0, 0}},
	{10046, 0, 1000, {10008, 0, 0, 0, 0, 0, 0}}
};
...
static void calc_fid(unsigned char *str)
{
	...
	t = ktime_get();
	rdm = get_random_u64();
	snprintf(strHashSource, MAX_PAYLOAD_DATASIZE, "%lu %lld %s", rdm, t, str);
...
static void mm_fb_kevent_upload_jobs(struct work_struct *work)
{
	...
	list_for_each_entry_safe(kevent, n, &mm_kevent_list, head) {
		...
		if (OPLUS_MM_DIRVER_FB_EVENT_AUDIO == kevent->module) {
			...
			PAYLOAD("CT@@%d$$", kevent->count);
			PAYLOAD("FT@@%lld$$", ktime_to_ms(kevent->first) / 1000);
			PAYLOAD("ET@@%lld$$", ktime_to_ms(kevent->last) / 1000);
