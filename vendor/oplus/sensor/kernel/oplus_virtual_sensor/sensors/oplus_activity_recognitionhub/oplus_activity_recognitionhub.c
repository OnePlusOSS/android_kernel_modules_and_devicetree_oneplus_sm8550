...
static int oplus_activity_recognition_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	OPLUS_ACTIVITY_RECOGNITION_LOG("oplus_activity_recognition recv data, flush_action = %d, motion_count = %d, \
		motion_ith = %d, incar_state = %d, activity_mode = %d, delta_time = %d, timestamp = %lld\n",
		event->flush_action,
