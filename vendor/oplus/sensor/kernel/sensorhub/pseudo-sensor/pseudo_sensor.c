...
#if defined(CFG_OPLUS_ARCH_IS_QCOM)
#define SENS_PSEUDO_SENSOR_MEM_ID           490
...
static int qcom_get_smem(void)
{
	...
	cust_cmd[1] = SENS_PSEUDO_SENSOR_MEM_ID;
	...
	vmem = qcom_smem_get(g_data.client.host, SENS_PSEUDO_SENSOR_MEM_ID, &size);

	if (IS_ERR(vmem)) {
		ERR("qcom_smem_alloc failed, host: %d type: %d\n",
			g_data.client.host,
			SENS_PSEUDO_SENSOR_MEM_ID);
...
static int mtk_get_smem(void)
{
	...
	int mem_id = SENS_PSEUDO_SENSOR_MEM_ID;
