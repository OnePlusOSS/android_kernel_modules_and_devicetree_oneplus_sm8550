				...
				struct hdd_adapter *adapter)
{
	...
	nl_buf_len = NLMSG_HDRLEN;
	nl_buf_len +=
	    (sizeof(hdd_sta_ctx->cache_conn_info.last_ssid.SSID.length) +
	     NLA_HDRLEN) +
	    (QDF_MAC_ADDR_SIZE + NLA_HDRLEN) +
	    (sizeof(hdd_sta_ctx->cache_conn_info.chan_freq) + NLA_HDRLEN) +
	    (sizeof(hdd_sta_ctx->cache_conn_info.noise) + NLA_HDRLEN) +
	    (sizeof(hdd_sta_ctx->cache_conn_info.signal) + NLA_HDRLEN) +
	    ((sizeof(uint32_t) + NLA_HDRLEN) * 2) +
	    (sizeof(hdd_sta_ctx->cache_conn_info.txrate.nss) + NLA_HDRLEN) +
	    (sizeof(hdd_sta_ctx->cache_conn_info.roam_count) + NLA_HDRLEN) +
	    (sizeof(hdd_sta_ctx->cache_conn_info.last_auth_type) + NLA_HDRLEN) +
	    (sizeof(hdd_sta_ctx->cache_conn_info.dot11mode) + NLA_HDRLEN) +
	    (sizeof(uint32_t) + NLA_HDRLEN) +
	    (NLA_HDRLEN * 4);

	if (hdd_sta_ctx->cache_conn_info.conn_flag.vht_present)
		nl_buf_len +=
		   sizeof(hdd_sta_ctx->cache_conn_info.vht_caps) + NLA_HDRLEN;
	if (hdd_sta_ctx->cache_conn_info.conn_flag.ht_present)
		nl_buf_len +=
		   sizeof(hdd_sta_ctx->cache_conn_info.ht_caps) + NLA_HDRLEN;
	...
	nl_buf_len += hdd_he_op_len + NLA_HDRLEN;
