...
const struct nla_policy wlan_hdd_wifi_config_policy[
	...
	[QCA_WLAN_VENDOR_ATTR_CONFIG_DBAM] = {.type = NLA_U8 },
        ...
        //Add for ULL TX 20M
	[QCA_WLAN_VENDOR_ATTR_CONFIG_MAX_TX_BANDWIDTH] = {.type = NLA_U16 },
...
//OPLUS_WIFI_VENDOR_EDIT_START
//Add for ULL TX 20M
static int hdd_set_max_tx_bandwidth_config(struct hdd_adapter *adapter,
					const struct nlattr *attr)
{
	uint8_t tx_bw;
	uint8_t defualt_bw;
	...
	qdf_mem_copy(peer_mac_addr, &adapter->session.station.conn_info.bssid, QDF_MAC_ADDR_SIZE);
	wma = cds_get_context(QDF_MODULE_ID_WMA);
	if(!wma)
		return -EINVAL;

	ret = wma_set_peer_param(wma, peer_mac_addr, WMI_PEER_CHWIDTH,
			tx_bw, adapter->vdev_id);
	return ret;
}
//OPLUS_WIFI_VENDOR_EDIT_END
...
static const struct independent_setters independent_setters[] = {
	...
	{QCA_WLAN_VENDOR_ATTR_CONFIG_DBAM,
	 hdd_set_dbam_config},
        ...
        //OPLUS_WIFI_VENDOR_EDIT_START
        //Add for ULL TX 20M
	{QCA_WLAN_VENDOR_ATTR_CONFIG_MAX_TX_BANDWIDTH,
