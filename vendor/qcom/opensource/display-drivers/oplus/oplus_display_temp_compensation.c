...
int oplus_temp_compensation_init(void *dsi_panel)
{
	...
	unsigned int reg_repeat = 4;
	static unsigned int failure_count = 0;
	struct dsi_panel *panel = dsi_panel;
	struct dsi_parser_utils *utils = NULL;
	struct oplus_temp_compensation_params *p_oplus_temp_compensation_params = oplus_temp_compensation_get_params();

	TEMP_COMPENSATION_INFO("oplus_temp_compensation_init start\n");

	if (registered) {
		TEMP_COMPENSATION_INFO("oplus_temp_compensation_init already inited\n");
		return rc;
	}
	...
	rc = utils->read_u32(utils->data, "oplus,temp-compensation-config", &config);
	if (rc) {
	...
	if (oplus_temp_compensation_is_supported()) {
		...
		rc = utils->read_u32(utils->data, "oplus,temp-compensation-reg-repeat", &reg_repeat);
		if (rc) {
			TEMP_COMPENSATION_INFO("oplus,temp-compensation-reg-repeat not defined, set reg_repeat = 4\n", rc);
			p_oplus_temp_compensation_params->reg_repeat = 4;
		} else {
			TEMP_COMPENSATION_INFO("read oplus,temp-compensation-reg-repeat: %u\n", reg_repeat);
			p_oplus_temp_compensation_params->reg_repeat = reg_repeat;
		}
		...
		TEMP_COMPENSATION_INFO("temp_compensation not enable\n");
	}

	OPLUS_TEMP_COMPENSAITON_TRACE_END("oplus_temp_compensation_init");

	TEMP_COMPENSATION_INFO("oplus_temp_compensation_init end\n");
...
int oplus_temp_compensation_register_ntc_channel(void *dsi_display)
{
	...
	TEMP_COMPENSATION_INFO("oplus_temp_compensation_register_ntc_channel start\n");

	if (registered) {
		TEMP_COMPENSATION_INFO("panel ntc already registered\n");
		return rc;
	}
	...
	TEMP_COMPENSATION_INFO("oplus_temp_compensation_register_ntc_channel end\n");
