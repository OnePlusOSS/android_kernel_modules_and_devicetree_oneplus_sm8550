...
void sde_encoder_adfr_prepare_commit(void *crt, void *enc, void *conn) {
	...
	*/
	...
	if (__oplus_get_power_status() != SDE_MODE_DPMS_ON) {
...
static int dsi_panel_auto_minfps_check(struct dsi_panel *panel, u32 extend_frame)
{
	...
	if (h_skew == SDC_ADFR) {
		if (oplus_adfr_auto_mode == OPLUS_ADFR_AUTO_OFF) {
			if (refresh_rate == 120) {
				if (str_equal(panel->name, "AC052 S 3 A0001 dsc cmd mode panel")) {
					if ((extend_frame < OPLUS_ADFR_AUTO_MIN_FPS_MAX) || (extend_frame > OPLUS_ADFR_AUTO_MIN_FPS_10HZ)) {
						/* The highest frame rate is the most stable */
						extend_frame = OPLUS_ADFR_AUTO_MIN_FPS_MAX;
					}
				} else if (str_equal(panel->name, "AC052 P 3 A0003 dsc cmd mode panel")) {
		...
		if (str_equal(panel->name, "AC052 S 3 A0001 dsc cmd mode panel")) {
			if ((extend_frame < OPLUS_ADFR_AUTO_MIN_FPS_60HZ) || (extend_frame > OPLUS_ADFR_AUTO_MIN_FPS_10HZ)) {
				/* The highest frame rate is the most stable */
		...
		int ctrl_idx, u32 extend_frame)
{
	...
	if (rc)
		goto exit;
	if(extend_frame == 119 && str_equal(panel->name, "AC052 P 1 A0002 dsc cmd mode panel")) {
		VRR_INFO("special process of 1hz");
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_QSYNC_MIN_FPS_1HZ);
	} else {
...
void oplus_adfr_handle_idle_mode(void *sde_enc_v, int enter_idle)
{
	...
	if (str_equal(panel->name, "AC052 S 3 A0001 dsc cmd mode panel")) {
		VRR_DEBUG("special process of idle mode\n");
		return;
	}
