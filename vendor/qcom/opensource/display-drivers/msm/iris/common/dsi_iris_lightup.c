...
static uint32_t iris_model_cnt;
...
static int32_t _iris_alloc_desc_buf(struct dsi_cmd_desc **cmds, int cmd_cnt)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		...
		*cmds  = kvzalloc(cmd_size, GFP_KERNEL);
	if (!(*cmds)) {
...
static void _iris_count_cnn_model(const struct iris_cmd_header *phdr)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	uint8_t opt = 0;
	...
	if (phdr->ip_type != SR_LUT)
		return;

	opt = phdr->opt_and_link & 0xFF;
	if (opt == 0xA0)
		return;

	opt /= 0x10;
	opt += 1;
	if (opt > iris_model_cnt)
		iris_model_cnt = opt;
}

uint32_t iris_get_cnn_model_count(void)
{
	return iris_model_cnt;
}
...
static int32_t _iris_parse_ocp_read_path(const struct device_node *np,
		struct iris_cfg *pcfg)
{
	int32_t rc = 0;
	u8 *key = "pxlw,ocp_read_by_i2c";
	struct iris_dts_ops *p_dts_ops = iris_get_dts_ops();

	if (!p_dts_ops)
		return -EINVAL;

	rc = p_dts_ops->read_u32(np, key, &(pcfg->ocp_read_by_i2c));
	if (rc) {
		IRIS_LOGE("%s(), failed to %s, return: %d",
			__func__, key, rc);
	...
	IRIS_LOGI("%s(), pxlw ocp read path: %s", __func__, pcfg->ocp_read_by_i2c ? "i2c" : "dsi");
		...
		struct iris_cfg *pcfg)
{
	...
	payload = iris_get_ipopt_payload_data(IRIS_IP_PWIL, pcfg->id_piad_blend_info, 2);
	if (!payload) {
...
static void _iris_parse_default_aux_size(struct iris_cfg *pcfg)
{
	...
	if (!payload) {
		IRIS_LOGE("%s(), failed to find: %02x", __func__, IRIS_IP_DSC_DEC_AUX);
		return;
	}
...
static int _iris_parse_cmd_list(struct device_node *np)
{
	...
	do {
		...
		if (pcfg->iris_chip_type == CHIP_IRIS7) {
			...
			//when there is only one array of command list
			if (rc && cmd_idx != IRIS_DTSI_PIP_IDX_START)
				return 0;
			else if (rc)
				return rc;
		}
...
static int _iris_parse_subnode(struct dsi_display *display, void *node)
{
	...
	iris_parse_memc_param0(lightup_node);
...
static int _iris_parse_iris_golden_fw(struct device_node *np, const char *chip_name)
{
	...
	rc = of_property_match_string(np, "iris-calibration-golden-fw-list", chip_name);
	if (unlikely(rc < 0)) {
		IRIS_LOGD("%s(), doesn't contain %s", __func__, chip_name);
		...
		rc = of_property_read_string_array(np, "iris-calibration-golden-fw-list",
			names_array, count);
		if (rc < 0) {
			IRIS_LOGD("Failed to invoke of_property_read_string_array()");
			goto default_fw_name;
		}
		if (index + 3 < count) {
...
default_fw_name:
	...
	pcfg->ccf2_name = NULL;
	pcfg->ccf3_name = NULL;
	IRIS_LOGI("Can not parse golden fw name, use default name");
	return rc;
}
...
int iris_parse_param(struct dsi_display *display)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7P)
		_iris_parse_iris_golden_fw(display->panel_node, "iris7p");
	else if (pcfg->iris_chip_type == CHIP_IRIS7)
		_iris_parse_iris_golden_fw(display->panel_node, "iris7");
...
static void __iris_mult_addr_pad(uint8_t **p, uint32_t *poff, uint32_t left_len)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
	iris_mult_addr_pad_i7(p, poff, left_len);
	else if (pcfg->iris_chip_type == CHIP_IRIS7P)
	iris_mult_addr_pad_i7p(p, poff, left_len);
}
	...
	uint32_t left_len, struct dsi_cmd_desc *pdesc)
{
	...
	iris_set_ocp_type(*p, ocp_header);
	*p += tmp_len;
	*poff += tmp_len;
	...
	uint32_t left_len, struct dsi_cmd_desc *pdesc)
{
	...
	*poff += tmp_len;
	...
	iris_set_ocp_type(*p, ocp_header);
	iris_set_ocp_base_addr(*p, base_addr + (tmp_len-8));
	*p += 8;
	*poff += 8;
	memcpy(*p, ptmp, wc-tmp_len);
	*p += wc-tmp_len;
		...
		struct iris_cmd_comp *pcmd_comp)
{
	...
	for (i = 0; i < pcmd_comp->cnt; i++) {
		...
		if (left_len >= wc) {
			...
			ocp_header &= 0xFF0000FF;
			ocp_header |= (wc << 8) | 0x1;
			iris_set_ocp_type(ptr, ocp_header);
			ptr += wc;
...
int32_t iris_send_ipopt_cmds(int32_t ip, int32_t opt_id)
{
	...
	cmd = NULL;
	cmd = kvzalloc(cmd_comp.cnt * sizeof(struct dsi_cmd_desc), GFP_KERNEL);
	if (!cmd) {
		IRIS_LOGE("%s(), failed to alloc desc cmd", __func__);
		return -ENOMEM;
	}
	memcpy(cmd, cmd_comp.cmd, cmd_comp.cnt * sizeof(struct dsi_cmd_desc));
	cmd_comp.cmd = cmd;

	cmd_comp.op_type = IRIS_PQUPDATE_OP;
	rc = _iris_send_cmds(pcfg->panel, &cmd_comp, PATH_DSI);
	kvfree(cmd);
	return rc;
}
...
int iris_init_update_ipopt_t(uint8_t ip, uint8_t opt_old, uint8_t opt_new, uint8_t chain)
{
	...
	if (!pcfg->pq_update_cmd.update_ipopt_array) {
		IRIS_LOGE("%s(), update_ipopt_array is NULL", __func__);
		return -EINVAL;
	}
		...
		for (opt_index = 0; opt_index < pip_index[ip_index].opt_cnt; opt_index++) {
			...
			for (desc_index = 0; desc_index < pip_index[ip_index].opt[opt_index].cmd_cnt; desc_index++) {
				...
				if (pcfg->iris_chip_type == CHIP_IRIS7)
					...
					kvfree(pip_index[ip_index].opt[opt_index].cmd[desc_index].msg.tx_buf);
...
static void _iris_pre_rx_cmd(void)
{
	...
	iris_send_ipopt_cmds(IRIS_IP_RX, pcfg->id_rx_dphy);
}
...
int iris_enable(struct dsi_panel *panel, struct dsi_panel_cmd_set *on_cmds)
{
	...
	if ((pcfg->iris_chip_type == CHIP_IRIS7P) && (iris_platform_get() == IRIS_FPGA))
...
int iris_dbgfs_cont_splash_init(struct dsi_display *display)
{
	struct iris_cfg *pcfg = iris_get_cfg();
...
static void _iris_fw_create_name(struct dsi_display *display, char *name)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		len = sprintf(np, "%s", "pxlw_i7");
	else if (pcfg->iris_chip_type == CHIP_IRIS7P)
		len = sprintf(np, "%s", "pxlw_i7p");
