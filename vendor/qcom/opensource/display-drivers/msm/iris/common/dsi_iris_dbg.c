		...
		if (pcfg->dual_enabled) {
			if (pcfg->pwil_mode == FRC_MODE) {
				if (pcfg->frcgame_pq_guided_level > 1)
					strlcpy(pcfg->display_mode_name, "DUAL-MEMC-SR",
						sizeof(pcfg->display_mode_name));
				else
					strlcpy(pcfg->display_mode_name, "DUAL-MEMC",
						sizeof(pcfg->display_mode_name));
			} else
			...
			if (pcfg->pwil_mode == FRC_MODE) {
				if (pcfg->iris_chip_type == CHIP_IRIS7) {
					if (pcfg->frcgame_pq_guided_level > 1)
						strlcpy(pcfg->display_mode_name, "SINGLE-MEMC-SR",
								sizeof(pcfg->display_mode_name));
					else
						strlcpy(pcfg->display_mode_name, "SINGLE-MEMC",
								sizeof(pcfg->display_mode_name));
				} else if (pcfg->iris_chip_type == CHIP_IRIS7P) {
					const char *status = iris_ptsr_status();

					if (status == NULL)
						strlcpy(pcfg->display_mode_name, "SINGLE-MEMC",
								sizeof(pcfg->display_mode_name));
					else
						strlcpy(pcfg->display_mode_name, status,
								sizeof(pcfg->display_mode_name));
				}
				...
				if (pcfg->iris_chip_type == CHIP_IRIS7) {
					if (pcfg->pt_sr_enable)
						strlcpy(pcfg->display_mode_name, "SINGLE-PT-SR",
								sizeof(pcfg->display_mode_name));
					else
						strlcpy(pcfg->display_mode_name, "SINGLE-PT",
								sizeof(pcfg->display_mode_name));
				} else if (pcfg->iris_chip_type == CHIP_IRIS7P) {
					const char *status = iris_ptsr_status();
...
int iris_debug_display_mode_get(char *kbuf, int size, bool debug)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		rc = iris_debug_display_mode_get_i7(kbuf, size, debug);
	else if (pcfg->iris_chip_type == CHIP_IRIS7P)
		rc = iris_debug_display_mode_get_i7p(kbuf, size, debug);
...
int iris_debug_pq_info_get(char *kbuf, int size, bool debug)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		rc = iris_debug_pq_info_get_i7(kbuf, size, debug);
	//else if (pcfg->iris_chip_type == CHIP_IRIS7P)
	//	rc = iris_debug_pq_info_get_i7p(kbuf, size, debug);
