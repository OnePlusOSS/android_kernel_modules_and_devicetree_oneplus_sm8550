...
static u32 sdr_to_edr_two_backlight(u32 nit)
{
	u32 sdr_nit = nit * 10000;
	u32 edr_nit = 0;

	if (sdr_nit <= 150 * 10000)
		edr_nit = sdr_nit * 2;
	else if (sdr_nit > 150 * 10000 && sdr_nit <= IRIS_EDR_BK * 10000)
		edr_nit = (1650 * 10000 + sdr_nit * 5) / 8;
	else
		edr_nit = sdr_nit;
...
static u32 sdr_to_edr_two_and_half_backlight(u32 nit)
{
	u32 sdr_nit = nit * 10000;
	u32 edr_nit = 0;

	if (sdr_nit <= 150 * 10000)
		edr_nit = sdr_nit / 10 * 25;
	else if (sdr_nit > 150 * 10000 && sdr_nit <= IRIS_EDR_BK * 10000)
		edr_nit = (4950 * 10000 + sdr_nit * 7) / 16;
	else
		edr_nit = sdr_nit;
...
static u32 sdr_to_edr_triple_backlight(u32 nit)
{
	u32 sdr_nit = nit * 10000;
	u32 edr_nit = 0;
	...
	if (sdr_nit <= 150 * 10000)
		...
		edr_nit = 4125 * 10000 / 10 + sdr_nit * 1 / 4;
	else
		edr_nit = sdr_nit;
...
int iris_sdr_to_edr_nits(u32 nit)
{
	u32 edr_nit = 0;
	struct quality_setting *pqlt_cur_setting = &iris_setting.quality_cur;
	...
	switch (backlight_ratio) {
		case 0x1:
			edr_nit = sdr_to_edr_two_backlight(nit);
			break;
		case 0x2:
			edr_nit = sdr_to_edr_two_and_half_backlight(nit);
			break;
		case 0x3:
			edr_nit = sdr_to_edr_triple_backlight(nit);
			break;
		default:
			edr_nit = sdr_to_edr_two_backlight(nit);
			IRIS_LOGI("default two backlight edr_nit");
			break;
...
void iris_EDR_backlight_ctrl(u32 hdr_nit, u32 ratio_panel)
{
	...
	//assert(ratio_panel>=1000);
	if (pcfg->panel && ratio_panel < 10000) {
		IRIS_LOGE("ratio_panel need >= 10000");
		return;
	}
	IRIS_LOGI("iris_EDR_backlight_ctrl hdr_nit %u scale %u", hdr_nit, ratio_panel);
	ratio = ratio_panel * 10000 / 10000;
	sdr_nit = hdr_nit * 10000 / ratio;

	if (hdr_nit < IRIS_EDR_BK) {
		edr_final_nit = iris_sdr_to_edr_nits(sdr_nit);
		edr_final_ratio = edr_final_nit/sdr_nit;
		if (edr_final_ratio >= ratio) {
			edr_cur_ratio = (ratio - 1 * 10000) * 10000 / (edr_final_ratio - 1 * 10000);
			sdr_nit = (sdr_nit > IRIS_EDR_BK) ? IRIS_EDR_BK : sdr_nit;
			sdr_nit = (sdr_nit < 150) ? 150 : sdr_nit;
			edr_cur_nit = IRIS_EDR_BK - (IRIS_EDR_BK - sdr_nit) * edr_cur_ratio / 10000;
		} else {
			edr_cur_nit = hdr_nit;
		}
	} else {
	...
	edr->curr_ratio = ratio_panel;
	iris_EDR_backlight_status(edr);

	if (edr_state && ratio == 10000) {
...
void iris_sdr2hdr_level_set(u32 level)
{
	...
	if (level != SDR2HDR_Bypass) {
		...
		if (level == SDR2HDR_9) {
			iris_hdr_ai_input_bl(IRIS_EDR_BK, false);
			iris_edr_info_init();
		} else
