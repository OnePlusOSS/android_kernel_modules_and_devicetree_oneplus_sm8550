...
int iris_parse_gpio(void *dev, void *cfg)
{
	...
	pcfg->iris_vdd_0p9v_gpio = of_get_named_gpio(of_node,
			"qcom,iris-vdd-0p9v-gpio", 0);
	IRIS_LOGI("%s(), iris vdd 0p9v gpio %d", __func__,
			pcfg->iris_vdd_0p9v_gpio);
	if (!gpio_is_valid(pcfg->iris_vdd_0p9v_gpio))
		IRIS_LOGW("%s(), iris vdd 0p9v gpio not specified", __func__);
...
void iris_request_gpio(void)
{
	...
	if (gpio_is_valid(pcfg->iris_vdd_0p9v_gpio)) {
		rc = gpio_request(pcfg->iris_vdd_0p9v_gpio, "iris_vdd_0p9v");
		if (rc)
			IRIS_LOGW("%s(), failed to request vdd 0p9v, return: %d", __func__, rc);
	}
...
void iris_release_gpio(void *cfg)
{
	...
	if (gpio_is_valid(pcfg->iris_vdd_0p9v_gpio))
		gpio_free(pcfg->iris_vdd_0p9v_gpio);
...
bool iris_vdd_valid(void)
{
	...
	if (gpio_is_valid(pcfg->iris_vdd_gpio) && gpio_is_valid(pcfg->iris_vdd_0p9v_gpio))
		return true;
...
void iris_enable_vdd(void)
{
	...
	rc = gpio_direction_output(pcfg->iris_vdd_0p9v_gpio, IRIS_GPIO_HIGH);
	if (rc)
		IRIS_LOGE("%s(), unable to set dir for iris vdd 0p9v gpio, return: %d",
				__func__, rc);
	gpio_set_value(pcfg->iris_vdd_0p9v_gpio, IRIS_GPIO_HIGH);
...
void iris_disable_vdd(void)
{
	...
	gpio_set_value(pcfg->iris_vdd_0p9v_gpio, IRIS_GPIO_LOW);
}
