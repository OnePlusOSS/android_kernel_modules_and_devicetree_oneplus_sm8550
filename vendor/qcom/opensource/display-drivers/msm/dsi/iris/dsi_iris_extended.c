		...
		const struct mipi_dsi_msg *msg)
{
	...
	struct mipi_dsi_msg_pxlw *msg_pxlw;

	msg_pxlw = (struct mipi_dsi_msg_pxlw *)msg;
	...
	buf = vmalloc(1024);
	if (!buf) {
		IRIS_LOGE("%s(), mem alloc failed!!!", __func__);
		return;
	}
	...
	/* Delay */
	len += snprintf(buf + len, sizeof(buf) - len, "%02X ", msg_pxlw->wait_ms);

	len += snprintf(buf + len, sizeof(buf) - len, "%02X %02X ",
			(unsigned int)(msg->tx_len) >> 8, (unsigned int)(msg->tx_len) & 0x00FF);
