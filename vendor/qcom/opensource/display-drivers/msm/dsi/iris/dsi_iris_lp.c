...
#if defined(IRIS_HDK_DEV) && defined(CONFIG_PXLW_FPGA_IRIS)
...
void iris_dump_status(void)
{
	...
	for (i = 0; i < len; i++) {
		if (read_path == PATH_I2C) { //use i2c to read
			rc = iris_ioctl_i2c_read(_regs_dump[i], &value);
			if (rc != 0) {
...
void iris_frc_lp_switch(bool frc_mode, bool chain)
{
	...
	if (frc_mode) {
		if (iris_dynamic_power_get()) {
			...
			iris_pmu_power_set(PQ_PWR, false, 0);
			iris_pmu_power_set(PQ_PWR, true, 0);
			usleep_range(1000, 1010);
